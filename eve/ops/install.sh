...
CORE_SRC="${EVE_CORE_SRC:-$REPO/../core/src}"
...
if [ ! -d "$CORE_SRC" ]; then
    echo "REFUSING to install: shared core source directory is missing: $CORE_SRC" >&2
    echo "Fix: deploy core beside eve, or set EVE_CORE_SRC=/absolute/path/to/core/src." >&2
    exit 1
fi
...
if ! (
    ...
    echo "REFUSING to install: Eve cannot import the exact shared core at $CORE_SRC." >&2
