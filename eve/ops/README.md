...
| `install.sh` | resolves the sibling `../core/src`, proves `/usr/bin/python3` imports that exact `inventor_core/__init__.py`, renders both plists for the current checkout into `~/Library/LaunchAgents`, validates them, then `launchctl bootstrap`s them. Refuses a missing or different core — a broken deploy must fail at install time, not silently every 30 minutes. Idempotent (re-run = redeploy). |
...
sibling `../core/src` tree and refuses unless the same `/usr/bin/python3` used
...
CAD-builder tick reaches the core artifact boundary. For a nonstandard layout,
set `EVE_CORE_SRC=/absolute/path/to/core/src` when running `ops/install.sh`.
Core's publication database and retained exact packets live under `state/` and
...
Eve's pytest suite on Python 3.9 and 3.12. The pinned test requirements install
the sibling core package; no credentials are required, and mocked publication
tests prove the outbox blocks a duplicate POST after an ambiguous response.
