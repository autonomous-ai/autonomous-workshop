...
Eve now uses the shared [`../core`](../core/README.md) at two real execution
boundaries: the CAD builder records a content-addressed core artifact manifest,
and the Panda draft adapter uses core's canonical packet builder, durable SQLite
...
core database is publication infrastructure, not a second lifecycle store.
...
| `eve/core_adapter.py` | shared-core artifact, canonical packet, and durable Panda-outbox boundary |
| `eve/publish.py` | static catalog renderer plus core-backed Panda draft adapter |
...
PYTHONPATH=.:../core/src PYTHONWARNINGS=error python3 -m pytest -q
```

`requirements.txt` installs the sibling `autonomous-inventor-core` package. A
source-tree smoke can instead put `../core/src` on
...
installer binds the service to the exact sibling `../core/src` tree, so a stale
globally installed core cannot satisfy the scheduled runtime by accident.

## Current release boundaries

- `ship` is still an inventor-local stage, not proof of a core
  `CadReleaseBundle` or Factory fulfillment contract. The core publication
  product intentionally has only the stage `publication-ready`; it never
  mirrors or advances Eve's queue.
- Publication remains draft/manual. Core persists the intent before POST and
...
- New successful imports write `games/<slug>/published.json` containing the
  exact core `intent_id` and `PublicationReceipt`. Every attempt also updates
...
- A slug is permanently bound to its first selected artifact bytes. Until core
...
- Historical `published.json` responses remain recognized as already imported
  and are not silently rewritten into stronger core receipts.
...
The migration order and exact definition of “core-connected” are in
[`../core/docs/MIGRATION.md`](../core/docs/MIGRATION.md).
