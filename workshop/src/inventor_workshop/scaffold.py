...
{name} is an experimental inventor for **{niche}** built with Inventor
Workshop's `{template}` workshop.
...
2. Edit `src/{package}/workflow.py`: this is the inventor-owned route from
   Make through Inspect and Pack, with Send enabled only when a Shop or
   Delivery Door is needed.
3. Replace the credential-free starter Doors with real model, CAD, and Inspection
   Doors.
4. Add a Shop Door or Delivery Door only when this inventor must Send.

## Run locally

```bash
python3 -m pip install -e ../../workshop -e .
{package} doctor
...
`.workshop/clockwork.sqlite3`. Runtime state and credentials are never
committed.

## Before autonomous or public operation

Keep every Inspection bound to exact artifact bytes. Add real slicer, form,
safety, and physical evidence. Configure budgets and scoped credentials. Send
through Workshop's durable Sender; never treat a local flag or HTTP success as
a verified Stamp.
...
class SmokeTest(unittest.TestCase):
    def test_workflow_exposes_direct_workshop_route(self):
        ...
        self.assertEqual(WORKFLOW.legal_targets("pack"), ("send",))
