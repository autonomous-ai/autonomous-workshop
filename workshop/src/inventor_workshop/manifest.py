...
# Schema-v3 features are a deliberately small, reviewed contract. Inventors
# compose these Workshop capabilities; they do not mint near-synonyms that
# make the registry impossible to understand.
...
class InventorManifest:
    ...
    def core_features(self) -> Sequence[str]:
        ...
        New code and manifests use ``workshop_features``. Keeping this
        read-only alias lets older integrations inspect a v2 manifest while
        they migrate without exposing the old name in generated projects.
        """
    ...
    def parse(cls, raw: Mapping[str, Any], path: Path) -> "InventorManifest":
        ...
        if type(schema_version) is not int or schema_version not in (1, 2, 3):
            raise ManifestError("%s: schema_version must be 1, 2, or 3" % path)
        ...
        conflicts = sorted(other_feature_fields.intersection(raw))
        if conflicts:
            raise ManifestError(
                "%s: schema_version %s uses %s, not %s"
                % (path, schema_version, feature_field, conflicts)
            )
        if schema_version in (2, 3) and "checks" not in raw:
            raise ManifestError(
                "%s: checks is required for schema_version %s"
                % (path, schema_version)
            )
        if schema_version == 1 and "checks" in raw:
            raise ManifestError("%s: checks requires schema_version 2 or 3" % path)
