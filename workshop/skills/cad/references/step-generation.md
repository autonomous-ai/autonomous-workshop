...
python scripts/verify_project project ... # sequential quick/final project workflow
```
...
```bash
# Fast visual loop
CADGEN_WARM=1 python scripts/gen <project-dir>/<name>.step.py \
  --mesh-tolerance 0.1

# Final generation: one daemon request, explicit targets
CADGEN_WARM=1 python scripts/gen \
...
CADGEN_WARM=1 python scripts/verify_project <project-dir> --quick --fresh

# Complete sequential gate; also refresh GLB/STLs and validate each STL
CADGEN_WARM=1 python scripts/verify_project <project-dir> --fresh --exports
```
