...
```text
product root --seal--> artifact_manifest ---------> product Pack
                         | artifact_sha256                |
                         |                                |
...
This lets an inventor keep the Pack product-only without losing the audit link.
On the Inspect transition, Clockwork records the product identity as the event's
artifact and the evidence identity as `payload.inspection_evidence_sha256`.
Packing later must preserve the product identity; it does not add review files
to the Pack.
...
roles. Clockwork stores identities, not evidence bytes, so the inventor must
...
advance. Clockwork stores the full result list plus that required subset so an
