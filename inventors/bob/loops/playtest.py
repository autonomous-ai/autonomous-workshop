...
``toys/<slug>/playtest/engine.py`` is a plain Python module (stdlib only)
exposing exactly:
...
- transcript: ``toys/<slug>/playtest/table_<k>.json`` —
...
- summary: ``toys/<slug>/playtest/table_report.json`` — one row per table:
...
def _game_dir(slug, home=None):
    return os.path.join(_home(home), "toys", slug)
...
def idea_sha(slug, home=None):
    """sha256 hex of toys/<slug>/idea.json — the version every verdict binds to."""
...
def build_engine_prompt(slug, home=None):
    ...
    return """You are writing toys/%(slug)s/playtest/engine.py — a complete,
...
THE IDEA (toys/%(slug)s/idea.json, sha256 %(sha)s):
%(idea)s
...
def run_sim(slug, home=None, n_games=1000, seed=0):
    ...
    Writes ``toys/<slug>/playtest/sim_report.json`` (atomic) carrying the
