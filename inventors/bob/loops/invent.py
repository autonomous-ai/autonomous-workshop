...
1. composes the stage prompt (the matching ``.claude/agents/*.md`` body +
   the game's own artifacts under ``toys/<slug>/``),
...
def _handle_rules_gated(step):
    ...
    prompt = "\n\n".join(filter(None, [
        ...
        framing,
        "## The rules document (toys/%s/rules.md)\n%s"
...
def _handle_built(step):
    ...
    prompt = "\n\n".join([
        ...
        "## The parts brief (toys/%s/brief.md)\n%s" % (slug, brief),
        "## Output contract\nWrite the part files under toys/%s/parts/. "
