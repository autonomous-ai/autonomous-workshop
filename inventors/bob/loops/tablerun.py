...
that — the engine (toys/<slug>/playtest/engine.py) is the only rules
...
binding spec): transcripts to ``toys/<slug>/playtest/table_<k>.json``,
summary to ``toys/<slug>/playtest/table_report.json``, every file embedding
...
def run_tables(slug, home=None, n_tables=N_TABLES, seed=0):
    ...
    Writes ``toys/<slug>/playtest/table_<k>.json`` per game and the
