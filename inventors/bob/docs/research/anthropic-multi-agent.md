...
**Bob:** One lead "Inventor" process per game owns the plan (theme, mechanics hypothesis, target audience) and writes it to disk *first* (e.g. `toys/<slug>/PLAN.md`). Workers — mechanic researcher, rules drafter, LLM playtesters, art/component briefer, marketplace-fit checker — are spawned `claude -p` calls that read the plan file, do one job, and write one artifact back. The lead never regenerates the plan from memory; it re-reads it from disk each tick (this is also what makes launchd ticks resumable).
