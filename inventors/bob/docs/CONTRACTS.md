...
| send | `harness/send.py`, `harness/workshop_runtime.py` | Implements the Shop Door protocol researched in `docs/research/publish-contract.md` through `workshop`. `pack_game(slug)` selects Bob's product files, then Workshop `pack_artifact()` creates a secret-scanned `PackedArtifact`; `inspect_pack()` verifies it. `validate(slug) -> list[str]` also checks RULES.md, cover, copy caps, and Bob's pinned Shop Door identity. `send_draft(slug)` uses `Sender` + `ShopDoor` with Clockwork at `state/inventor-workshop.sqlite3`, always requests a private **model-only** draft, records intent before HTTP, validates the typed `Stamp`, and persists `workshop_intent_id` in `send.json`; timeout/5xx/invalid-201 outcomes become unknown and cannot POST again. It never uploads thumbnails or writes `use_case`/`story_blocks`; Factory enriches copy, images, and video on the server. The historical `curate(slug)` name remains only to fail closed before credentials or HTTP. `BOB_SEND_VIA=workshop` is the only autonomous mode and the default. `harness/export_box.py`/`bob export` preserve the historical text2game payload for **local inspection only**: `push_box` fails before subprocess/network, and neither path writes send authority or advances the queue. Obsolete `mark-published` also fails closed. A legacy remote observation without complete Workshop intent/content identity remains stranded. `flip_public(slug, price_cents)` reuses the same intent and requires exact owner/artifact/history/active-USD-listing/price/SKU readback; `reconcile_public(slug)` and other authenticated GETs may inspect server-generated content but never replace it. `unpublish(slug)` is the reversible kill switch. `BOB_SEND_DRY_RUN=1` still creates and verifies the Workshop pack without network; `send.json` carries `send_authority: none`, so it is rehearsal, never a Stamp. Older publish/Portal/Panda/Foundation/Core names and files are read only at a conflict-checked compatibility edge. |
