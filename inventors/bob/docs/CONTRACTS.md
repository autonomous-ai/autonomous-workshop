...
| publish | `harness/publish.py`, `harness/core_runtime.py` | Implements docs/research/publish-contract.md Path B through Foundation. `build_zip` produces a secret-scanned canonical Foundation packet with an embedded content-addressed manifest; `validate(slug) -> list[str]` also checks RULES.md, cover, copy caps, and the pinned Bob identity. `import_draft(slug)` uses `PandaPublicationCoordinator` + `state/inventor-core.sqlite3`, always requests draft, records the effect before HTTP, and persists `core_intent_id` in `published.json`; timeout/5xx/invalid-201 outcomes become unknown and cannot POST again. A legacy remote receipt without complete Foundation intent/content identity is explicitly stranded: Bob cannot adopt its owner or safely retry its slug. The current owner must resolve that effect separately, then a distinct pinned Bob principal may re-import under a new slug. The Foundation product key is stable per slug, so changing bytes cannot bypass an unknown intent; corrected bytes need a new slug until Panda supplies content/idempotency proof. `curate(slug)` retains Bob's use-case/story-block API. `flip_public(slug, price_cents)` reuses the same intent and requires Foundation's exact owner/artifact/history/active-USD-listing/price/SKU readback; `reconcile_public(slug)` never resends `/publish` and performs only auth rotation plus GET readback for a Foundation-recorded ambiguous flip. `unpublish(slug)` remains the reversible kill switch. `BOB_PUBLISH_DRY_RUN=1` still produces and verifies the Foundation packet without network; its `published.json` carries `publication_authority: none`, so arming real effects promotes it through the Foundation outbox instead of treating the rehearsal as an idempotency receipt. |
