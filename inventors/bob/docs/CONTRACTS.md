...
| invent loop | `loops/invent.py` | `tick(step) -> None`: state-machine dispatch table `STEP_HANDLERS: dict[state, handler]`; each handler composes a prompt, runs one agent, validates output, writes artifacts under `games/<slug>/`, scores at the review boundary, then advances or parks. Bob's idea-selection requests load only the immediate root `TASTE.md` through Workshop, include its exact content/SHA-256/byte count for both ideator and triage, verify it did not change during selection, and persist the complete binding in `idea.json`; `knowledge/TASTE.md` is evidence only. |
