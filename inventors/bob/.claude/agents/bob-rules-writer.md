...
document at `toys/<slug>/RULES.md`. You are a generator: you never see reward
...
Also emit/refresh the machine block `toys/<slug>/idea.json` fields you own:
...
- If you find the spark unimplementable while writing (a real hole, not a
  style itch), STOP and write `toys/<slug>/rules_blocker.md` naming the hole
