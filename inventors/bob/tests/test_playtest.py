...
def _make_home(idea=None, engine_sha=None):
    """Build a temp BOB_HOME with toys/testrace/{idea.json, playtest/engine.py}.
    ...
    game_dir = os.path.join(home, "toys", "testrace")
...
class LoaderTest(unittest.TestCase):
    def setUp(self):
        ...
        self.engine_path = os.path.join(
            self.home, "toys", "testrace", "playtest", "engine.py")
...
class RunSimTest(unittest.TestCase):
    ...
    def test_writes_report_with_idea_sha_and_verdicts(self):
        report = playtest.run_sim("testrace", home=self.home,
                                  n_games=150, seed=0)
        path = os.path.join(self.home, "toys", "testrace", "playtest",
                            "sim_report.json")
...
def _goodgame_assumptions(home):
    engine = playtest.load_engine(
        os.path.join(home, "toys", "testrace", "playtest", "engine.py"))
