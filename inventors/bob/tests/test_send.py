"""Tests for harness/send.py — the Layer-1 validator wall-by-wall, the
multipart encoder against golden bytes, idempotency, dry-run, and the flip's
refusals. Zero network: every test monkeypatches send._http; a test that
...
from harness import ledger, send, queue

SLUG = "tower-duel"

GOOD_DESCRIPTION = (
    "A tactical tower-stacking duel for 2-4 players, 30 minutes. "
    + send.DISCLOSURE_LINE
)
GOOD_TAGS = ["board-game", "3d-print", "cadquery", send.AI_TAG]
...
class SendHome(unittest.TestCase):
    """Temp BOB_HOME + a synthetic, fully-green game dir. Each wall test
    breaks exactly one brick and asserts the validator names it."""

    ENV_KEYS = (
        "BOB_HOME", "BOB_SEND_DRY_RUN", "BOB_PUBLISH_DRY_RUN",
        "BOB_PRICE_OVERRIDE",
        "BOB_TELEGRAM_TOKEN", "BOB_TELEGRAM_CHAT", "BOB_FACTORY_TOKEN",
        "BOB_FACTORY_USER_ID", "BOB_SHOP_API", "BOB_PORTAL_API",
        "BOB_PANDA_API", "BOB_SHOP_ALLOWED_ORIGINS",
        "BOB_PORTAL_ALLOWED_ORIGINS", "BOB_PANDA_ALLOWED_ORIGINS",
        "BOB_SHOP_OWNER_ID", "BOB_PORTAL_OWNER_ID", "PORTAL_OWNER_ID",
        "PANDA_OWNER_ID", "BOB_WORKSHOP_SRC", "BOB_FOUNDATION_SRC",
        "BOB_CORE_SRC",
    )
    ...
    def setUp(self):
        ...
        self._orig_http = send._http
        send._http = _fail_http  # default: offline; tests opt in
        self._orig_entries = send.MAX_ZIP_ENTRIES
        self._orig_bytes = send.MAX_ZIP_BYTES

    def tearDown(self):
        send._http = self._orig_http
        send.MAX_ZIP_ENTRIES = self._orig_entries
        send.MAX_ZIP_BYTES = self._orig_bytes
        for key in self.ENV_KEYS:
    ...
    def gdir(self):
        return os.path.join(self.home, "toys", SLUG)
    ...
    def make_auth(self, user_id="b0b" * 8, pinned=None, legacy=False):
        auth = {
            "access_token": "acc-1",
            "refresh_token": "ref-1",
            "user": {"id": user_id, "username": "bob"},
            "shop_owner_id": pinned if pinned is not None else user_id,
        }
        if legacy:
            auth["bob_user_id"] = auth.pop("shop_owner_id")
        filename = (
            send.LEGACY_PANDA_AUTH_FILE if legacy else send.SHOP_AUTH_FILE
        )
        path = os.path.join(self.home, "state", filename)
    ...
    def write_dry_projection(self):
        identity = send._workshop_pack_identity(send.pack_game(SLUG))
        record = {
            "dry_run": True,
            "send_authority": "none",
            "workshop_artifact_sha256": identity["artifact_sha256"],
            "workshop_pack_sha256": identity["pack_sha256"],
            "workshop_contract": "inventor_workshop.artifacts/v1",
        }
        send.write_send_projection(SLUG, record)
        return record
    ...
    def import_workshop_draft(self):
        ...
        send._http = fake
        return send.send_draft(SLUG)


class TestValidatorWalls(SendHome):
    def assert_wall(self, needle):
        problems = send.validate(SLUG)
    ...
    def test_green_game_validates_clean(self):
        self.green()
        self.assertEqual(send.validate(SLUG), [])

    def test_missing_game_dir(self):
        self.assertTrue(send.validate(SLUG)[0].startswith("game dir"))
    ...
    def test_description_over_cap(self):
        self.green()
        self.write_listing(description="z" * 900 + send.DISCLOSURE_LINE)
        self.assert_wall("chars > %d cap" % send.MAX_DESCRIPTION)
    ...
    def test_too_many_tags(self):
        self.green()
        self.write_listing(tags=[send.AI_TAG] + ["t%d" % i
                                                    for i in range(10)])
    ...
    def test_missing_ai_created_tag(self):
        ...
        self.assert_wall(send.AI_TAG)

    def test_tag_over_40_chars(self):
        self.green()
        self.write_listing(tags=[send.AI_TAG, "x" * 41])
        self.assert_wall("41 chars")

    def test_zip_entry_cap(self):
        self.green()
        send.MAX_ZIP_ENTRIES = 1  # synthetic cap; real cap needs 4097 files
        self.assert_wall("entries")

    def test_zip_size_cap(self):
        self.green()
        send.MAX_ZIP_BYTES = 10
        self.assert_wall("bytes")

    def test_handbuilt_zip_is_not_a_workshop_packet(self):
        self.green()
        pp = os.path.join(self.gdir(), "pack")
        ...
        with self.assertRaises(send.SendError) as ctx:
            send._workshop_pack_identity(bad)
        self.assertIn("Workshop rejected", str(ctx.exception))
    ...
    def test_auth_wrong_account(self):
        ...
        self.assert_wall("Shop Door owner id")


class TestBuildZip(SendHome):
    def test_strips_secrets_and_process_artifacts(self):
        ...
        for name in (".env", "secrets.json", "sim.jsonl", "key.pem",
                     "send.json", "listing.json", ".DS_Store"):
            with open(os.path.join(g, name), "w") as fh:
                fh.write("x")
        path = send.pack_game(SLUG)
    ...
    def test_deterministic_rebuild(self):
        self.green()
        p1 = send.pack_game(SLUG)
        with open(p1, "rb") as fh:
            first = fh.read()
        p2 = send.pack_game(SLUG)
        with open(p2, "rb") as fh:
            second = fh.read()
        self.assertEqual(first, second)

    def test_workshop_unavailable_fails_closed(self):
        self.green()
        os.environ["BOB_WORKSHOP_SRC"] = os.path.join(
            self.home, "missing-workshop"
        )
        with self.assertRaises(send.SendError) as ctx:
            send.pack_game(SLUG)
        self.assertIn("workshop", str(ctx.exception).lower())

    def test_workshop_secret_scan_blocks_publishable_file(self):
        self.green()
...
class TestHttpRedirectSafety(unittest.TestCase):
    def test_authenticated_redirect_is_returned_and_never_followed(self):
        ...
        class FakeOpener:
            def open(self, request, timeout=None):
                ...
                raise send.urllib.error.HTTPError(
        ...
        with mock.patch.object(send.urllib.request, "build_opener",
                               side_effect=fake_build_opener):
            status, headers, body = send._http(
                "GET", "https://portal.example/designs/one",
                headers={"Authorization": "Bearer secret"}, timeout=7)
        self.assertEqual(status, 302)
        self.assertEqual(body, b"redirect refused")
        self.assertEqual(calls, [("https://portal.example/designs/one",
                                  "Bearer secret", 7)])
        self.assertEqual(len(built_with), 1)
        self.assertIsInstance(built_with[0], send._NoRedirectHandler)
    ...
    def test_http_error_body_is_bounded_and_closed(self):
        ...
        with mock.patch.object(send.urllib.request, "build_opener",
                ...
                mock.patch.object(send, "MAX_HTTP_RESPONSE_BYTES", 8):
            with self.assertRaises(send.SendError) as ctx:
                send._http("GET", "https://portal.example/designs/one")
    ...
    def test_curl_fallback_bounds_body_and_keeps_bearer_off_argv(self):
        ...
        with mock.patch("subprocess.run", side_effect=fake_run):
            status, headers, body = send._http_curl(
                "POST", "https://portal.example/designs/import",
    ...
    def test_curl_fallback_rejects_oversized_body(self):
        ...
        with mock.patch("subprocess.run", side_effect=fake_run), \
                mock.patch.object(send, "MAX_HTTP_RESPONSE_BYTES", 8):
            with self.assertRaises(send.SendError) as ctx:
                send._http_curl("GET", "https://portal.example/designs/one")
        self.assertIn("response exceeds", str(ctx.exception))


class TestCredentialOriginPinning(SendHome):
    def test_refresh_refuses_unpinned_origin_before_http(self):
        self.green()
        calls = []
        send._http = lambda *args, **kwargs: calls.append((args, kwargs))
        for api_base in (
            "http://evil.example/api/v1",
            "https://evil.example/api/v1",
        ):
            os.environ["BOB_SHOP_API"] = api_base
            with self.assertRaises(send.SendError) as ctx:
                send.refresh_auth()
    ...
    def test_bearer_call_refuses_unpinned_origin_before_http(self):
        self.green()
        os.environ["BOB_SHOP_API"] = "https://evil.example/api/v1"
        calls = []
        send._http = lambda *args, **kwargs: calls.append((args, kwargs))
        with self.assertRaises(send.SendError) as ctx:
            send._authed_call(
    ...
    def test_explicitly_pinned_staging_origin_can_refresh(self):
        self.green()
        os.environ["BOB_SHOP_API"] = "https://staging.example/api/v1"
        os.environ["BOB_SHOP_ALLOWED_ORIGINS"] = "https://staging.example"
        calls = []
...
class TestVocabularyCompatibility(SendHome):
    def test_publish_module_is_the_exact_send_module(self):
        from harness import publish

        self.assertIs(publish, send)

    def test_legacy_publish_dry_run_is_a_conflict_checked_fallback(self):
        os.environ["BOB_PUBLISH_DRY_RUN"] = "0"
        self.assertFalse(send._dry_run())
        os.environ["BOB_SEND_DRY_RUN"] = "1"
        with self.assertRaises(send.SendError) as ctx:
            send._dry_run()
        self.assertIn("disagree", str(ctx.exception))

    def test_legacy_portal_settings_are_explicit_fallbacks(self):
        os.environ["BOB_PORTAL_API"] = "https://legacy.example/api/v1/"
        os.environ["BOB_PORTAL_ALLOWED_ORIGINS"] = "https://legacy.example/"
        self.assertEqual(
            send._api_base(), "https://legacy.example/api/v1"
        )
        self.assertEqual(
            send._shop_allowed_origins(), ("https://legacy.example",)
        )

    def test_legacy_panda_settings_are_explicit_fallbacks(self):
        os.environ["BOB_PANDA_API"] = "https://legacy.example/api/v1/"
        os.environ["BOB_PANDA_ALLOWED_ORIGINS"] = "https://legacy.example/"
        self.assertEqual(send._api_base(), "https://legacy.example/api/v1")
        self.assertEqual(
            send._shop_allowed_origins(), ("https://legacy.example",)
        )

    def test_new_and_legacy_portal_api_must_agree(self):
        os.environ["BOB_SHOP_API"] = "https://new.example/api/v1"
        os.environ["BOB_PORTAL_API"] = "https://old.example/api/v1"
        with self.assertRaises(send.SendError) as ctx:
            send._api_base()
        self.assertIn("disagree", str(ctx.exception))

    def test_new_and_legacy_allowed_origins_must_agree(self):
        os.environ["BOB_SHOP_ALLOWED_ORIGINS"] = "https://new.example"
        os.environ["BOB_PORTAL_ALLOWED_ORIGINS"] = "https://old.example"
        with self.assertRaises(send.SendError) as ctx:
            send._shop_allowed_origins()
        self.assertIn("disagree", str(ctx.exception))

    def test_canonical_and_legacy_owner_settings_must_agree(self):
        auth = self.make_auth()
        os.environ["BOB_SHOP_OWNER_ID"] = auth["shop_owner_id"]
        os.environ["PORTAL_OWNER_ID"] = "someone-else"
        with self.assertRaises(send.SendError) as ctx:
            send._shop_owner(auth)
        self.assertIn("disagree", str(ctx.exception))

    def test_canonical_bob_shop_owner_setting_is_primary(self):
        auth = self.make_auth()
        owner = auth["shop_owner_id"]
        os.environ["BOB_SHOP_OWNER_ID"] = owner
        self.assertEqual(send._shop_owner(auth), owner)

    def test_owner_settings_read_both_legacy_fallbacks(self):
        auth = self.make_auth()
        owner = auth["shop_owner_id"]
        os.environ["PORTAL_OWNER_ID"] = owner
        self.assertEqual(send._shop_owner(auth), owner)
        os.environ.pop("PORTAL_OWNER_ID")
        os.environ["PANDA_OWNER_ID"] = owner
        self.assertEqual(send._shop_owner(auth), owner)

    def test_two_legacy_owner_settings_must_also_agree(self):
        auth = self.make_auth()
        os.environ["PORTAL_OWNER_ID"] = auth["shop_owner_id"]
        os.environ["PANDA_OWNER_ID"] = "someone-else"
        with self.assertRaises(send.SendError) as ctx:
            send._shop_owner(auth)
        self.assertIn("disagree", str(ctx.exception))

    def test_new_and_legacy_owner_fields_must_agree(self):
        auth = self.make_auth()
        auth["bob_user_id"] = "someone-else"
        with self.assertRaises(send.SendError) as ctx:
            send._shop_owner(auth)
        self.assertIn("disagree", str(ctx.exception))

    def test_workshop_request_fields_read_legacy_but_refuse_disagreement(self):
        self.assertEqual(
            send._workshop_request_value(
                {"_core_owner_id": "bob-owner"}, "owner_id"
            ),
            "bob-owner",
        )
        self.assertEqual(
            send._workshop_request_value(
                {"_foundation_owner_id": "bob-owner"}, "owner_id"
            ),
            "bob-owner",
        )
        with self.assertRaises(send.SendError) as ctx:
            send._workshop_request_value({
                "_workshop_owner_id": "bob-owner",
                "_core_owner_id": "someone-else",
            }, "owner_id")
        self.assertIn("disagree", str(ctx.exception))

    def test_legacy_auth_file_is_refreshed_in_place(self):
        self.make_auth(legacy=True)
        ...
        send.refresh_auth()
        legacy = os.path.join(
            self.home, "state", send.LEGACY_PANDA_AUTH_FILE
        )
        current = os.path.join(self.home, "state", send.SHOP_AUTH_FILE)
        self.assertEqual(send._auth_path(), legacy)
        self.assertTrue(os.path.exists(legacy))
        self.assertFalse(os.path.exists(current))

    def test_legacy_portal_auth_file_is_refreshed_in_place(self):
        auth = {
            "access_token": "acc-1",
            "refresh_token": "ref-1",
            "user": {"id": "b0b" * 8, "username": "bob"},
            "portal_owner_id": "b0b" * 8,
        }
        state = os.path.join(self.home, "state")
        os.makedirs(state, exist_ok=True)
        legacy = os.path.join(state, send.LEGACY_PORTAL_AUTH_FILE)
        with open(legacy, "w") as fh:
            json.dump(auth, fh)

        send._http = lambda *args, **kwargs: (
            200,
            {},
            json.dumps({
                "access_token": "fresh",
        ...
        send.refresh_auth()
        self.assertEqual(send._auth_path(), legacy)
        self.assertFalse(os.path.exists(os.path.join(state, send.SHOP_AUTH_FILE)))

    def test_two_auth_files_are_refused(self):
        self.make_auth()
        self.make_auth(legacy=True)
        with self.assertRaises(send.SendError) as ctx:
            send._load_auth()
    ...
    def test_legacy_workshop_store_is_continued_but_two_are_refused(self):
        state = os.path.join(self.home, "state")
        os.makedirs(state, exist_ok=True)
        legacy = os.path.join(state, send.LEGACY_CORE_STATE_FILE)
        current = os.path.join(state, send.WORKSHOP_STATE_FILE)
        with open(legacy, "wb"):
            pass
        self.assertEqual(send._workshop_store_path(), legacy)
        with open(current, "wb"):
            pass
        with self.assertRaises(send.SendError) as ctx:
            send._workshop_store_path()
        self.assertIn("multiple paths", str(ctx.exception))

    def test_legacy_foundation_store_is_continued(self):
        state = os.path.join(self.home, "state")
        os.makedirs(state, exist_ok=True)
        legacy = os.path.join(state, send.LEGACY_FOUNDATION_STATE_FILE)
        with open(legacy, "wb"):
            pass
        self.assertEqual(send._workshop_store_path(), legacy)

    def test_legacy_projection_is_read_and_updated_in_place(self):
        self.make_game()
        legacy = os.path.join(
            self.gdir(), send.LEGACY_PUBLICATION_PROJECTION_FILE
        )
        with open(legacy, "w") as fh:
            json.dump({
                "publication_authority": "workshop",
                "core_intent_id": "intent-1",
                "core_packet_sha256": "a" * 64,
                "draft_receipt": {"status": "draft", "design_id": "d1"},
            }, fh)
        record = send.read_send_projection(SLUG)
        self.assertEqual(record["send_authority"], "workshop")
        self.assertEqual(record["workshop_intent_id"], "intent-1")
        self.assertEqual(record["workshop_pack_sha256"], "a" * 64)
        self.assertEqual(record["draft_stamp"]["design_id"], "d1")
        self.assertNotIn("core_intent_id", record)
        self.assertNotIn("draft_receipt", record)
        record["send_intent"] = {"state": "unknown"}
        send.write_send_projection(SLUG, record)
        self.assertFalse(os.path.exists(os.path.join(
            self.gdir(), send.SEND_PROJECTION_FILE
        )))
        with open(legacy) as fh:
            persisted = json.load(fh)
        self.assertEqual(persisted["flip_intent"], {"state": "unknown"})
        self.assertEqual(persisted["send_intent"], {"state": "unknown"})
        self.assertEqual(persisted["draft_receipt"],
                         persisted["draft_stamp"])

    def test_fresh_projection_uses_send_name_and_two_are_refused(self):
        self.make_game()
        send.write_send_projection(SLUG, {"status": "draft"})
        current = os.path.join(self.gdir(), send.SEND_PROJECTION_FILE)
        legacy = os.path.join(
            self.gdir(), send.LEGACY_PUBLICATION_PROJECTION_FILE
        )
        ...
        with self.assertRaises(send.SendError) as ctx:
            send.send_projection_path(SLUG)
        self.assertIn("multiple paths", str(ctx.exception))

    def test_v02_launch_projection_is_read_and_updated_in_place(self):
        self.make_game()
        legacy = os.path.join(
            self.gdir(), send.LEGACY_LAUNCH_PROJECTION_FILE
        )
        with open(legacy, "w") as fh:
            json.dump({"launch_authority": "none", "dry_run": True}, fh)
        record = send.read_send_projection(SLUG)
        self.assertEqual(record["send_authority"], "none")
        send.write_send_projection(SLUG, record)
        self.assertTrue(os.path.exists(legacy))
        self.assertFalse(os.path.exists(os.path.join(
            self.gdir(), send.SEND_PROJECTION_FILE
...
class TestDryRun(SendHome):
    def test_dry_run_writes_manifest_and_never_calls_http(self):
        # BOB_SEND_DRY_RUN unset = default ON; _http is the failing stub.
        self.green()
        self.make_queue("reviewed")
        result = send.send_draft(SLUG)
        self.assertTrue(result["dry_run"])
        manifest_path = os.path.join(self.gdir(), "pack",
                                     "manifest.json")
        self.assertTrue(os.path.exists(manifest_path))
        with open(manifest_path) as fh:
            manifest = json.load(fh)
        self.assertEqual(manifest["fields"]["status"], "draft")
        self.assertIn(send.DISCLOSURE_LINE,
    ...
    def test_dry_run_still_blocks_on_content_walls(self):
        ...
        with self.assertRaises(send.SendError):
            send.send_draft(SLUG)


class TestIdempotency(SendHome):
    def test_existing_workshop_bound_send_is_hard_noop(self):
        self.green()
        first = self.import_workshop_draft()
        send._http = _fail_http
        result = send.send_draft(SLUG)
        self.assertTrue(result["noop"])
        self.assertEqual(result["send"]["design"]["id"],
                         first["design"]["id"])

    def test_projection_fields_without_durable_stamp_are_not_authority(self):
        self.green()
        with open(os.path.join(self.gdir(), "send.json"), "w") as fh:
            json.dump({
                "slug": SLUG,
                "design": {"id": "d1"},
                "workshop_intent_id": "intent-1",
                "workshop_product_id": "product-1",
                "workshop_artifact_sha256": "a" * 64,
                "workshop_pack_sha256": "b" * 64,
            }, fh)
        os.environ["BOB_SEND_DRY_RUN"] = "0"
        with self.assertRaises(send.SendError) as ctx:
            send.send_draft(SLUG)  # _http would raise if touched
        self.assertIn("durable intent and validated Stamp", str(ctx.exception))
    ...
    def test_current_dry_run_keeps_existing_rehearsal_as_noop(self):
        ...
        result = send.send_draft(SLUG)
        self.assertTrue(result["noop"])
        self.assertEqual(result["send"]["send_authority"], "none")
    ...
    def test_legacy_remote_projection_is_stranded_not_adopted(self):
        ...
        self.assertIn("legacy pre-Workshop", message)
        self.assertIn("Shop Door design", message)
...
class TestLiveImport(SendHome):
    ...
    def test_import_persists_ledger_and_refreshes_auth_0600(self):
        ...
        record = send.send_draft(SLUG)
        self.assertEqual(record["design"]["id"], "d9")
        self.assertEqual(record["status"], "draft")
        self.assertEqual(record["send_authority"], "workshop")
        self.assertEqual(record["via"], "workshop-sender")
        self.assertTrue(record["workshop_intent_id"])
        self.assertEqual(len(record["workshop_pack_sha256"]), 64)
        self.assertNotIn("workshop_packet_sha256", record)
        self.assertEqual(record["draft_stamp"]["artifact_sha256"],
                         record["workshop_artifact_sha256"])
        self.assertTrue(os.path.exists(os.path.join(
            self.home, "state", send.WORKSHOP_STATE_FILE)))
        runtime = send._require_workshop()
        store = runtime.Clockwork(Path(send._workshop_store_path()))
        intent = store.get_publish_intent(record["workshop_intent_id"])
        self.assertEqual(intent["state"], "succeeded")
        self.assertEqual(intent["receipt"], record["draft_stamp"])
        # rotated pair persisted, 0600
        auth_path = os.path.join(self.home, "state", send.SHOP_AUTH_FILE)
        ...
        self.assertTrue(os.path.exists(
            os.path.join(self.gdir(), "send.json")))
        rows = ledger.rows(slug=SLUG)
        self.assertEqual(rows[-1]["kind"], "send")
        self.assertEqual(queue.load()["games"][SLUG]["state"], "published")

    def test_queue_does_not_advance_when_durable_stamp_validation_fails(self):
        os.environ["BOB_SEND_DRY_RUN"] = "0"
        self.green()
        self.make_queue("reviewed")
        self.http_mock([
        ...
        with mock.patch.object(
            send,
            "_validated_draft_stamp",
            side_effect=send.SendError("Stamp mismatch (mock)"),
        ):
            with self.assertRaises(send.SendError):
                send.send_draft(SLUG)
        self.assertEqual(queue.load()["games"][SLUG]["state"], "reviewed")
    ...
    def test_import_rebuilds_a_stale_packet_at_the_effect_boundary(self):
        os.environ["BOB_SEND_DRY_RUN"] = "0"
        self.green()
        stale_path = send.pack_game(SLUG)
        stale = send._workshop_pack_identity(stale_path)
        ...
        record = send.send_draft(SLUG)
        self.assertNotEqual(record["workshop_artifact_sha256"],
...
class TestFlip(SendHome):
    ...
    def test_refuses_outside_price_corner(self):
        ...
        for price in (3999, 8001, 99, 2000000):
            with self.assertRaises(send.SendError):
                send.flip_public(SLUG, price)

    def test_override_allows_outside_corner_but_not_api_bounds(self):
        self.setUpFlip()
        os.environ["BOB_PRICE_OVERRIDE"] = "1"
        with self.assertRaises(send.SendError):
            send.flip_public(SLUG, 99)  # API bound holds even overridden
        send._http = self.successful_flip_http()
        record = send.flip_public(SLUG, 9000)
        self.assertEqual(record["price_cents"], 9000)

    def test_refuses_without_send_projection(self):
        os.environ["BOB_SEND_DRY_RUN"] = "0"
        self.green()
        with self.assertRaises(send.SendError):
            send.flip_public(SLUG, 5000)
    ...
    def test_refuses_to_flip_if_product_bytes_changed_after_import(self):
        ...
        send._http = _fail_http
        with self.assertRaises(send.SendError) as ctx:
            send.flip_public(SLUG, 5500)
        self.assertIn("Workshop-bound artifact", str(ctx.exception))

    def test_projection_hash_edit_cannot_bypass_artifact_drift(self):
        self.setUpFlip()
        with open(os.path.join(self.gdir(), "RULES.md"), "a") as fh:
            fh.write("\nchanged after import\n")
        new_identity = send._workshop_pack_identity(send.pack_game(SLUG))
        pub_path = os.path.join(self.gdir(), "send.json")
        with open(pub_path) as fh:
            record = json.load(fh)
        record["workshop_artifact_sha256"] = new_identity["artifact_sha256"]
    ...
    def test_projection_cannot_select_another_workshop_intent(self):
        self.setUpFlip()
        pub_path = os.path.join(self.gdir(), "send.json")
        with open(pub_path) as fh:
            record = json.load(fh)
        record["workshop_intent_id"] = "forged-other-product-intent"
        with open(pub_path, "w") as fh:
            json.dump(record, fh)
        send._http = _fail_http
        with self.assertRaises(send.SendError) as ctx:
    ...
    def test_happy_flip_updates_record_ledger_queue(self):
        ...
        send._http = self.successful_flip_http(seen)
        record = send.flip_public(SLUG, 5500)
        self.assertEqual(record["status"], "public")
        self.assertEqual(record["price_cents"], 5500)
        self.assertIn("send_stamp", record)
        self.assertEqual(record["send_stamp"]["current_history_id"],
                         "h9")
        ...
        self.assertEqual(ledger.rows(slug=SLUG)[-1]["stage"], "make_public")
    ...
    def test_ambiguous_readback_does_not_mark_live_or_repost(self):
        ...
        with open(os.path.join(self.gdir(), "send.json")) as fh:
            record = json.load(fh)
        self.assertEqual(record["status"], "draft")
        self.assertEqual(record["send_intent"]["state"], "unknown")
        with self.assertRaises(send.SendError):
            send.flip_public(SLUG, 5500)
    ...
    def test_reconcile_public_uses_get_only(self):
        ...
        def ambiguous(method, url, headers=None, data=None, timeout=None):
            if method == "POST":
                return 200, {}, b"{}"
            return 503, {}, b'{"error":"temporarily unavailable"}'
        send._http = ambiguous
        with self.assertRaises(send.SendError):
            send.flip_public(SLUG, 5500)
        seen = []
        send._http = self.successful_flip_http(seen)
        result = send.reconcile_public(SLUG)
    ...
    def test_reconcile_refuses_changed_current_artifact_even_if_projection_edits(self):
        ...
        send._http = _fail_http
        with self.assertRaises(send.SendError) as ctx:
            send.reconcile_public(SLUG)
        self.assertIn("Workshop-bound artifact", str(ctx.exception))


class TestCurateAndUnpublish(SendHome):
    def setUpDraft(self):
        os.environ["BOB_SEND_DRY_RUN"] = "0"
        self.green()
        with open(os.path.join(self.gdir(), "send.json"), "w") as fh:
    ...
    def test_unpublish_flips_record_back_to_draft(self):
        self.setUpDraft()
        send._http = lambda m, u, headers=None, data=None, timeout=None: \
            (200, {}, b"{}")
        record = send.unpublish(SLUG)
        self.assertEqual(record["status"], "draft")
