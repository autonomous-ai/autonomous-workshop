...
1. **Idempotency lives here, not in the API.** "uploading the same zip twice
   ...
   games/<slug>/send.json remains Bob's human-readable projection. Existing
...
def pack_game(slug):
    """Assemble games/<slug>/pack/<slug>.zip through Workshop.
    ...
    gdir = _game_dir(slug)
    if not os.path.isdir(gdir):
        raise SendError("games/%s does not exist — nothing to zip" % slug)
