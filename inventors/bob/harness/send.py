"""Send Bob's inspected game through Workshop and an optional Shop Door.
...
1. **Idempotency lives here, not in the API.** "uploading the same zip twice
   creates two designs" — Workshop's SQLite outbox records the pack-bound intent
   before HTTP and blocks retries after any ambiguous result. The historical
   toys/<slug>/send.json remains Bob's human-readable projection. Existing
   send.json and published.json files are compatibility authorities only.
...
3. **Dry-run is the default** (BOB_SEND_DRY_RUN=1): the import API has no
...
from harness import workshop_runtime, ledger, queue, telegram
...
STRIP_DIRS = frozenset([
    ...
    "pack",              # our own output — packing it would recurse
    "launch_payload",    # v0.2 compatibility output; never pack it
    "publish_payload",   # legacy send output; continue, but never ship it
...
STRIP_FILES = frozenset([
    ...
    "send.json",         # Bob's send projection is about the design,
    "launch.json",       # v0.2 compatibility projection,
    "published.json",    # legacy projection name, also never product content
...
SHOP_AUTH_FILE = "shop-auth.json"  # state/shop-auth.json, chmod 600
LEGACY_PORTAL_AUTH_FILE = "portal-auth.json"
LEGACY_PANDA_AUTH_FILE = "panda-auth.json"
WORKSHOP_STATE_FILE = "inventor-workshop.sqlite3"
LEGACY_FOUNDATION_STATE_FILE = "inventor-foundation.sqlite3"
LEGACY_CORE_STATE_FILE = "inventor-core.sqlite3"
SEND_PROJECTION_FILE = "send.json"
LEGACY_LAUNCH_PROJECTION_FILE = "launch.json"
LEGACY_PUBLICATION_PROJECTION_FILE = "published.json"
PACK_DIR = "pack"
LEGACY_LAUNCH_PAYLOAD_DIR = "launch_payload"
LEGACY_PUBLISH_PAYLOAD_DIR = "publish_payload"
DEFAULT_SHOP_API = "https://panda-social-api.autonomous.ai/api/v1"
DEFAULT_SHOP_ORIGIN = "https://panda-social-api.autonomous.ai"
...
class SendError(Exception):
    """Raised when sending cannot proceed. The message always says what
    to do next (CONTRACTS §6) — a bare traceback teaches the 3am log reader
    nothing."""


# v0.2 source compatibility. New Bob code raises and documents ``SendError``.
...
def _api_base():
    # Overridable for the Layer-2 live rehearsal against a staging Shop Door.
    return _compatible_settings(
        "BOB_SHOP_API",
        ("BOB_PORTAL_API", "BOB_PANDA_API"),
        DEFAULT_SHOP_API,
...
def _compatible_settings(primary, legacy_names, default, normalize):
    """Read renamed settings without accepting multiple configuration truths."""

    names = (primary,) + tuple(legacy_names)
    configured = [(name, os.environ[name]) for name in names if name in os.environ]
    normalized = [(name, normalize(value)) for name, value in configured]
    if normalized and any(value != normalized[0][1] for _, value in normalized[1:]):
        raise SendError(
            "%s disagree" % " and ".join(
                [primary] + ["legacy %s" % name for name in legacy_names]
            )
        )
    selected = configured[0][1] if configured else default
    if selected is None:
        selected = default
    return normalize(selected)


def _compatible_setting(primary, legacy, default, normalize):
    """Pair-shaped compatibility wrapper used by the API/origin settings."""

    return _compatible_settings(primary, (legacy,), default, normalize)


def _select_state_file(current, legacy, label):
    """Use one legacy authority in place, refusing split state."""

    candidates = [current] + list(legacy)
    existing = [path for path in candidates if os.path.exists(path)]
    authorities = []
    for path in existing:
        if not any(
            _same_file(path, authority) for authority in authorities
        ):
            authorities.append(path)
    if len(authorities) > 1:
        raise SendError(
            "%s exists at multiple paths (%s); resolve the duplicate before send"
            % (label, ", ".join(authorities))
        )
...
def send_projection_path(slug):
    root = _game_dir(slug)
    return _select_state_file(
        os.path.join(root, SEND_PROJECTION_FILE),
        (
            os.path.join(root, LEGACY_LAUNCH_PROJECTION_FILE),
            os.path.join(root, LEGACY_PUBLICATION_PROJECTION_FILE),
        ),
        "send projection",
    )
...
def _pack_dir(slug):
    root = _game_dir(slug)
    return _select_state_file(
        os.path.join(root, PACK_DIR),
        (
            os.path.join(root, LEGACY_LAUNCH_PAYLOAD_DIR),
            os.path.join(root, LEGACY_PUBLISH_PAYLOAD_DIR),
        ),
        "packed artifact directory",
    )


def _auth_path():
    state = os.path.join(_home(), "state")
    return _select_state_file(
        os.path.join(state, SHOP_AUTH_FILE),
        (
            os.path.join(state, LEGACY_PORTAL_AUTH_FILE),
            os.path.join(state, LEGACY_PANDA_AUTH_FILE),
        ),
        "Shop Door credentials",
    )


def _workshop_store_path():
    state = os.path.join(_home(), "state")
    return _select_state_file(
        os.path.join(state, WORKSHOP_STATE_FILE),
        (
            os.path.join(state, LEGACY_FOUNDATION_STATE_FILE),
            os.path.join(state, LEGACY_CORE_STATE_FILE),
        ),
        "Workshop Clockwork state",
    )
...
_SEND_FIELD_ALIASES = {
    "send_authority": ("launch_authority", "publication_authority"),
    "workshop_intent_id": ("foundation_intent_id", "core_intent_id"),
    "workshop_product_id": ("foundation_product_id", "core_product_id"),
    "workshop_artifact_sha256": (
        "foundation_artifact_sha256", "core_artifact_sha256"
    ),
    "workshop_pack_sha256": (
        "workshop_packet_sha256", "foundation_packet_sha256",
        "core_packet_sha256", "zip_sha256"
    ),
    "workshop_contract": ("foundation_contract", "core_contract"),
    "workshop_store": ("foundation_store", "core_store"),
    "draft_stamp": ("draft_launch_receipt", "draft_receipt"),
    "send_stamp": ("launch_receipt", "publication_receipt"),
    "send_intent": ("launch_intent", "flip_intent"),
}


def _normalize_send_projection(record):
    if not isinstance(record, dict):
        raise SendError("send projection must be a JSON object")
    normalized = dict(record)
    for current, aliases in _SEND_FIELD_ALIASES.items():
        values = []
        for name in (current,) + aliases:
            if name in record and record[name] is not None:
                values.append((name, record[name]))
        if values and any(value != values[0][1] for _, value in values[1:]):
            raise SendError(
                "send projection fields %s disagree"
...
def read_send_projection(slug):
    return _normalize_send_projection(_read_json(send_projection_path(slug)))


def write_send_projection(slug, record):
    path = send_projection_path(slug)
    normalized = _normalize_send_projection(record)
    if os.path.basename(path) != SEND_PROJECTION_FILE:
        # Keep old readers working while this existing file remains authoritative.
        for current, aliases in _SEND_FIELD_ALIASES.items():
            if current in normalized:
                for alias in aliases:
                    normalized[alias] = normalized[current]
    else:
        for aliases in _SEND_FIELD_ALIASES.values():
            for alias in aliases:
                normalized.pop(alias, None)
...
# Direct compatibility aliases: one implementation and one state authority.
launch_projection_path = send_projection_path
read_launch_projection = read_send_projection
write_launch_projection = write_send_projection
_launch_payload_dir = _pack_dir


def _require_workshop():
    try:
        return workshop_runtime.require_workshop()
    except workshop_runtime.WorkshopUnavailable as exc:
        raise SendError(
            "Inventor Workshop is unavailable — send cannot safely "
            "continue: %s" % exc
        ) from exc


def _workshop_pack_identity(zip_path):
    runtime = _require_workshop()
    try:
        identity = runtime.inspect_pack(Path(zip_path))
    except Exception as exc:
        raise SendError(
            "Workshop rejected packed artifact %s: %s" % (zip_path, exc)
        ) from exc
    if isinstance(identity, dict):
        details = dict(identity)
        values = [
            value for value in (
                details.get("pack_sha256"), details.get("packet_sha256")
            ) if value is not None
        ]
        if values and any(value != values[0] for value in values[1:]):
            raise SendError("Workshop pack identity SHA-256 fields disagree")
        if not values:
            raise SendError("Workshop pack identity has no pack SHA-256")
        details["pack_sha256"] = values[0]
        details.pop("packet_sha256", None)
        return details
    return {
...
def _shop_allowed_origins():
    origins = _compatible_settings(
        "BOB_SHOP_ALLOWED_ORIGINS",
        ("BOB_PORTAL_ALLOWED_ORIGINS", "BOB_PANDA_ALLOWED_ORIGINS"),
        DEFAULT_SHOP_ORIGIN,
        lambda value: tuple(
            sorted(
                set(
    ...
    if not origins:
        raise SendError(
            "BOB_SHOP_ALLOWED_ORIGINS must pin at least one HTTPS origin"
        )
    return origins


def _shop_owner(auth):
    configured = _compatible_settings(
        "BOB_SHOP_OWNER_ID",
        ("BOB_PORTAL_OWNER_ID", "PORTAL_OWNER_ID", "PANDA_OWNER_ID"),
        "",
        lambda value: value.strip(),
    )
    pins = [
        ("shop_owner_id", auth.get("shop_owner_id")),
        ("portal_owner_id", auth.get("portal_owner_id")),
        ("bob_user_id", auth.get("bob_user_id")),
    ]
    pins = [(name, value) for name, value in pins if value]
    if pins and any(value != pins[0][1] for _, value in pins[1:]):
        raise SendError(
            "Shop Door credential owner fields %s disagree"
            % ", ".join(name for name, _ in pins)
        )
    auth_pin = pins[0][1] if pins else ""
    if configured and auth_pin and configured != auth_pin:
        raise SendError(
            "BOB_SHOP_OWNER_ID disagrees with the owner pinned in Shop Door "
            "credentials"
        )
    pinned = configured or auth_pin
    actual = (auth.get("user") or {}).get("id")
    if not pinned or not actual or pinned != actual:
        raise SendError(
            "send requires Bob's non-empty Shop Door owner id to match the "
            "authenticated user; set BOB_SHOP_OWNER_ID (and "
            "BOB_FACTORY_USER_ID for an operator bearer) or repair state/%s"
            % SHOP_AUTH_FILE
        )
    return pinned


def _workshop_transport(runtime):
...
def _validated_shop_api_base():
    """Return the API base only after Workshop has pinned its HTTPS origin.
    ...
    runtime = _require_workshop()
    try:
        shop = runtime.ShopDoor(
            "origin-validation-only",
            api_base=_api_base(),
            transport=_workshop_transport(runtime),
            allowed_origins=_shop_allowed_origins(),
        )
    except Exception as exc:
        raise SendError(
            "Shop Door API origin is not an allowed HTTPS endpoint: %s" % exc
        ) from exc
    return shop.api_base


def _validated_shop_url(url):
    """Require one credential-bearing URL to stay below the pinned API base."""
    api_base = _validated_shop_api_base()
    try:
        expected = urllib.parse.urlsplit(api_base)
        candidate = urllib.parse.urlsplit(url)
    except (TypeError, ValueError) as exc:
        raise SendError("Shop Door request URL is malformed") from exc
    ...
    if (
        ...
        raise SendError(
            "credential-bearing Shop Door request escaped the pinned API base"
        )
    return urllib.parse.urlunsplit(candidate)


def _send_context(auth):
    runtime = _require_workshop()
    owner = _shop_owner(auth)
    token = auth.get("access_token")
    if not isinstance(token, str) or not token:
        raise SendError("Bob's authenticated access token is empty")
    try:
        store = runtime.Clockwork(Path(_workshop_store_path()))
        shop = runtime.ShopDoor(
            token,
            api_base=_api_base(),
            transport=_workshop_transport(runtime),
        ...
        sender = runtime.Sender(store, shop, owner)
    except Exception as exc:
        raise SendError("could not initialize Workshop Sender: %s" % exc) from exc
    return runtime, store, sender, owner


# v0.2 internal-name compatibility for operator scripts. All resolve to the
# same Shop Door/Sender implementation; none create a second authority.
_portal_allowed_origins = _shop_allowed_origins
_portal_owner = _shop_owner
_validated_portal_api_base = _validated_shop_api_base
_validated_portal_url = _validated_shop_url
_launch_context = _send_context


def _workshop_product_id(slug):
    if not isinstance(slug, str) or not slug:
        raise SendError("Bob send slug must be a non-empty string")
    return "bob:%s" % hashlib.sha256(slug.encode("utf-8")).hexdigest()


def _workshop_product(store, runtime, slug, artifact_sha):
    ...
    product_id = _workshop_product_id(slug)
    try:
    ...
    if product.get("artifact_sha256") != artifact_sha:
        raise SendError(
            "Workshop product for '%s' is already bound to different artifact "
            "bytes; corrected bytes require a new slug because an earlier "
            "non-idempotent import may have crossed the Shop Door" % slug
        )
    return product_id


def _workshop_request_value(request, field):
    """Read a renamed durable request field without accepting split truth."""

    names = (
        "_workshop_%s" % field,
        "_foundation_%s" % field,
        "_core_%s" % field,
    )
    values = [
    ...
    if values and any(value != values[0][1] for _, value in values[1:]):
        raise SendError(
            "Workshop send request fields %s disagree"
            % ", ".join(name for name, _ in values)
        )
    return values[0][1] if values else None


def _bound_workshop_intent(store, slug, identity, owner=None, api_origin=None):
    """Resolve send truth from Workshop, never from the JSON projection.
    ...
    intent are keyed from Bob's logical slug and must identify the exact pack
    ...
    try:
        ...
        raise SendError(
            "Workshop has no send product for '%s'; send its draft first"
            % slug
        ) from exc
    if product.get("artifact_sha256") != identity.get("artifact_sha256"):
        raise SendError(
            "current game files do not match the exact Workshop-bound artifact "
            "%s" % product_id
        )
    try:
        ...
        raise SendError(
            "Workshop lacks the public latest_publish_intent contract"
        ) from exc
    if intent is None:
        raise SendError(
            "Workshop has no send intent for '%s'; send its draft first"
            % slug
        )
    request = intent.get("request")
    if not isinstance(request, dict):
        raise SendError("Workshop send intent has no persisted request")
    if (
        intent.get("product_id") != product_id
        or intent.get("packet_sha256") != identity.get("pack_sha256")
        or _workshop_request_value(request, "artifact_sha256")
        != identity.get("artifact_sha256")
    ):
        raise SendError(
            "current Workshop pack does not match the slug-bound intent; "
            "refusing artifact drift"
        )
    if owner is not None and _workshop_request_value(request, "owner_id") != owner:
        raise SendError(
            "Workshop send intent belongs to a different Shop Door owner"
        )
    if (
        api_origin is not None
        and _workshop_request_value(request, "api_origin") != api_origin
    ):
        raise SendError(
            "Workshop send intent belongs to a different Shop Door API origin"
        )
    return product_id, intent


def _validated_draft_stamp(runtime, store, sender, outcome, product_id,
                           identity, owner):
    """Prove Sender durably recorded the exact typed Shop Door Stamp.

    Bob does this readback before writing send.json or advancing the queue.
    It is intentionally redundant with Sender's internal checks: the queue
    boundary must never depend on an adapter return value or remote stdout
    alone, which is exactly what made the historical box route unsafe.
    """
    try:
        stamp = outcome.stamp
        stamp.assert_owner(owner)
        stamp.assert_artifact(identity["artifact_sha256"])
        stamp_document = stamp.to_dict()
        intent = store.get_publish_intent(outcome.intent_id)
        request = intent.get("request") or {}
        checks = (
            (intent.get("state") == "succeeded", "intent is not succeeded"),
            (intent.get("product_id") == product_id, "product id drifted"),
            (intent.get("packet_sha256") == identity["pack_sha256"],
             "packet hash drifted"),
            (_workshop_request_value(request, "artifact_sha256")
             == identity["artifact_sha256"], "artifact hash drifted"),
            (_workshop_request_value(request, "owner_id") == owner,
             "owner id drifted"),
            (_workshop_request_value(request, "api_origin")
             == sender.client.api_origin, "Shop Door origin drifted"),
            (intent.get("receipt") == stamp_document,
             "durable Stamp differs from Sender outcome"),
            (stamp.pack_sha256 == identity["pack_sha256"],
             "Stamp packet hash drifted"),
            (stamp.status == "draft", "Stamp is not a draft"),
        )
        failed = [message for passed, message in checks if not passed]
        if failed:
            raise SendError(
                "Workshop draft authority validation failed: %s"
                % "; ".join(failed)
            )
        # Reconstructing the typed Stamp proves the persisted document itself
        # (not only the in-memory object) still satisfies Workshop's schema.
        runtime.Stamp(**intent["receipt"])
        return intent, stamp_document
    except SendError:
        raise
    except Exception as exc:
        raise SendError(
            "Workshop did not persist a valid draft Stamp: %s" % exc
        ) from exc


def _send_metadata(slug, listing):
    metadata = {
    ...
    try:
        ...
        with opener.open(req, timeout=timeout) as resp:
            ...
            if len(body) > MAX_HTTP_RESPONSE_BYTES:
                raise SendError("Shop Door response exceeds the 2 MB safety limit")
        ...
        try:
            ...
            if len(body) > MAX_HTTP_RESPONSE_BYTES:
                raise SendError(
                    "Shop Door error response exceeds the 2 MB safety limit"
                )
...
def _load_auth():
    """state/shop-auth.json (or a compatibility credential file):
    {access_token, refresh_token, user: {id, ...}, shop_owner_id}

    shop_owner_id is the PIN — written once by the human who minted the
    ...
    if env_token:
        return {
            ...
            "shop_owner_id": os.environ.get("BOB_FACTORY_USER_ID", ""),
...
def refresh_auth():
    ...
    if not auth or not auth.get("refresh_token"):
        raise SendError(
            ...
            "alone." % _auth_path())
    _shop_owner(auth)  # refuse split owner configuration before credentials move
    api_base = _validated_shop_api_base()
    ...
    if status != 200:
        raise SendError(
            ...
            "dead, sending halts; raise a human task to re-mint the bob "
    ...
    try:
        ...
        raise SendError(
            "Shop Door refresh rotated and persisted the token pair, but the "
            "returned user no longer matches Bob's pinned owner; send is "
            "refused: %s" % exc
        ) from exc
    return auth


def _fresh_send_auth():
    """Refresh a minted account before send; keep session bearers in memory."""
    auth = _load_auth()
    if not auth:
        raise SendError("no auth on disk — mint the bob token first")
    if auth.get("source") == "env:BOB_FACTORY_TOKEN":
        return auth
    return refresh_auth()


_fresh_launch_auth = _fresh_send_auth
...
def validate(slug):
    ...
    Rebuilds the Workshop pack every run: validation measures the current
    game tree, never stale compatibility output left by an earlier tick.
    """
    problems = []
    gdir = _game_dir(slug)
    if not os.path.isdir(gdir):
        return ["game dir: toys/%s does not exist — nothing to validate"
                % slug]
    ...
    # -- the canonical Workshop pack --------------------------------------
    zip_path = os.path.join(_pack_dir(slug), "%s.zip" % slug)
    try:
        zip_path = pack_game(slug)
    except (OSError, SendError) as exc:
    ...
    if zip_path and os.path.exists(zip_path):
        ...
        if size > MAX_ZIP_BYTES:
            problems.append(
                ...
                % (size, MAX_ZIP_BYTES))
        try:
            identity = _workshop_pack_identity(zip_path)
        except (OSError, SendError, zipfile.BadZipFile) as exc:
            identity = None
            problems.append(
                "zip: not an exact canonical Workshop pack — rebuild with "
                "pack_game('%s'): %s" % (slug, exc)
            )
    ...
    try:
        ...
        auth = None
        problems.append("auth: %s" % exc)
    if auth is None:
        if not problems or not problems[-1].startswith("auth:"):
            problems.append(
                "auth: state/%s missing or unparseable — a human must mint the "
                "bob token pair first (publish-contract §2)" % SHOP_AUTH_FILE
            )
    else:
        try:
            _shop_owner(auth)
            ...
            problems.append("auth: %s" % exc)
        if auth.get("source") != "env:BOB_FACTORY_TOKEN" and not auth.get(
            "refresh_token"
        ):
...
def pack_game(slug):
    """Assemble toys/<slug>/pack/<slug>.zip through Workshop.

    Bob chooses which product files ship, while Workshop owns the byte
    ...
    exact packet hash. The project lives at the archive root, which the Shop
    Door accepts and which avoids pretending the Workshop manifest is a
    second design folder.
    """
    gdir = _game_dir(slug)
    if not os.path.isdir(gdir):
        raise SendError("toys/%s does not exist — nothing to zip" % slug)
    out_dir = _pack_dir(slug)
    ...
    try:
        workshop_runtime.build_game_packet(
            entries, Path(zip_path), maximum_bytes=MAX_ZIP_BYTES
        )
    except Exception as exc:
        raise SendError(
            "Workshop could not pack '%s': %s"
            % (slug, exc)
        ) from exc
    return zip_path


def workshop_pack_identity(slug):
    """Return Workshop's verified identity for Bob's current pack."""
    path = os.path.join(_pack_dir(slug), "%s.zip" % slug)
    if not os.path.exists(path):
        path = pack_game(slug)
    return _workshop_pack_identity(path)


# v0.2 callable compatibility. Canonical Bob code uses Pack vocabulary above.
build_zip = pack_game


def _legacy_packet_identity(identity):
    legacy = dict(identity)
    legacy["packet_sha256"] = legacy["pack_sha256"]
    return legacy


def workshop_packet_identity(slug):
    return _legacy_packet_identity(workshop_pack_identity(slug))


def _workshop_packet_identity(zip_path):
    return _legacy_packet_identity(_workshop_pack_identity(zip_path))
...
def _is_non_authoritative_dry_projection(record):
    """Recognize Bob's no-effect rehearsal marker, never a remote Stamp.

    Current writers stamp ``send_authority=none`` explicitly.  The
    structural fallback accepts Bob's older dry stubs, which predate that
    field, only when they contain neither a design nor a Workshop intent.
    """
    if not isinstance(record, dict) or record.get("dry_run") is not True:
        return False
    if record.get("send_authority") not in (None, "none"):
        return False
    return not record.get("workshop_intent_id") and not record.get("design")


def _verify_persisted_workshop_authority(slug, record):
    """Verify a projection against Workshop's product, intent, and Stamp.

    Projection fields alone are not authority: they are mutable JSON and the
    historical box path could write arbitrary observations. Existing effects
    are a safe no-op only when the durable outbox and its typed Stamp prove
    the exact product/packet/owner chain.
    """
    if record.get("via") == "text2game-box":
        raise SendError(
            "legacy text2game box observations are not Workshop send authority"
        )
    if record.get("send_authority") not in (None, "workshop"):
        raise SendError(
            "send projection explicitly lacks Workshop authority"
        )
    store_path = _workshop_store_path()
    if not os.path.isfile(store_path):
        raise SendError(
            "the durable Workshop Clockwork store is missing at %s" % store_path
        )
    runtime = _require_workshop()
    try:
        store = runtime.Clockwork(Path(store_path))
        product_id = record["workshop_product_id"]
        intent_id = record["workshop_intent_id"]
        product = store.get_product(product_id)
        intent = store.get_publish_intent(intent_id)
        request = intent.get("request") or {}
        stamp_document = intent.get("receipt")
        if not isinstance(stamp_document, dict):
            raise SendError("durable Workshop intent has no Stamp")
        stamp = runtime.Stamp(**stamp_document)
        owner = _workshop_request_value(request, "owner_id")
        if not owner:
            raise SendError("durable Workshop intent has no owner binding")
        stamp.assert_owner(owner)
        stamp.assert_artifact(record["workshop_artifact_sha256"])
        expected_projection_stamp = (
            record.get("send_stamp")
            if intent.get("state") == "live"
            else record.get("draft_stamp")
        )
        checks = (
            (intent.get("state") in ("succeeded", "live"),
             "intent is not succeeded/live"),
            (intent.get("product_id") == product_id, "product id drifted"),
            (product.get("artifact_sha256")
             == record["workshop_artifact_sha256"],
             "product artifact hash drifted"),
            (intent.get("packet_sha256")
             == record["workshop_pack_sha256"],
             "intent packet hash drifted"),
            (_workshop_request_value(request, "artifact_sha256")
             == record["workshop_artifact_sha256"],
             "intent artifact hash drifted"),
            (stamp.pack_sha256 == record["workshop_pack_sha256"],
             "Stamp packet hash drifted"),
            (expected_projection_stamp == stamp_document,
             "projection Stamp differs from the durable Stamp"),
        )
        failed = [message for passed, message in checks if not passed]
        if failed:
            raise SendError("; ".join(failed))
        projected_store = record.get("workshop_store")
        if projected_store and os.path.normpath(projected_store) != os.path.normpath(
            os.path.relpath(store_path, _home())
        ):
            raise SendError("projection points at a different Workshop store")
    except SendError:
        raise
    except Exception as exc:
...
def _require_workshop_authority_for_existing_effect(slug, record):
    """Reject a remote Stamp that predates Bob's durable Workshop intent.

    A Shop Door design id proves that *an* effect happened, but without the local
    intent and content hashes it cannot prove which bytes were sent or grant
    Bob authority over the remote owner. Such a Stamp must stay stranded;
    deleting it and retrying the same slug could create a duplicate design.
    """
    required = (
        "workshop_intent_id", "workshop_product_id",
        "workshop_artifact_sha256", "workshop_pack_sha256",
    )
    if (
        ...
        try:
            _verify_persisted_workshop_authority(slug, record)
            return
        except SendError as exc:
            raise SendError(
                "send projection for '%s' claims Workshop identity, but "
                "its durable intent and validated Stamp cannot be verified: "
                "%s. Do not treat box output or mutable projection fields as "
                "authority; restore the matching Workshop state or resolve "
                "the remote effect separately before using a new slug."
                % (slug, exc)
            ) from exc
    ...
    projection_name = os.path.basename(send_projection_path(slug))
    source = (
        "legacy pre-Workshop published.json"
        if projection_name == LEGACY_PUBLICATION_PROJECTION_FILE
        else "pre-Workshop send projection"
    )
    raise SendError(
        "%s for '%s' records Shop Door design %s "
        "owned by %s, but has no complete Workshop intent/content identity. "
        ...
        "then re-import the product under a new slug so Workshop records the "
        "first effect. Do not delete this observation and retry the same slug."
        % (source, slug, design_id, owner)
    )


def send_draft(slug):
    """Pack and send the game as a private draft through the Shop Door.

    A fully Workshop-backed send projection is a compatibility no-op.
    A remote pre-Workshop Stamp is stranded with an actionable error because its
    ...
    suppress the first Workshop-backed import. The crash-safe guard is Workshop's
    content-bound SQLite outbox, which records the effect before HTTP and
    permanently blocks an ambiguous retry.

    Dry-run (default): validate + pack_game + write
    pack/manifest.json, advance the queue with note 'dry-run',
    ...
    if os.path.exists(projection_path):
        existing_projection = read_send_projection(slug)
        non_authoritative = _is_non_authoritative_dry_projection(
            existing_projection
        )
        if not non_authoritative:
            _require_workshop_authority_for_existing_effect(
                slug, existing_projection
            )
        if dry or not non_authoritative:
            return {"noop": True, "reason": "the send projection represents an "
                    "existing Workshop-bound effect or the current dry run — a "
                    "second import "
                    "would fork the game into a second design",
                    "send": existing_projection}
    ...
    if dry:
        manifest = {
            ...
            "zip": {"path": os.path.relpath(zip_path, _home()),
                    ...
                    "sha256": pack_sha,
                    "artifact_sha256": identity["artifact_sha256"],
                    "contract": "inventor_workshop.artifacts/v1"},
            "auth_warnings": auth_warnings,
        }
        _write_json(os.path.join(_pack_dir(slug), "manifest.json"), manifest)
        try:
    ...
    auth = _fresh_send_auth()
    runtime, store, sender, owner = _send_context(auth)
    product_id = _workshop_product(
        store, runtime, slug, identity["artifact_sha256"]
    )
    try:
        outcome = workshop_runtime.send_draft(
            sender, product_id, Path(zip_path), _send_metadata(slug, listing)
        )
    except runtime.AmbiguousSendError as exc:
        raise SendError(
            "%s — Workshop recorded the unknown import in %s and blocks every "
            "retry; the Shop Door must expose content/idempotency proof before "
            "this packet can be reconciled" % (exc, _workshop_store_path())
        ) from exc
    except (runtime.WorkshopSendError, runtime.ContractError,
            runtime.StateConflict) as exc:
        raise SendError("Workshop Sender refused the draft import: %s" % exc) from exc

    intent, draft_stamp = _validated_draft_stamp(
        runtime, store, sender, outcome, product_id, identity, owner
    )
    design = intent.get("response") or {
        "id": outcome.stamp.design_id,
        "slug": outcome.stamp.slug,
        "owner_id": outcome.stamp.owner_id,
        "root_id": outcome.stamp.root_id,
        "current_history_id": outcome.stamp.current_history_id,
        "published_history_id": outcome.stamp.published_history_id,
        "status": outcome.stamp.status,
        "project_url": outcome.stamp.project_url,
    }
    ...
    record = {
        ...
        "via": "workshop-sender",
        "send_authority": "workshop",
        "workshop_contract": "inventor_workshop.artifacts/v1",
        "workshop_pack_sha256": pack_sha,
        "workshop_intent_id": outcome.intent_id,
        "workshop_product_id": product_id,
        "workshop_artifact_sha256": identity["artifact_sha256"],
        "workshop_store": os.path.relpath(_workshop_store_path(), _home()),
        "draft_stamp": draft_stamp,
    }
    record = write_send_projection(slug, record)

    ledger.append({"slug": slug, "kind": "send", "stage": "send_draft",
                   "notes": "draft %s imported" % design.get("slug", slug)})
    try:
        queue.advance(slug, "published",
                      "draft imported: %s" % design.get("slug", slug))
    except (KeyError, ValueError) as exc:
        sys.stderr.write("send: queue advance skipped: %s\n" % exc)
    telegram.send(
...
def import_draft(slug):
    """v0.2 compatibility wrapper for :func:`send_draft`."""

    result = send_draft(slug)
    if isinstance(result, dict) and "send" in result and "launch" not in result:
        result = dict(result)
        result["launch"] = result["send"]
    return result
...
def _public_readback(slug, record):
    """Return an authenticated design readback or fail closed.
    ...
    if not isinstance(remote, dict):
        raise SendError("public readback was not a design object")
    ...
    auth = _load_auth() or {}
    expected_owner = _shop_owner(auth)
    ...
    if not str(remote.get("project_url") or "").startswith("https://"):
        raise SendError("public readback lacks an HTTPS project_url")
    return remote
...
def _design_from_stamp(stamp, previous=None):
    """Translate a Workshop Stamp to Bob's long-lived JSON surface."""
    ...
    if stamp.listing_active is not None:
        design["listing"] = {
            "active": stamp.listing_active,
            "price_cents": stamp.listing_price_cents,
            "currency": stamp.listing_currency,
            "sku": stamp.listing_sku,
        }
    return design


_design_from_workshop_receipt = _design_from_stamp  # v0.2 compatibility
...
def flip_public(slug, price_cents):
    ...
    the send projection is missing (never imported), the validator is dirty, or
    ...
    On success: send.json updated, ledger row, queue -> live, and the
    Telegram notice with the link + one-tap UNPUBLISH (the kill switch the
    auto-publish ruling demands).
    """
    if _dry_run():
        raise SendError(
            "BOB_SEND_DRY_RUN is on — the flip is the one irreversible-"
            "facing act; set BOB_SEND_DRY_RUN=0 deliberately to arm it")
    projection_path = send_projection_path(slug)
    if not os.path.exists(projection_path):
        raise SendError(
            "no send projection for '%s' — import_draft must succeed before "
    ...
    try:
        ...
        raise SendError("price_cents must be an integer of USD cents; "
                           "got %r" % (price_cents,))
    ...
    # possibly stale compatibility payload left by an earlier validation pass.
    pack_game(slug)
    current_identity = workshop_pack_identity(slug)
    runtime = _require_workshop()
    local_store = runtime.Clockwork(Path(_workshop_store_path()))
    workshop_product_id, intent = _bound_workshop_intent(
        local_store, slug, current_identity
    )
    workshop_intent_id = intent["id"]
    projected_intent_id = record.get("workshop_intent_id")
    if projected_intent_id and projected_intent_id != workshop_intent_id:
        raise SendError(
            "send.json points at a different intent than the slug-bound "
            "Workshop outbox; refusing the mutable projection"
        )
    auth = _fresh_send_auth()
    runtime, store, sender, owner = _send_context(auth)
    workshop_product_id, persisted_intent = _bound_workshop_intent(
        store,
        slug,
        current_identity,
        owner=owner,
        api_origin=sender.client.api_origin,
    )
    if persisted_intent["id"] != workshop_intent_id:
        raise SendError("slug-bound Workshop intent changed during send setup")
    if persisted_intent.get("state") == "live":
        ...
        if listing_request.get("price_cents") != price:
            raise SendError(
                "Workshop already recorded this intent live at a different price"
            )
    record["send_intent"] = {
        "state": "sending", "started_at": _now_iso(), "price_cents": price,
        "workshop_intent_id": workshop_intent_id,
    }
    record = write_send_projection(slug, record)
    try:
        stamp = workshop_runtime.send_live(sender, workshop_intent_id, price)
    except runtime.AmbiguousSendError as exc:
        record["send_intent"]["state"] = "unknown"
        record["send_intent"]["failed_at"] = _now_iso()
        record = write_send_projection(slug, record)
        raise SendError(
            ...
            runtime.StateConflict) as exc:
        record["send_intent"]["state"] = "rejected"
        record["send_intent"]["failed_at"] = _now_iso()
        record = write_send_projection(slug, record)
        raise SendError("Workshop sender refused the public flip: %s" % exc) from exc

    remote = _design_from_stamp(stamp, record.get("design"))
    record = _record_verified_live(slug, record, remote, price=price)
    record["workshop_intent_id"] = workshop_intent_id
    record["workshop_product_id"] = workshop_product_id
    record["workshop_artifact_sha256"] = current_identity["artifact_sha256"]
    record["workshop_pack_sha256"] = current_identity["pack_sha256"]
    record["send_stamp"] = stamp.to_dict()
    record["send_intent"]["workshop_intent_id"] = workshop_intent_id
    record = write_send_projection(slug, record)
    design = remote

    ledger.append({"slug": slug, "kind": "send", "stage": "make_public",
                   "notes": "verified public at %d cents" % price})

    listing = _load_listing(slug) or {}
    telegram.send(
        "Bob sent '%s' to the Shop Door at $%.2f — %s\n"
...
def reconcile_public(slug):
    """Reconcile a Workshop-recorded ambiguous public flip by authenticated GET.

    This command may rotate auth, but never sends /publish, so it is safe after
    a timeout or crash. A human click without the persisted Workshop price intent
    cannot satisfy the exact-listing Stamp and remains deliberately blocked.
    """
    projection_path = send_projection_path(slug)
    if not os.path.exists(projection_path):
        raise SendError("no send projection for '%s' — nothing to reconcile"
                           % slug)
    record = read_send_projection(slug)
    pack_game(slug)
    current_identity = workshop_pack_identity(slug)
    ...
    if intent["id"] != workshop_intent_id:
        raise SendError("slug-bound Workshop intent changed during reconciliation setup")
    try:
        if intent["state"] == "live":
            stamp = runtime.Stamp(**intent["receipt"])
            stamp.assert_owner(owner)
        else:
            stamp = sender.reconcile_live(workshop_intent_id)
    except runtime.AmbiguousSendError as exc:
        raise SendError(
            "%s — the exact Workshop live intent remains ambiguous" % exc
        ) from exc
    except (runtime.WorkshopSendError, runtime.ContractError,
            runtime.StateConflict, KeyError) as exc:
        raise SendError("Workshop cannot reconcile the public flip: %s" % exc) from exc
    remote = _design_from_stamp(stamp, record.get("design"))
    record = _record_verified_live(
        slug, record, remote, price=stamp.listing_price_cents)
    record["workshop_intent_id"] = workshop_intent_id
    record["workshop_product_id"] = workshop_product_id
    record["workshop_artifact_sha256"] = current_identity["artifact_sha256"]
    record["workshop_pack_sha256"] = current_identity["pack_sha256"]
    record["send_stamp"] = stamp.to_dict()
    record = write_send_projection(slug, record)
    ledger.append({"slug": slug, "kind": "send",
...
def unpublish(slug):
    ...
    Updates the send projection only; the queue's `live` state is terminal by
    ...
    if not os.path.exists(projection_path):
        raise SendError(
            "no send projection for '%s' — it was never imported, so there "
    ...
    record = write_send_projection(slug, record)
    ledger.append({"slug": slug, "kind": "send", "stage": "unpublish",
