...
adapter prefers an installed package, then resolves ``../../foundation/src``
from this file.
...
def _repository_source() -> Path:
    return Path(__file__).resolve().parents[3] / "foundation" / "src"
...
def require_core() -> CoreRuntime:
    ...
    if source is not None:
        ...
        try:
            ...
            if not (source / "inventor_core" / "__init__.py").is_file():
                raise CoreUnavailable(
                    "Foundation is unavailable; expected %s or set BOB_CORE_SRC"
                    % source
                )
            source_text = str(source)
            if source_text not in sys.path:
    ...
    try:
        ...
        raise CoreUnavailable(
            "Foundation could not be imported: %s" % exc
        ) from exc
    if source is not None:
        _require_loaded_core_from(source)
    return CoreRuntime(
...
def build_game_packet(
    ...
    """Build Bob's selected product bytes with Foundation's canonical ZIP contract."""
