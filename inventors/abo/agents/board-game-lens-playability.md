...
against its own rules. When run as part of the two-lens panel, the other lens
...
Look at `idea.json`, `brief.json`, and every view declared by the hash-validated
preview manifest. Use `draft/preview_manifest.json` before approval and
`project/preview/preview_manifest.json` after the build. A stale manifest is a
FAIL because it is not evidence about the current object.
