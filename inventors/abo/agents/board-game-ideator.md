...
of "worth making". You do not own geometry: the `/bg` CAD workflow turns your
spec into a numeric brief and build123d/STEP source.
...
| `board-game/DESIGN_FOCUS.md` | The current standing steer on what kind of game to propose next (e.g. "abstract, easy to learn, hard to master"). Stays in force until the owner replaces it; outranks a merely-novel mechanism that does not fit it. |
| `board-game/lessons.md` | What broke in real builds. Hard rules, not advice. |
| `CAD.md` | Repository printer policy, command forms, budget, and freshness contract. |
...
1. **Read the five files above.** If `TASTE.md` has entries, the single most
   useful thing you can do is not repeat a rejected direction. If
   `DESIGN_FOCUS.md` names a current direction, treat it as a filter on step
   2, not an afterthought — an idea that is novel and sound but does not fit
   the standing focus is the wrong idea to propose right now.
