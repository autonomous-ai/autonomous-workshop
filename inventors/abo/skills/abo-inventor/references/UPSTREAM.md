...
| `research.py` | Invented the game and stated its physical facts as a Python capability. Concept is now a native host-gated stage; the native session does this research itself. |
...
| `concept.py` | ABO's Concept hook (draw, then seal rules with pixels) under the deleted `Workshop(concept=...)` capability wiring. Concept is a shared, host-gated stage every Inventor goes through the same way; there is no per-Inventor Concept job to hook. |
...
| `cad_compat.py` | Adapted six ways the locked CAD skill differed from what `gate.py` expected. `gate.py` is gone with `harness/`; the native Make turn calls the shared `cad` / `product-to-cad` / `step-parts` skills directly. |
