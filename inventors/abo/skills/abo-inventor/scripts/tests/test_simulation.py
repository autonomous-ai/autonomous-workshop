...
Adapted from the upstream-snapshot `inventors/abo/tests/test_simulation.py`
only to drop the `config`/vendored-harness indirection that no longer exists;
every assertion below is unchanged. See `references/UPSTREAM.md`.
"""
...
# `simulation.py` is now self-contained (design decision D7 deletes the
# vendored harness/ tree this used to load through config.load_harness);
# the deterministic policy/statistics core it needs is defined directly in
# simulation.py and exposed as simulation._HARNESS under the same names.
HARNESS = S._HARNESS
ARTIFACT = "a" * 64
...
class ShortReturnTest(unittest.TestCase):
    ...
    The upstream-snapshot version of this test also asserted the shape of the
    `Need` that the deleted `playtest_job.py` built from a short outcome
    (design decision D7: Playtest's job-level orchestration is gone; the
    native Playtest stage owns that now). What is still this module's own
    behavior -- that a short sample never reports a pass -- is asserted
    below.
    """
