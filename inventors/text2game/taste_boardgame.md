...
## Physics-first balance

For a physical game the trade-off lives in the OBJECT, never in a price table.
"How many do I dare load" is a complete decision when a heavier hull spills
wilder; it needs no seat menu, no spare pool, no row values to make it bite.
Measured 2026-08-22 on dead-stop: three critic rounds each bought one point of
agency with one more rules subsystem - seats, lanes, spares, a seed, a rotation
- because a referee and a critic can only read a document, and a document
cannot feel that mass prices a throw. A rule that prices a physical choice is a
SMELL, not a fix: change the part (mass, rail height, spill geometry) or leave
the question to the table as `physics_untested`. The rules must fit the box
lid: a checker fails a `## First minute` over `FIRST_MINUTE_MAX_WORDS` (220)
and a `## Turn structure` over `TURN_MAX_STEPS` (4).
