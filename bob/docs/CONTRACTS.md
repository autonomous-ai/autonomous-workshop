...
| publish | `harness/publish.py` | Implements docs/research/publish-contract.md Path B. `validate(slug) -> list[str]` (Layer-1 local validator: zip walls, RULES.md, cover, description caps, token belongs to pinned bob user id); `import_draft(slug) -> dict` (POST /designs/import, always status=draft, persists `games/<slug>/published.json` as idempotency ledger); `curate(slug)` (use-case + story-blocks); `flip_public(slug, price_cents)` (POST /designs/{slug}/publish with explicit price in 4000–8000 corner, floored by fulfilment formula) — called by the pipeline ONLY when publish-eligible + validator green (auto-publish per Dee 2026-08-22); `unpublish(slug)`. Every flip sends Telegram notice w/ link + undo command. `BOB_PUBLISH_DRY_RUN=1` default until creds exist — writes `games/<slug>/publish_payload/` and stops. AI-disclosure: bob account only (refuse if token user != pinned id), fixed description line, `ai-created` tag. |
...
| ops | `ops/launchd/ai.autonomous.bob.plist`, `ops/watchdog.sh`, `ops/install.sh`, `ops/uninstall.sh` | launchd StartInterval 1800s → `bob.py tick`; watchdog daily: heartbeat >6h stale ⇒ Telegram alert. |
