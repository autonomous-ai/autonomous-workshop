#!/bin/bash
# Bob dead-man watchdog. Runs hourly from launchd (ai.autonomous.bob.watchdog),
# deliberately NOT from inside bob.py: a dead main loop cannot report its own
# death (text2cad receipt: "silent-channel death must alarm - 4-night scraper
# blackout 6/5-6/8, admindash $13 silent burn 8/9-8/10").
#
# Two alarms:
#   1. Heartbeat stale: state/DAYBOOK.json (the tick loop touches it every
#      tick, per CONTRACTS.md sec 1) is missing or its mtime is older than 6h.
#      Ticks fire every 30 min, so 6h = 12 missed ticks - well past any
#      normal quota back-off (quota_until pauses are 60 min) but fast enough
#      that a dead night costs 6 hours, not text2cad's 4 nights.
...
alert() {
    local msg="bob watchdog: $1"
    if [ -n "${BOB_TELEGRAM_TOKEN:-}" ] && [ -n "${BOB_TELEGRAM_CHAT:-}" ]; then
        curl -sS --max-time 30 \
            "https://api.telegram.org/bot${BOB_TELEGRAM_TOKEN}/sendMessage" \
            -d "chat_id=${BOB_TELEGRAM_CHAT}" \
            --data-urlencode "text=${msg}" >/dev/null \
            || echo "watchdog: telegram send FAILED: ${msg}" >&2
    else
        echo "watchdog: ALARM (no BOB_TELEGRAM_TOKEN/CHAT set): ${msg}" >&2
    fi
}
...
HB_STALE=0
if [ ! -f "$HEARTBEAT" ]; then
    # Missing counts as stale: text2cad's watchdog alarms "if the file is
    # missing or now - mtime > ..." - a deploy that never ticked once must
    # alarm too, not just one that stopped.
    HB_STALE=1
...
if [ "$HB_STALE" -eq 1 ]; then
    ...
    if [ "$RECENT_ALARM" -eq 0 ]; then
        alert "HEARTBEAT STALE - state/DAYBOOK.json last touched ${HB_AGE} (threshold 6h). The tick loop is not running. Check: tail -50 '$TICK_LOG' ; launchctl print gui/$(id -u)/ai.autonomous.bob"
