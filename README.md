...
- **Choose Make:** `workshop wish "..."` defaults to `--make print` for 3D-printed products. Opt into `--make mixed` with Spark to combine printed parts with card, wood, fabric, elastics, hardware and simple purchased electrical components. The internal workshop receives the complete assembly and manufacturing BOM; the public page receives only selected finished-product assets. The [six-pilot portfolio](docs/ideas/mixed-material-spark-mvp.md) tracks capability targets and [live CLI evidence](docs/ideas/mixed-material-cli-runs.md). Digital design does not establish physical manufacture or testing.
...
`--agent` chooses the Workshop Manager runtime; `--model` and `--effort` choose
its model and reasoning level. Those choices apply to both the daydream and
product run, and ordinary resumes preserve them. Supported Codex Spark
token-budget products can explicitly change reasoning with `resume ID --effort
medium`; `--max-tokens 500000000` sets their total allowance while retaining
prior usage. The model and workflow stay fixed. Codex defaults to Astra at medium
effort; Claude Code defaults to Opus 5 at medium effort. Friendly Codex aliases
such as `astra` and `sol` resolve to exact model ids. Grok's first ✨ Spark run,
from a typed brief, produced [Horn Tip](toys/pico-press-horn-tip/):
