"""Build a sealed release package for an approved product."""
...
ReleaseSiteWriter = Callable[
    [ReleaseContext, Path, ArtifactManifest], Receipt
]
...
def _manifest_from_dict(value: Any) -> ArtifactManifest:
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise ContractError("Release manifest is malformed")
    raw_entries = value.get("entries")
    if not isinstance(raw_entries, list):
        raise ContractError("Release manifest entries are malformed")
    entries = []
    for raw_entry in raw_entries:
        if not isinstance(raw_entry, Mapping) or set(raw_entry) != {
            ...
            raise ContractError("Release manifest entry is malformed")
        entries.append(
            ArtifactEntry(
                raw_entry["path"],
                raw_entry["bytes"],
...
def _manifest_path(root: Path) -> Path:
    # Workshop uses a directory literally named ``release``, yielding the
    # stable public filename ``release-manifest.json``. Named test or
...
def _write_manifest_once(root: Path, manifest: ArtifactManifest) -> None:
    """Durably seal Release before any externally visible site effect."""

    path = _manifest_path(root)
    if path.exists() or path.is_symlink():
        raise ContractError("Release manifest seal already exists")
    payload = json.dumps(
    ...
    try:
        ...
        try:
            ...
            raise ContractError("Release manifest seal already exists") from exc
    finally:
        try:
            os.unlink(temporary)
...
def sealed_release_manifest(root: Path) -> ArtifactManifest:
    """Load the immutable seal and prove the current tree is byte-for-byte exact."""

    root = Path(root)
    path = _manifest_path(root)
    if path.is_symlink() or not path.is_file():
        raise ContractError("sealed Release tree is missing its manifest")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ContractError("Release manifest must be valid UTF-8 JSON") from exc
    ...
    if current.to_dict() != manifest.to_dict():
        raise ContractError("sealed Release bytes changed while waiting")
    return manifest


def evidence_claims(context: ReleaseContext) -> Dict[str, Any]:
    ...
    if not claims:
        raise ContractError("Release requires non-empty Playtest evidence")
    return claims
...
class DefaultRelease:
    ...
    ``site_writer`` receives the unchanged context, the sealed Release root,
    ...
    ``release_sha256`` equal to the supplied manifest's artifact hash.  This
    binding covers product.json, MANUAL.md, and the Playtest evidence
    ...
    def __init__(
        ...
        *,
        media_maker: Optional[Callable[[ReleaseContext], Mapping[str, str]]] = None,
    ) -> None:
        if media_maker is not None:
            raise ContractError(
                "Release media_maker is retired; Factory owns generated page media"
            )
    ...
    def __call__(self, context: ReleaseContext) -> ProductRelease:
        if not isinstance(context, ReleaseContext):
            raise ContractError(
                "DefaultRelease requires a ReleaseContext"
            )
        ...
        if self.site_writer is None:
            needs.append(
                Need(
                    "release",
                    "site-page",
                    "Release includes the factual model handoff, and it is not "
                    "complete until its private draft is proven in the Shop.",
                    "Configure the shared Release site writer with authenticated "
        ...
        (root / "MANUAL.md").write_text(markdown, encoding="utf-8")
    ...
    def resume(self, context: ReleaseContext) -> ProductRelease:
        """Resume only the site portion of one already sealed Release job.
        ...
        if not isinstance(context, ReleaseContext):
            raise ContractError(
                "DefaultRelease.resume requires a ReleaseContext"
            )
        context.assert_current()
        if self.site_writer is None:
            raise WaitingFor(
                Need(
                    "release",
                    "site-page",
                    "The sealed Release facts still have no configured model-handoff writer.",
                    "Configure the shared Release site writer with authenticated readback, then resume this exact job.",
                )
            )
        root = context.workspace
        if root.is_symlink() or not root.is_dir() or not any(root.iterdir()):
            raise ContractError("resumed Release requires a non-empty sealed tree")
        manifest = sealed_release_manifest(root)
        try:
            page_value = json.loads((root / "product.json").read_text(encoding="utf-8"))
        except (OSError, UnicodeError, json.JSONDecodeError) as exc:
            raise ContractError(
                "sealed Release product.json must be valid UTF-8 JSON"
            ) from exc
        if not isinstance(page_value, Mapping):
            raise ContractError("sealed Release product.json must be an object")
        claims = evidence_claims(context)
        ...
        if any(page_value.get(key) != value for key, value in expected.items()):
            raise ContractError(
                "sealed Release tree belongs to a different Wish, Taste, Make, or Playtest"
            )
        site_receipt = self._write_site(context, root, manifest)
        return ProductRelease.from_root(
            root,
        ...
        try:
            ...
            raise WaitingFor(
                Need(
                    ...
                    "Reconcile the recorded site intent with authenticated readback, then resume this same Release job.",
                )
            ) from exc
            ...
            raise WaitingFor(
                Need(
                    "release",
                    "site-page",
                    "The site rejected this exact Release handoff before a verified private draft was recorded.",
                    "Correct the site account or model/facts input and resume this same Release job.",
                )
            ) from exc
        context.assert_current()
        if not isinstance(site_receipt, Receipt):
            raise ContractError(
                "Release site writer must return an authenticated Receipt"
            )
        return site_receipt
