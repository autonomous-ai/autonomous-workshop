...
from workshop._validation import require_sha256, require_utc_timestamp
...
_PLACEHOLDERS = frozenset(("tbd", "todo", "unknown", "placeholder", "n/a", "later"))
...
def _meaningful_text(value: Any, label: str, maximum: int = 4_000) -> str:
    text = _bounded_text(value, label, maximum)
    if _normalize(text) in _PLACEHOLDERS:
        raise ContractError("%s must not be a placeholder" % label)
    return text
...
def _validate_features(brief: Mapping[str, Any]) -> Mapping[str, str]:
    ...
    for feature in features:
        ...
        by_id[feature_id] = _meaningful_text(feature["text"], "concept brief feature text", 2_000)
    return by_id
...
def _validate_components(brief: Mapping[str, Any]) -> Mapping[str, Mapping[str, Any]]:
    ...
    for component in components:
        ...
        _meaningful_text(component["name"], "concept brief component %s name" % key, 200)
        _meaningful_text(component["purpose"], "concept brief component %s purpose" % key)
        _meaningful_text(component["form"], "concept brief component %s form" % key)
        _dimensions_mm(
            component["dimensions_mm"],
            "concept brief component %s dimensions_mm" % key,
        )
        _meaningful_text(component["placement"], "concept brief component %s placement" % key)
        _meaningful_text(component["interfaces"], "concept brief component %s interfaces" % key)
...
def _validate_facts(
    ...
    *,
    ...
    for source in sources:
        ...
        _bounded_text(source["origin"], "concept research source origin", 4_000)
        excerpt = _bounded_text(
            source["excerpt"], "concept research source excerpt", 8_000
        )
        require_sha256(
            source["excerpt_sha256"], "concept research source excerpt_sha256"
        )
        expected_excerpt = hashlib.sha256(
            json.dumps(
                excerpt,
                sort_keys=True,
                separators=(",", ":"),
        ...
        if source["excerpt_sha256"] != expected_excerpt:
            raise ContractError("concept research source excerpt_sha256 differs from excerpt")
        require_utc_timestamp(
            source["retrieved_at"], "concept research source retrieved_at"
        )

    findings = research.get("findings")
    if isinstance(findings, (str, bytes)) or not isinstance(findings, Sequence) or not findings:
        raise ContractError("concept research must state at least one finding")
    for finding in findings:
        if not isinstance(finding, Mapping) or set(finding) != {"finding", "source_ids"}:
            raise ContractError("concept research finding fields are invalid")
        _bounded_text(finding["finding"], "concept research finding", 8_000)
        references = finding["source_ids"]
        if (
            ...
            raise ContractError(
                "concept research finding must name distinct recorded source ids"
            )
    ...
    if isinstance(facts, (str, bytes)) or not isinstance(facts, Sequence) or not facts:
...
def _validate_drawing_instructions(
    ...
    *,
    ...
    if require_presentation:
        _bounded_text(
            drawing_instructions["presentation"],
            "concept drawing presentation treatment",
            2_000,
        )
    for role, expected_references in _EXPECTED_OVERALL_REFERENCES.items():
        ...
        if (
            isinstance(references, (str, bytes))
            or not isinstance(references, Sequence)
            or tuple(references) != expected_references
        ):
            raise ContractError(
    ...
    for key, component in component_keys.items():
        ...
        if (
            isinstance(references, (str, bytes))
            or not isinstance(references, Sequence)
            or tuple(references) != _COMPONENT_REFERENCES
        ):
            raise ContractError(
...
def evaluate_concept_brief(tree: ConceptTree, *, wish: Wish) -> Mapping[str, Any]:
    ...
    for field_name in ("object", "category"):
        ...
        _meaningful_text(brief[field_name], "concept brief %s" % field_name, 2_000)
    ...
    if hasattr(tree, "provenance") and not has_fit_target:
        raise ContractError("concept brief is missing its fit_target")
    ...
    _validate_drawing_instructions(
        tree.drawing_instructions,
        component_keys=component_keys,
        require_presentation=hasattr(tree, "provenance"),
    )
    _validate_descriptor(tree.descriptor, component_keys=component_keys)
    return MappingProxyType({
        "checks_kind": "concept-structure-v1",
        "object": brief["object"],
