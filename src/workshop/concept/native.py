...
def _strict_json_object(path: Path, label: str) -> tuple[dict[str, Any], bytes]:
    try:
        ...
        if before.st_size > MAX_CONCEPT_JSON_BYTES:
            raise ArtifactError(
                "%s exceeds the %d-byte limit" % (label, MAX_CONCEPT_JSON_BYTES)
            )
    ...
    try:
        value = json.loads(
            ...
            parse_constant=lambda constant: (_ for _ in ()).throw(
                ValueError("non-finite JSON number: %s" % constant)
            ),
        )
...
class DerivedWish:
    ...
    def assert_context(self, wish: Wish) -> None:
        ...
        if (
            self.wish_sha256 != hashlib.sha256(_canonical_json(wish.to_dict())).hexdigest()
            or self.product_id != wish.product_id
    ...
    def assert_context(
        ...
        *,
        ...
        if expected_round is not None:
            if type(expected_round) is not int or not 1 <= expected_round <= 100:
                raise ContractError("expected native Concept round must be from 1 through 100")
            if self.round != expected_round:
                raise ContractError("native Concept belongs to a stale repair round")
