...
def _shortlist_schema_for(routable_count: int) -> Dict[str, Any]:
    """Bind structured-output cardinality to the catalog validator."""
    ...
    return {
        **_SHORTLIST_SCHEMA,
        "properties": {
            **_SHORTLIST_SCHEMA["properties"],
            "inventor_ids": {
                **_SHORTLIST_SCHEMA["properties"]["inventor_ids"],
                "minItems": minimum,
                "maxItems": maximum,
            },
        },
...
class CodexSemanticManager:
    ...
    def retrieve(self, context: RoutingContext):
        ...
        if not routable_count:
            raise WaitingFor(
                Need(
                    job="wish",
                    capability="inventor-fit",
                    reason="This inventor catalog has no routable Inventors.",
                    instructions=(
                        "Mark at least one Inventor active or experimental, then run "
                        "this exact Wish again."
                    ),
                )
            )
        ...
        payload = self._invoke(
            prompt=prompt,
            schema=_shortlist_schema_for(routable_count),
