...
from workshop.artifacts.core import MAX_FILE_BYTES, build_artifact_manifest
from workshop.errors import ContractError
from workshop.make import Made
...
_GAME_STYLE_ORDER = ("optimizing", "social", "exploratory", "adversarial")
_GAME_STYLES = frozenset(_GAME_STYLE_ORDER)
...
def _validate_game(
    ...
    *,
    ...
    for name in (
        "balance_cases",
        "ordered_pairing_cases",
        "first_seat_cases",
        "move_order_cases",
        "solved_first_player_cases",
        "exploit_cases",
        "choice_cases",
        "flow_cases",
    ):
    ...
    for name in (
        ...
        _int_measurement(measurements, name, minimum=0, maximum=0)
    if measurements["ordered_pairing_cases"] != 16:
        raise ContractError("game proof must cover all 16 ordered style pairings")
    if measurements["first_seat_cases"] != 2:
        raise ContractError("game proof must cover both designated first seats")
    if measurements["move_order_cases"] != 2:
        raise ContractError("game proof must distinguish first and second movers")
    if (
    ...
    ordered_pairing_counts = {
        "%s|%s" % (left, right): 0
        for left in _GAME_STYLES
        for right in _GAME_STYLES
    }
    ...
    for game in trace_document["games"]:
        ...
        try:
            outcome = json.loads(game.get("outcome", ""))
        except (TypeError, ValueError, json.JSONDecodeError):
            outcome = None
        winner = outcome.get("winner") if isinstance(outcome, Mapping) else None
        expected_first_seat = game.get("index", -1) % 2
        pairing_index = game.get("index", -1) % 16
        expected_styles = [
            _GAME_STYLE_ORDER[pairing_index // 4],
            _GAME_STYLE_ORDER[pairing_index % 4],
        ]
        if (
            not isinstance(game, Mapping)
            or type(game.get("index")) is not int
            or game["index"] in seen_indices
            or type(game.get("seed")) is not int
            ...
            or len(styles) != 2
            or any(style not in _GAME_STYLES for style in styles)
            or styles != expected_styles
            or game.get("first_seat") not in (0, 1)
            or game["first_seat"] != expected_first_seat
            or not isinstance(actions, list)
            or len(actions) != game["turns"]
        ...
        seen_indices.add(game["index"])
        seen_seeds.add(game["seed"])
        observed_styles.update(styles)
        pairing_key = "%s|%s" % (styles[0], styles[1])
        ordered_pairing_counts[pairing_key] += 1
        ordered_pairing_wins[pairing_key][str(winner)] += 1
        first_seat_counts[str(game["first_seat"])] += 1
        move_order = "first" if winner == game["first_seat"] else "second"
        move_order_wins[move_order] += 1
        first_action = game.get("first_action")
        if first_action is None and actions:
            first_turn = actions[0]
            first_action = first_turn.get("action", first_turn)
        first_action_key = _json_text(first_action)
        first_action_row = first_action_results.setdefault(
            first_action_key, {"games": 0, "first_mover_wins": 0}
        )
        first_action_row["games"] += 1
        first_action_row["first_mover_wins"] += winner == game["first_seat"]
    ...
    if (
        any(count == 0 for count in pairing_values)
        or sum(pairing_values) != requested
        or max(pairing_values) - min(pairing_values) > 1
    ):
        raise ContractError("game proof lacks balanced coverage of all ordered pairings")
    first_seat_values = tuple(first_seat_counts.values())
    if (
        any(count == 0 for count in first_seat_values)
        or sum(first_seat_values) != requested
        or max(first_seat_values) - min(first_seat_values) > 1
    ):
        raise ContractError("game proof lacks alternating designated first seats")
    if any(count == 0 for count in move_order_wins.values()):
        raise ContractError("game proof contains wins from only one move order")
    if (
        optimizing_self_play_cases < 1
        or optimizing_self_play_first_wins == optimizing_self_play_cases
    ):
        raise ContractError(
            "optimizing self-play exposes a solved first-mover win"
        )
    if (
        measurements.get("solved_first_player_cases")
        != optimizing_self_play_cases
        or measurements.get("solved_first_player_wins")
        != optimizing_self_play_first_wins
    ):
        raise ContractError("solved first-player measurements do not match traces")
    if (
        ...
        or analysis_document.get("ordered_pairing_games")
        != ordered_pairing_counts
        or analysis_document.get("ordered_pairing_wins")
        != ordered_pairing_wins
        or analysis_document.get("first_seat_games") != first_seat_counts
        or analysis_document.get("move_order_wins") != move_order_wins
        or analysis_document.get("first_keeper_results")
        != {
            "games": requested,
            "wins": move_order_wins["first"],
            "losses": move_order_wins["second"],
        }
        or analysis_document.get("first_action_results")
