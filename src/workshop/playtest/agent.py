...
GAME_STYLES = FINITE_GAME_STYLES
PLAYER_ROLES = (
    "optimizing-player",
    "first-time-player",
    "exploratory-player",
...
def _locked_cad_design(context: PlaytestContext) -> Tuple[Mapping[str, Any], Mapping[str, Any]]:
    ...
    if (
        ...
        or generator.get("id") != LOCKED_CAD_GENERATOR_ID
        or generator.get("version") != LOCKED_CAD_GENERATOR_VERSION
        or not isinstance(action, Mapping)
        or not isinstance(action.get("parts"), list)
    ):
        raise ValueError("Make lacks a complete locked STEP-first CAD design")
    try:
        validated_action = validate_cad_design_action(action)
...
def _assert_canonical_cad_sources(
    ...
    expected = canonical_cad_project_sources(action)
...
def _recheck_locked_cad(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="workshop-cad-recheck-") as temporary:
            ...
            build = locked_cad_project_verifier().verify(
                action,
...
def _trusted_seed_strategy(
    ...
    if isinstance(declared, str):
        ...
        requested_games = simulator_design.get("minimum_complete_games")
        if (
            match is not None
            and int(match.group(1)) < 2**32
            and type(requested_games) is int
            and requested_games >= 1
        ):
            return {
    ...
    if rule_kind == "ordered-shore-sweep":
        raise ValueError("shore-sweep rules lack their fixed Invent seed strategy")
    return {
...
def _trusted_game_rules(
    ...
    return {
        "schema_version": 2,
        "protocol": FINITE_GAME_RULES_PROTOCOL,
        "kind": rule_kind,
        "title": spec["title"],
        "theme": spec["theme"],
        "players": 2,
        ...
        "seed_strategy": dict(_trusted_seed_strategy(rule_kind, lane_contract)),
        "policy_contract": {
        ...
        "simulator": {
            ...
            "id": FINITE_GAME_SIMULATOR_ID,
            "version": FINITE_GAME_SIMULATOR_VERSION,
        },
    }


def _trusted_game_rules_markdown(rules: Mapping[str, Any]) -> str:
    lines = [
        "# %s" % rules["title"], "", str(rules["theme"]), "",
        "## Players", "", "Two players.", "", "## Turn order", "",
        str(rules["turn_order"]), "", "## Setup", "", str(rules["setup"]),
        "", "## Your turn", "", str(rules["legal_actions"]), "",
        "## End and winner", "", str(rules["end_condition"]),
        str(rules["winner"]), str(rules["ties"]), "",
        ...
        (
            ...
            % (rules["seed_strategy"]["prng"], rules["seed_strategy"]["kind"])
        ),
        "", "## Evidence boundary", "",
    ...
    if isinstance(ordered, list) and ordered:
        ...
        configuration.extend(
            "| %d | `%s` | %d |" % (index, token["part_id"], token["sweep"])
...
def default_sealed_game_simulator(
    ...
    if source_bytes != FINITE_GAME_SIMULATOR_SOURCE.encode("utf-8"):
        raise ValueError("sealed simulator source differs from the pinned Workshop template")
    rules = _sealed_json(context, rules_path)
    simulator_declaration = rules.get("simulator")
    raw_spec = rules.get("game_spec")
    if (
        rules.get("schema_version") != 2
        or rules.get("protocol") != FINITE_GAME_RULES_PROTOCOL
        or not isinstance(raw_spec, Mapping)
        or rules.get("kind") != raw_spec.get("rule_kind")
        or simulator_declaration
        != {
            "path": source_path,
            "id": FINITE_GAME_SIMULATOR_ID,
            "version": FINITE_GAME_SIMULATOR_VERSION,
    ...
    try:
        ...
        raise ValueError("sealed game rules contain an invalid game_spec") from exc
    seed_strategy = rules.get("seed_strategy")
    if (
        not isinstance(seed_strategy, Mapping)
        or plan.get("seed_strategy") != seed_strategy
        ...
        raise ValueError("seeded plan does not match the sealed seed strategy")
    if seed_strategy.get("kind") == "fixed-index-offset-u32":
        if (
            type(seed_strategy.get("base_seed")) is not int
            or not 0 <= seed_strategy["base_seed"] < 2**32
            or plan.get("base_seed") != seed_strategy["base_seed"]
            or plan.get("requested_games")
            != seed_strategy.get("requested_games")
        ):
            raise ValueError("fixed seeded plan does not match Invent")
    elif seed_strategy.get("kind") != "request-base-index-offset":
        raise ValueError("sealed seed strategy is unsupported")
    if any(
        request.get("index") != index
        or request.get("seed")
        != (plan["base_seed"] + index) % (2**32)
        for index, request in enumerate(plan.get("games", ()))
    ):
        raise ValueError("seeded plan does not follow its exact index offset")

    lane_contract = rules.get("invent_lane_contract")
    lane_contract_sha256 = rules.get("invent_lane_contract_sha256")
    if lane_contract is None:
        if lane_contract_sha256 is not None:
            raise ValueError("sealed game rules contain an unbound Invent contract hash")
    elif (
        not isinstance(lane_contract, Mapping)
        or lane_contract_sha256 != _sha256(lane_contract)
    ):
        raise ValueError("sealed game rules contain an invalid Invent contract binding")
    mechanical = _sealed_json(context, "playtest/mechanical.json")
    digital_plan = mechanical.get("digital_test_plan")
    if (
        not isinstance(digital_plan, Mapping)
        or digital_plan.get("invent_lane_contract") != lane_contract
        or digital_plan.get("invent_lane_contract_sha256")
        != lane_contract_sha256
    ):
        raise ValueError("sealed game rules and mechanical plan bind different Invent contracts")
    design = _sealed_json(context, "cad/design.json")
    action = design.get("action")
    if not isinstance(action, Mapping):
        raise ValueError("sealed game rules lack their exact Make action")
    expected_rules = _trusted_game_rules(action, lane_contract)
    if dict(rules) != expected_rules:
        raise ValueError("sealed game rules differ from the trusted Workshop reconstruction")
    rules_markdown, unused_markdown_sha256 = _sealed_entry(context, "game/RULES.md")
    del unused_markdown_sha256
    if rules_markdown.read_bytes() != _trusted_game_rules_markdown(
        expected_rules
    ).encode("utf-8"):
        raise ValueError("sealed game rules README differs from trusted rules bytes")
    ordered_tokens = rules.get("ordered_tokens")
    expected_ordered_tokens = [
        {"part_id": part_id, "sweep": sweep}
        for part_id, sweep in zip(
            spec["token_part_ids"], spec["token_sweep_values"]
        )
    ]
    if ordered_tokens != expected_ordered_tokens:
        raise ValueError("sealed ordered token rules do not match game_spec")
    ...
    if (
        ...
        or simulator.get("id") != FINITE_GAME_SIMULATOR_ID
        or simulator.get("version") != FINITE_GAME_SIMULATOR_VERSION
        or not isinstance(games, list)
        or len(games) != len(plan["games"])
    ):
        raise ValueError("pinned game simulator output provenance is incomplete")

    normalized_games = []
    for expected_request, game in zip(plan["games"], games):
        ...
        try:
            trusted = replay_finite_game(spec, expected_request)
        except ContractError as exc:
            raise ValueError("seeded game plan is invalid") from exc
        if dict(game) != trusted:
            raise ValueError(
                "pinned game simulator trace differs from trusted rule replay"
            )
        issue_findings = []
        ...
        normalized_games.append(
            {
                ...
                "first_seat": game.get("first_seat"),
                "rule_kind": game.get("rule_kind"),
                "completed": game.get("completed"),
                "turns": game.get("turns"),
                "actions": game.get("actions"),
                "restoration_log": game.get("restoration_log"),
                "first_action": game.get("first_action"),
                "outcome": (
                    json.dumps(outcome, sort_keys=True, separators=(",", ":"))
                    if outcome is not None
                    else "no-complete-outcome"
                ),
                "rng_values": game.get("rng_values"),
                "rng_state": game.get("rng_state"),
...
def _seal_game_release_proof(
    ...
    *,
    ...
    ordered_pairing_counts = {
        "%s|%s" % (left, right): 0
        for left in GAME_STYLES
        for right in GAME_STYLES
    }
    ordered_pairing_wins = {
        key: {"0": 0, "1": 0}
        for key in ordered_pairing_counts
    }
    first_seat_counts = {0: 0, 1: 0}
    move_order_wins = {"first": 0, "second": 0}
    first_action_results: Dict[str, Dict[str, int]] = {}
    optimizing_self_play_cases = 0
    optimizing_self_play_first_wins = 0
    unexpected_pairings = 0
    adversarial_games = 0
    total_turns = 0
    issue_count = 0
    for game in games:
        ...
        styles = game["player_styles"]
        pairing_key = (
            "%s|%s" % (styles[0], styles[1])
            if isinstance(styles, list) and len(styles) == 2
            else ""
        )
        if pairing_key in ordered_pairing_counts:
            ordered_pairing_counts[pairing_key] += 1
        else:
            unexpected_pairings += 1
        first_seat = game.get("first_seat")
        if first_seat in first_seat_counts:
            first_seat_counts[first_seat] += 1
        ...
        if isinstance(outcome, Mapping):
            ...
            if winner in seat_wins:
                seat_wins[winner] += 1
                if pairing_key in ordered_pairing_wins:
                    ordered_pairing_wins[pairing_key][str(winner)] += 1
                move_order = "first" if winner == first_seat else "second"
                move_order_wins[move_order] += 1
                actions = game.get("actions")
                first_action = game.get("first_action")
                if first_action is None and isinstance(actions, list) and actions:
                    first_turn = actions[0]
                    if isinstance(first_turn, Mapping):
                        first_action = first_turn.get("action", first_turn)
                if first_action is not None:
                    first_action_key = _canonical(first_action).decode("utf-8")
                    row = first_action_results.setdefault(
                        first_action_key, {"games": 0, "first_mover_wins": 0}
                    )
                    row["games"] += 1
                    row["first_mover_wins"] += winner == first_seat
                if styles == ["optimizing", "optimizing"]:
                    optimizing_self_play_cases += 1
                    if winner == first_seat:
                        optimizing_self_play_first_wins += 1
            if winner_style in style_wins:
                style_wins[winner_style] += 1
    pairing_values = tuple(ordered_pairing_counts.values())
    ordered_pairing_failure = int(
        unexpected_pairings != 0
        or sum(pairing_values) != requested_games
        or any(count == 0 for count in pairing_values)
        or max(pairing_values) - min(pairing_values) > 1
    )
    first_seat_failure = int(
        sum(first_seat_counts.values()) != requested_games
        or any(count == 0 for count in first_seat_counts.values())
        or max(first_seat_counts.values()) - min(first_seat_counts.values()) > 1
    )
    move_order_failure = int(
        sum(move_order_wins.values()) != completed
        or any(count == 0 for count in move_order_wins.values())
    )
    solved_first_player_failure = int(
        optimizing_self_play_cases > 0
        and optimizing_self_play_first_wins == optimizing_self_play_cases
    )
    balance_failure = int(
        completed != requested_games
        or any(wins == 0 for wins in seat_wins.values())
        or any(wins == 0 for wins in style_wins.values())
        or ordered_pairing_failure
        or first_seat_failure
        or move_order_failure
        or solved_first_player_failure
    )
    measurements = {
        "requested_games": requested_games,
        ...
        "ordered_pairing_cases": len(ordered_pairing_counts),
        "ordered_pairing_failures": ordered_pairing_failure,
        "first_seat_cases": len(first_seat_counts),
        "first_seat_failures": first_seat_failure,
        "move_order_cases": len(move_order_wins),
        "move_order_failures": move_order_failure,
        "solved_first_player_cases": optimizing_self_play_cases,
        "solved_first_player_wins": optimizing_self_play_first_wins,
        "solved_first_player_failures": solved_first_player_failure,
    ...
    analysis_document = {
        ...
        "ordered_pairing_games": ordered_pairing_counts,
        "ordered_pairing_wins": ordered_pairing_wins,
        "first_seat_games": {
            str(key): value for key, value in first_seat_counts.items()
        },
        "move_order_wins": move_order_wins,
        "first_keeper_results": {
            "games": completed,
            "wins": move_order_wins["first"],
            "losses": move_order_wins["second"],
        },
        "first_action_results": first_action_results,
        "measurements": measurements,
    }
...
def _game_plan(
    artifact_sha256: str,
    game_count: int,
    seed_strategy: Optional[Mapping[str, Any]] = None,
) -> Mapping[str, Any]:
    if seed_strategy is None:
        ...
        if (
            strategy.get("kind") != "fixed-index-offset-u32"
            or type(strategy.get("base_seed")) is not int
            or not 0 <= strategy["base_seed"] < 2**32
            or strategy.get("modulus") != 2**32
            or strategy.get("prng") != "mulberry32"
            or strategy.get("log_every_generated_u32") is not True
            or type(strategy.get("requested_games")) is not int
            or strategy["requested_games"] != game_count
        ):
            raise ContractError("sealed game seed strategy is unsupported")
        base_seed = strategy["base_seed"]
    pairings = tuple(
        (left, right)
        for left in GAME_STYLES
        for right in GAME_STYLES
...
def _validate_game_simulation(
    ...
    for expected, game in zip(expected_games, games):
        ...
        if (
            ...
            or game.get("first_seat") != expected["first_seat"]
            or not isinstance(game.get("completed"), bool)
            or type(game.get("turns")) is not int
            or game["turns"] < 0
            or game["turns"] > 10_000
            or not isinstance(game.get("actions"), list)
            or len(game["actions"]) != game["turns"]
            or not all(isinstance(item, Mapping) for item in game["actions"])
        ...
        for field in (
            ...
            if field in game:
                _canonical({"value": game[field]})
                normalized_game[field] = game[field]
...
class LaneAwarePlaytester:
    ...
    def __init__(
        ...
        *,
        ...
        if lane_providers is None:
            from workshop.playtest.providers import WorkshopLanePlaytestProviders
    ...
    def _run_game_simulation(
        ...
        sealed_seed_strategy: Optional[Mapping[str, Any]] = None
        if context.blueprint.lane == "invented-games":
            inventory_paths = {
                entry.path for entry in context.made.artifact_manifest.entries
            }
            if "game/rules.json" in inventory_paths:
                sealed_rules = _sealed_json(context, "game/rules.json")
                candidate_strategy = sealed_rules.get("seed_strategy")
                if (
                    isinstance(candidate_strategy, Mapping)
                    and candidate_strategy.get("kind")
                    == "fixed-index-offset-u32"
                ):
                    sealed_seed_strategy = candidate_strategy
        plan = _game_plan(
            context.made.artifact_sha256,
            self.game_count,
            sealed_seed_strategy,
        )
        try:
        ...
        if game_bundle is not None:
            ...
            game_blocked = (
                ...
                or any(
                    ...
                    for name in (
                        "balance_failures",
                        "ordered_pairing_failures",
                        "first_seat_failures",
                        "move_order_failures",
                        "solved_first_player_failures",
                        "exploits_found",
                        "degenerate_choices",
                        "flow_failures",
                    )
