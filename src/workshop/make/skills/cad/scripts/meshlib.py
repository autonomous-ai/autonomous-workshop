...
neither is obviously wrong. So the weld, the edge table and the defect counts
live here once, next to `cadfits.py`, and both CLIs import them.

Everything here is triangle soup in millimetres, and nothing here knows where it
came from. `printlib` owns the source->mesh step; this owns what a mesh is once
you have one. Keeping the two apart is why the analysis below did not have to
change when the input stopped being an exported STL and became a tessellated
B-rep.

Depends on numpy and scipy (connected components); the hole fill in
`repair_mesh` additionally needs shapely.

    python "$CAD_SKILL_ROOT/scripts/meshlib.py"        # self-check
"""
