...
3. **Write a natural-language CAD brief.** Extract dimensions, units, coordinate convention, feature intent, output paths, assumptions, and validation targets from all provided inputs — prose, reference images, technical drawings. Use `references/cad-brief.md`.
   For an image-derived build spec, section 6g is the design contract: it must
   already select every active exterior, mechanical, electrical, lighting and
   bought-device domain after its research log. Implement those selections;
   do not reopen the candidate search or invent a different mechanism, topology
   or MPN during CAD. If an active domain is missing or marked incomplete,
   return it to the analysis/research phase instead of hiding the gap behind a
   generic stand-in.
...
7. **Generate explicit targets.** Run `scripts/gen` on explicit generator targets only; do not run directory-wide generation. Pass all affected targets to one invocation instead of starting one process per entry. **Always pass `--write`** on the run that ends an edit cycle: `gen` is the only command that writes a `.step`, and a project whose newest artifacts are a GLB and an STL is indistinguishable from one whose STEP write failed. Use `scripts/export` for STL/3MF/GLB mesh files. For secondary geometry-only exports after a successful `--write`, prefer the fresh STEP target so export does not invoke the Python builders again.
...
10. **Review appearance with the instrument that can see it.** A silhouette is sufficient for an exterior-outline comparison and can hide every interior part of an exposed mechanism beneath a larger base. When the appearance claim depends on interior openings, overlapping parts, occurrence colours, or relative visual mass, inspect browser/CAD-viewer renders when available; otherwise run `scripts/render_review <source> --view front --view top --view iso -o <project>/snap/review` and inspect the shaded PNGs. This is visual evidence only and never replaces geometry gates.
11. **Repair and rerun.** If a check fails, change the smallest responsible source section, regenerate, and rerun the failed validation.
...
`check_fit`, `check_mesh`, `check_motion`, `check_mount`, `check_thickness`) and
...
- Do not declare image-derived CAD complete while its spec and source disagree, a defining landmark lacks a local geometry target, a reference viewpoint is omitted, source-vs-STEP comparison finds drift, or any likeness pair is below 0.90 without the user explicitly accepting that measured **failing** result. The integrated final runner keeps that floor fixed. After three consecutive non-improving rounds for the same view and reference, `--likeness-accept-mismatch "<reason>"` may record the user's delivery decision: the raw likeness row remains failed, the pipeline headline names an accepted failing gate, and the reason is preserved. A fresh improving row cancels that path so an active repair loop is never cut short. Run `scripts/verify_project <project> --image-derived --likeness-ref LABEL=PATH` with every usable reference and classify power explicitly: `--powered` for any functional electrical load, otherwise `--unpowered`. The mode requires `measure/check_spec.py` and `measure/check_landmarks.py` precisely because a generic gate cannot infer those project-specific claims. See `references/image-derived-verification.md`.
...
- Outside a restricted Workshop product run, delete the project's `__cadgen__/` after editing a shared `*_lib.py`, before the next build. `scripts/gen` does not notice that a sibling library module changed and `--force` does not repair it: you get `built` on stdout and the **previous** geometry in the `.step`. Inside a restricted Workshop product run, never manually delete that protected cache; regenerate every affected target explicitly with `scripts/gen <targets...> --write --force`, use non-`--fresh` iterative preflight, and let the trusted host perform the authoritative isolated fresh rebuild. Explicit regeneration is not proof that the shared-library cache defect is repaired; inspect the changed geometry and let the trusted fresh rebuild establish freshness. A protected empty cache directory is harmless and must not be reported as a blocker. The image-derived runner adds a three-view source-vs-STEP comparison, but silhouettes can miss an internal change; follow the environment-specific freshness path rather than relying on that partial detector. See "Shared-library cache defect" in `references/step-generation.md`.
...
- Do not declare a model able to hold a bought part while `scripts/check_mount` exits non-zero on the project. Deriving a seat with `cadmount` is not proof the model has one: the generator may never have subtracted it, may have cut it in the wrong place, or may have added a feature later that ate half of it, and `validate`, `interfere`, `check_fit`, `check_motion` and `check_mesh` pass all three. The gate places the component's own STEP into the combined entry at a declared pose and measures the built solids — clash, clearance, and whether a screw can reach each hole of the component's own mounting pattern from either side. Keep every bought/foreign STEP under `ref/` and declare it in `measure/mounts.json`, copying its checksum from the catalog/spec row; final `verify_project` rejects a supplier file hidden under `catalog/` or elsewhere, then requires every STEP under `ref/` to have a mount row whose `sha256` matches the current file. A derived envelope cannot replace the supplier source in that audit. `"bolts": false` is the escape hatch for a strapped or glued component. See `references/bought-parts.md`.
- Do not declare a powered assembly complete merely because its electrical parts are mounted. Every functional motor, servo, solenoid, LED, lamp, beacon, strip or illuminated control invokes `$electromechanical-integration`, records GitHub analogy and public component-service searches, and traces one complete path per independently rated load through an onboard or explicitly external source, protection where required, switching/control, interconnects and the return path. A motor or light is a load, never an energy source. Model every carried battery holder, switch, controller, connector, board, visible lens and diffuser from catalog evidence when available, give each seat a mount declaration, and reserve a routed clearance envelope plus strain relief for its wires. The bought battery, hidden electronics and flexible loom do **not** have to appear in the combined assembly or render: they may be validation-only STEP envelopes, provided their holder/channel/retention remains real product geometry and `check_mount` inserts those envelopes to measure clashes against the named parts. A centerline or schematic is not a wire-clearance proof. An external source still requires a physical inlet or lead and an explicit product boundary. When the brief says the product is self-contained or portable and must run, spin, move or emit light, do not discard its battery, switch or wiring as an unapproved "outside CAD scope" assumption. Verify source voltage/current and switch/controller ratings against actuator running/stall demand or other loads' continuous/peak demand from authoritative component evidence; no geometry gate can prove electrical compatibility. Write schema 3 `measure/power.json`, run `python "$(workshop skills path)/electromechanical-integration/scripts/check_power" <project-dir>`, and pass `--powered` to final `verify_project`; an absent or failing manifest blocks the powered-product claim. For every `removable_socket` light, implement the approved spec 6f interface: seat the exact purchased socket, or derive a justified printed receiver from authoritative male geometry with `cadfits` while retaining bought contacts; model the insertion path, lock ramp/channel, stop/capture, connector and tool access; and link insert, lock, retained pull, unlock and remove conditions in `measure/motion.json`. `check_power`, `check_mount` and `check_motion` prove declarations, seating and rigid-body paths respectively. None proves printer-process fit: only a `passed` coupon made with the exact hardware and final material/process/orientation permits that claim; `planned` must be reported as physically unverified and `failed` blocks delivery.
...
- Never call an exposed mechanism visually reviewed from silhouette images alone when a larger occurrence hides its interior layout. Use the CAD viewer or `scripts/render_review` and inspect the shaded output; keep silhouette renders for outline/likeness claims.
