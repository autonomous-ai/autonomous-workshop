...
python "$CAD_SKILL_ROOT/scripts/render_set" ...      # reference whole-set render: scenes to STEP, then cameras through render_review
...
11. **Review appearance with the instrument that can see it.** A silhouette suffices for an exterior-outline comparison and can hide every interior part of an exposed mechanism beneath a larger base. When the claim depends on interior openings, overlapping parts, occurrence colours or relative visual mass, use the CAD viewer when available, otherwise `scripts/render_review <source> --view front --view top --view iso -o <project>/snap/review` and inspect the shaded PNGs. Visual evidence only; it never replaces a geometry gate. **A whole-set render (a multi-camera hero, a multi-position state sheet, an Evidence Scene) starts from `scripts/render_set`, not from a previous toy's frame script.** Copying a prior toy's script carries forward whatever it did to tessellate, and more than one has reimplemented its own copy of `render_review`'s tessellation to name the angular tolerance instead of calling `tessellate_occurrences(..., angular=...)` — that copy reads and writes no cache, so every camera on every scene and every occurrence unchanged since the last board state retessellates from nothing. `render_set` builds each scene to STEP once and renders every camera through `render_review.build_shape`/`tessellate_occurrences` directly, so repeated cameras and unmoved occurrences across states are cache hits. It chooses no camera and builds no scene itself — name every `--scene` and `--view` explicitly.
