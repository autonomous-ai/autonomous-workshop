...
Only session ancestry, model context and native usage metadata are retained.
"""
...
class _TokenCountUsage:
    """Preserve the strict legacy notification contract for ledger-free files."""

    def __init__(self):
        self.totals = {key: 0 for key in COUNTERS}
        self.task_totals = None
    ...
    def observe(self, record):
        ...
        if info is None:  # Rate-limit-only notification carries no token usage.
            return
        if not isinstance(info, dict) or not isinstance(self.model, str) or not 1 <= len(self.model) <= 128:
        ...
        if self.task_start:
            ...
            if current == last:
                self.task_totals = {key: 0 for key in COUNTERS}
            elif self.task_totals is None or any(
                current[key] != self.task_totals[key] + last[key] for key in COUNTERS
            ):
                raise UsageUnavailable("native usage task baseline is ambiguous")
            self.task_start = False
        delta = {key: current[key] - self.task_totals[key] for key in COUNTERS}
        if any(v < 0 for v in delta.values()):
            raise UsageUnavailable("native token counters regressed")
        _counters(delta)
        self.totals = {key: self.totals[key] + delta[key] for key in COUNTERS}
        self.task_totals = current
        self.models.add(self.model)
        self.observations += 1
        self.last_at = record.get("timestamp")
...
def read_thread_usage(path, *, thread_id, workspace):
    ...
    if ledger.present:
        ...
        observed = legacy
    return {
        "thread_id": thread_id, "models": sorted(observed.models), "tokens": observed.totals,
        "last_observed_at": observed.last_at,
        "status": "observed" if observed.observations else "pending",
        **({"accounting_source": "response-ledger-v1"} if ledger.present else {}),
    }
