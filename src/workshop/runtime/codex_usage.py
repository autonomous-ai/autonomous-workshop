...
def read_thread_usage(path, *, thread_id, workspace):
    ...
    for record in _records(path):
        ...
        if task_totals is None:
            # Process resumes reset counters, while native follow-up tasks
            # can retain them. Accept only an exact first-request reset or
            # exact continuity with the preceding observed task, across every
            # counter. Never infer a reset from an arbitrary regression.
