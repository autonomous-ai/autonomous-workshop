...
# A likeness Wish names an existing object whose reference images live on
# whichever CDN a search happens to return, so an enumerated host list cannot
# serve them.  Text-derived shape guesses are not a substitute: they produce a
# product that does not look like the named thing.  The containment that
...
# and every `.env*`, so a read remains bounded to the run's own workspace.
