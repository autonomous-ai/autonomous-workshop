...
from workshop.release.public_archive import (
    ...
    read_public_archive_file,
    parse_public_archive_document,
...
def prepare_revision(source: Path, prompt: str) -> tuple[Wish, bytes]:
    ...
    manifest_bytes = read_public_archive_file(source / "MANIFEST.json", "public archive manifest")
    document = parse_public_archive_document(manifest_bytes, "public archive manifest")
    ...
    for entry in manifest.entries:
        ...
        content = read_public_archive_file(source / entry.path, "revision source file", allow_empty=True)
        if len(content) != entry.bytes or hashlib.sha256(content).hexdigest() != entry.sha256:
            raise StateConflict("revision source changed while cloning")
        files[entry.path] = content
    publication = parse_public_archive_document(files.get("publication/PUBLICATION.json", b""), "publication")
