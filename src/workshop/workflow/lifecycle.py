...
from workshop.errors import ContractError, ReceiptError, TransitionError
...
class InspectionPolicy:
    ...
    def __post_init__(self) -> None:
        if not self.gate_id or not self.evaluator:
            raise ContractError("inspection policy requires an id and evaluator")
        require_exact_version(self.evaluator_version, "inspection policy evaluator_version")
        require_sha256(self.config_sha256, "inspection policy config_sha256")
        if self.max_age_seconds is not None and (
            ...
            raise ContractError("inspection policy max_age_seconds must be a positive integer or null")
        if (
            ...
            raise ContractError(
                "inspection policy max_future_skew_seconds must be an integer from 0 to 3600"
            )


def _default_gate_policy(profile: str, gate_id: str) -> InspectionPolicy:
    evaluator = "autonomous-workshop.%s.%s" % (profile, gate_id)
    version = "1"
    freshness = {
        "novelty": 7 * 24 * 60 * 60,
    ...
    config = hashlib.sha256(
        (
            "%s-gate-policy-v1:%s:max-age=%s:future-skew=300"
            % (profile, gate_id, freshness)
        ).encode("utf-8")
    ).hexdigest()
    return InspectionPolicy(gate_id, evaluator, version, config, freshness)
...
class WorkflowSpec:
    ...
    gate_policies: Mapping[str, InspectionPolicy]

    def __post_init__(self) -> None:
        stage_set = set(self.stages)
        if not self.stages or len(stage_set) != len(self.stages):
            raise ContractError("workflow stages must be non-empty and unique")
        if self.initial_stage not in stage_set:
            raise ContractError("workflow initial_stage is not in stages")
        if set(self.edges) != stage_set:
            raise ContractError("workflow edges must define every stage")
    ...
    def board_game(cls) -> "WorkflowSpec":
        ...
        return cls(
            ...
            gate_policies={
                gate: _default_gate_policy("board-game", gate)
    ...
    def physical_product(cls) -> "WorkflowSpec":
        """Default lifecycle for printable products that are not board games."""
        ...
        forward = {
            "idea": {"researched"},
            "researched": {"designed"},
            "designed": {"built", "designed"},
            "built": {"validated", "built"},
            "validated": {"reviewed", "built", "designed"},
            "reviewed": {"draft", "built", "designed"},
            "draft": {"live", "killed"},
            "live": set(),
            "parked": set(stages) - {"parked", "live"},
            "killed": set(),
        ...
        for stage in stages:
            if stage not in ("live", "parked", "killed", "draft"):
                forward[stage] = set(forward[stage]) | {"parked", "killed"}
        required_gates = {
            "validated": ("cad", "printability", "safety"),
            "reviewed": ("cad", "printability", "safety", "form", "novelty"),
            "draft": ("cad", "printability", "safety", "form", "novelty"),
        }
        gate_ids = {gate for gates in required_gates.values() for gate in gates}
        return cls(
            initial_stage="idea",
            ...
            required_gates=required_gates,
            gate_policies={
                gate: _default_gate_policy("physical-product", gate)
                for gate in gate_ids
            },
        )
    ...
    def custom(cls) -> "WorkflowSpec":
        """Small product-neutral graph for inventors with their own domain gates."""
        ...
        forward = {
            "idea": {"developed", "parked", "killed"},
            "developed": {"validated", "developed", "parked", "killed"},
            "validated": {"draft", "developed", "parked", "killed"},
            "draft": {"live", "killed"},
            "live": set(),
            "parked": {"idea", "developed", "validated", "killed"},
            "killed": set(),
        }
        required_gates = {
            "validated": ("quality", "safety"),
            "draft": ("quality", "safety"),
        }
        gate_ids = {gate for gates in required_gates.values() for gate in gates}
        return cls(
            initial_stage="idea",
            ...
            required_gates=required_gates,
            gate_policies={
                gate: _default_gate_policy("custom", gate) for gate in gate_ids
            },
        )
...
class Pipeline:
    ...
    def legal_targets(
        self, stage: str, *, parked_from: Optional[str] = None
    ) -> Sequence[str]:
        """Return the public, sorted transition choices for a stage."""

        if stage not in self._edges:
            raise ContractError("unknown workflow stage %r" % stage)
        if stage == "parked" and parked_from is not None:
            if parked_from not in self._edges or parked_from in ("parked", "live"):
                raise ContractError("parked_from must name a resumable workflow stage")
            return tuple(sorted((parked_from, "killed")))
        return tuple(sorted(self._edges[stage]))
    ...
    def advance(
        ...
        for gate in gates:
            if not isinstance(gate, GateResult):
                raise TransitionError("gate results must use the Workshop GateResult contract")
            try:
                gate.assert_valid()
        ...
        if to_stage in ("draft", "live"):
            if (
                ...
                raise TransitionError(
                    "%s requires a durable publication intent, receipt, packet identity, "
            ...
            durable_raw = publication_intent.get("receipt")
            try:
                durable_receipt = PublicationReceipt.from_dict(durable_raw)
            except (TypeError, ValueError, ContractError, ReceiptError) as exc:
                raise TransitionError("durable outbox Stamp is malformed") from exc
            ...
            if to_stage == "live":
                ...
                prior_raw = prior_payload.get(
                    "stamp", prior_payload.get("publication_receipt")
                )
                prior_packet = prior_payload.get(
                    "pack_sha256",
                    prior_payload.get("publication_packet_sha256"),
                )
                prior_intent = prior_payload.get(
                    "send_intent_id",
                    prior_payload.get("publication_intent_id"),
                )
                if (
                ...
                try:
                    prior = PublicationReceipt.from_dict(prior_raw)
                except (TypeError, ValueError, ContractError, ReceiptError) as exc:
        ...
        if publication_packet_sha256:
            payload["pack_sha256"] = publication_packet_sha256
        if publication_intent_id:
            payload["send_intent_id"] = publication_intent_id
...
# Compatibility spelling used before Workshop 0.3.
GatePolicy = InspectionPolicy
PipelineSpec = WorkflowSpec
