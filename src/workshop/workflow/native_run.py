...
from workshop.workflow.reasoning_override import (
    REASONING_OVERRIDE_NAME, append_reasoning_override, reasoning_override_binding,
    validate_reasoning_override, validate_reasoning_selection,
)
...
# Legacy non-token runs retain the bounded rejection counter. Token-budget
# runs record the counter for diagnostics without using it as a spending cap.
...
def _persist_cad_gate_rejection(
    ...
    if (not _checkpoint_uses_token_budget(checkpoint)
            and rejection_number > _MAX_CAD_GATE_REJECTIONS):
        raise WorkshopError(
    ...
    if (
        ...
        or (
            ...
            and (
                ...
                or record["rejection_number"] < 1
                or (not _checkpoint_uses_token_budget(checkpoint)
                    and record["rejection_number"] > _MAX_CAD_GATE_REJECTIONS)
            )
        )
...
def _cad_gate_budget_outcome(
    ...
    if (_checkpoint_uses_token_budget(checkpoint)
            or _cad_gate_rejection_number(persisted) < _MAX_CAD_GATE_REJECTIONS):
        return checkpoint
...
def _token_budget_launcher(paths, checkpoint, launcher, budget, reasoning_override):
    ...
    if not supports_rollout_usage_version(result.cli_version):
        raise ContractError("token-budget rollout adapter requires Codex CLI 0.153.4 or newer")
...
def _unfinished_return_stop_armed(budget: Optional[Any]) -> bool:
    ...
    cost, so it has to read the same condition the loop enforces. Token-budget runs and legacy clock
    and turn-budget runs keep their unbounded continuation, and the
...
def _launcher_call(
    ...
    *,
    ...
    if unfinished_returns and not inventor_selection_boundary:
        prompt += (
    ...
    if reasoning_override is not None:
        prompt += (
            ...
            "reasoning effort for this and subsequent Manager and child work. "
            "This overrides older reasoning guidance; preserve the original "
            "%s selection as provenance in MANAGER.json. Continue the same session and Goal."
            % (reasoning_override["changes"][-1]["effort"],
