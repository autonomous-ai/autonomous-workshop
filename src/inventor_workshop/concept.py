...
No image provider ships with this repo. ``DefaultConcept`` owns the prompts, the
generation order, and the sealing; the pixels come from an injected
``concept_artist``. Without one the job waits truthfully rather than describing a
design it cannot actually draw.
"""
...
_DEFAULT_WALL_MM = 2.4
_DEFAULT_ENVELOPE_MM = (120.0, 120.0, 60.0)
_LANE_CATEGORIES = {
...
def derive_brief(context: ConceptContext) -> ConceptBrief:
    """Lock the design's facts from the Wish, the Taste, and the blueprint.

    Every fact the Wish did not determine is decided here and recorded in
    ``assumptions``; nothing invented is presented as if the Wish supplied it.
    A refining round starts from the standing brief so its numbers do not drift
    under feedback that never named a physical fact.
    """
    ...
    constraints = dict(context.wish.constraints)
    assumptions: list = []
    objective = " ".join(context.wish.objective.split())
    described = objective.split(".")[0].strip() or objective
    obj = described[:500]
    category = _LANE_CATEGORIES.get(context.blueprint.lane, context.blueprint.lane)

    envelope = _constraint_dimensions(constraints.get("envelope_mm"))
    if envelope is None:
        envelope = _DEFAULT_ENVELOPE_MM
        assumptions.append(
            "The Wish did not state an envelope; Concept decided %s."
            % _extent(envelope)
        )
    wall = constraints.get("wall_mm")
    if type(wall) not in (int, float):
        wall = _DEFAULT_WALL_MM
        assumptions.append(
            "The Wish did not state a wall thickness; Concept decided %s mm."
            % _mm(wall)
        )
    ...
    if not features:
        features = (
            "one signature interaction that exists because of this Wish: %s" % obj,
        )
        assumptions.append(
            "The Wish did not list distinctive features; Concept decided the "
            "signature interaction from its objective."
        )
    ...
    if not isinstance(orientation, str) or not orientation.strip():
        ...
        assumptions.append(
            "The Wish did not state a print orientation; Concept decided "
            "%s." % orientation
        )
    ...
    if supports is not True and supports is not False:
        supports = False
        assumptions.append(
            "The Wish did not state whether supports are used; Concept decided "
            "the design prints without them."
        )
    ...
    components = _components_from_constraints(constraints.get("components"))
    if not components:
        components = (
            ConceptComponent(
                "body",
        ...
        assumptions.append(
            "The Wish did not decide a part breakdown; Concept decided the "
            "design prints as one body."
        )
    assumptions.extend(_accumulated_edits(context))
    return ConceptBrief(
        obj,
        ...
        wall,
        features,
        {"orientation": orientation, "supports": supports},
        components,
        fits,
        tuple(assumptions),
    )
...
class DefaultConcept:
    ...
    def __call__(self, context: ConceptContext) -> ConceptImages:
        ...
        self._write_descriptor(root, brief, overall, components, context.round)
        ...
        return ConceptImages.from_root(
            root, brief, overall, components, context.round
        )
