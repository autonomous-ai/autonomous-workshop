...
from .runtime import _perform, _reconcile
...
class Sender(_ShopSender):
    ...
    def send(
        ...
        return _perform(self.store, product_id, packed, door, request, lease_token)

    def reconcile(self, send_intent_id: str, door: Any) -> Optional[SendResult]:
        """Compatibility spelling for ``Runtime.reconcile``."""

        return _reconcile(self.store, send_intent_id, door)
