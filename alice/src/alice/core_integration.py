"""Fail-closed bridge from Alice's release packet to shared inventor core.
...
class CoreIntegrationError(ValueError):
    """Alice's publication packet could not satisfy the shared core contract."""
...
def build_core_packet_binding(
    ...
    try:
        ...
        raise CoreIntegrationError(
            "shared core rejected Alice's publication packet: %s" % exc
        ) from exc
    ...
    if (
        ...
        raise CoreIntegrationError(
            "shared core packet identity is inconsistent with Alice's packet"
        )
...
def validate_core_packet_binding(
    ...
    *,
    ...
    """Rebuild core's deterministic packet and compare the complete binding."""
