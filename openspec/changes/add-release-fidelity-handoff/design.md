...
- `read_step_part_colors` applies the sRGB transfer to build123d channels (cadgen documents `Color` as linear). The cad skill's `export --glb` writes `baseColorFactor` as if the channels were sRGB, so Quarterhoot's owl is `#eabd76` in STEP terms and `#d1822e` in the GLB.
...
### 2. One colour convention: cadgen linear, STEP reader unchanged, GLB exporter fixed

Keep cadgen's documented convention and `read_step_part_colors`. Fix `export --glb` to write `baseColorFactor` from the raw channels (glTF stores linear), so the viewer, host renders, and the shop's `part_colors` agree. The CAD skill instructs `part.color = srgb("#hex")` and the layout check warns when a leaf occurrence of a multi-part project has no colour. `_part_color_plan` needs no change: with Decision 1 the Factory's `mesh_name` equals the STEP occurrence name.
