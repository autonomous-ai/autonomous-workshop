...
The required deterministic E2E proves Workshop host mechanics, but it cannot prove that an authenticated Codex session can discover and correctly use the materialized product-run constitution, current `STAGE.json`, accepted upstream artifacts, and normal finalizer. The useful mock-session work on `feat/concept-phase` targets that gap, but its standalone Match, active Concept, private Deliver, and single fixed route no longer match the effort-aware, terminal-Release workflow on `main`.

## What Changes

- Add an opt-in real-Codex mock-session acceptance tier that starts one authenticated native session per Wish and resumes that exact session through every enabled stage.
...
- `workshop/effort-aware-codex-mock-session-e2e`: Defines the opt-in real-Codex acceptance contract for persistent-session context discovery, minimal finalization, effort-route coverage, production host gates, local Factory transport, bounded diagnostics, and narrow evidence claims.
...
- Adds no production mock mode, alternate Python agent framework, Concept activation, external provider call, or change to lifecycle contracts.
