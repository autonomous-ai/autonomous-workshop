...
This prohibition SHALL hold at the level of bytes, not merely of types. It is not enough that a concept record cannot be passed where an Instructions media mapping is expected — the pixels themselves SHALL NOT reappear as product proof, however they were copied there. The closer Make comes to building what the concept shows, the more plausible the substitution becomes and the more it would conceal: a divergence between what was designed and what was actually made is exactly what product images exist to reveal.

#### Scenario: A concept record cannot be passed off as product media

- **WHEN** a concept image set is offered where Instructions requires its product images
- **THEN** it is rejected, because the two contracts are distinct records

#### Scenario: Concept pixels cannot reappear as product proof

- **WHEN** a file offered as an Instructions product image has the same bytes as any image in the concept the product was built from
...
- **AND** the product's images must still depict the artifact that was actually produced
