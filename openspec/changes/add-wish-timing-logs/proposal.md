...
`workshop wish` now reports privacy-safe native activity, but an operator still cannot tell when a host lifecycle operation began or how long it took. Timestamped operation boundaries and elapsed durations are needed to distinguish slow workspace setup, native turns, proposal verification, gates, and optional publication without inspecting private run state.
...
- Cover new-run initialization and each lifecycle iteration: stage preparation, native-session start/resume, proposal processing, deterministic gate evaluation, and optional Factory publication when attempted.
