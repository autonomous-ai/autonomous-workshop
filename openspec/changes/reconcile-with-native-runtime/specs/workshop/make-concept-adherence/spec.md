...
**Reason**: The type it names no longer exists. Make receives its inputs as a read-only stage packet and returns a sealed, content-addressed result; there is no constructed context object to carry an optional field, and Concept is now a mandatory stage, so the requirement's central allowance — that a Make round without a concept remains valid — is no longer true of any round.

**Migration**: Replaced by "The Make stage packet carries the round's concept", which keeps the stale-concept and wrong-round refusals, binds the concept into the sealed Make result as `concept_sha256`, and refuses a Make round that has no concept at all.

### Requirement: Make builds to the concept when one is present

**Reason**: "When one is present" is no longer a real condition. Every Make round after this change has a standing concept, so a requirement conditioned on the concept's presence describes a state the lifecycle cannot reach.

**Migration**: Replaced by "Make builds to the concept", which carries the form and proportion rule, the brief's-numbers-outrank-the-pictures rule, the component-breakdown rule, and the cannot-realize case forward unconditionally.
