...
**Reason**: The requirement existed because every component image was drawn from the exploded view, so an exploded view that omitted a part produced a component image of a shape nobody specified. Component images are now drawn from each component's own brief specification, with references supplying appearance only, so no image inherits shape from the exploded view and an omission in it corrupts nothing downstream. The check had also become uncheckable: deciding whether a drawing depicts N separated parts requires reading the picture, which the host is forbidden to do and which a second model grading the first was the only way to fake.
**Migration**: The guarantee that mattered — that a component is drawn to the shape the design decided — is now carried by `workshop/concept-images`, "Every image in a concept depicts one same design", which forbids a component instruction from asking for its shape to be reproduced from any reference. The exploded view keeps its own completeness obligation under "A concept provides the overall views, an exploded view, and one view per component", where it is checked as a property of the authored instruction rather than of the pixels, because the view is still the set's statement of the assembly and Make's map of the part breakdown. That the built part set matches the designed part set remains enforced in bytes at the Make boundary by `workshop/make-concept-adherence`.
