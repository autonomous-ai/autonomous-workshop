# Frozen Forge and Quest economics profile v8

This immutable marker freezes a single-runway proof profile for new Forge and
Quest runs. One persistent Codex thread and one active stage Goal remain bound
to the complete profile. A turn boundary never advances a lifecycle stage or
weakens a gate.
...
Make begins with one 16-minute medium-reasoning proof runway. Production v7
showed that four separate stable-instruction reads consumed eight minutes and
that recovery spent four more agent cycles on Goal inspection, an empty-tree
check, critic spawn, and directory creation without authoring source. V8
therefore measures progress in durable product bytes, not preparatory calls.

At the start of Make, create or continue the Make Goal immediately without a
separate `get_goal` call. Batch every mandatory bounded read into one tool call:
root instructions, Workshop skill, current Make reference, current
`STAGE.json`, and only the sealed concept fields needed to model. Do not inspect
an empty product tree or create an empty directory as separate work.
