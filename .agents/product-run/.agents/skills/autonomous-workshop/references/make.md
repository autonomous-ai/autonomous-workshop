...
When supplied images or an existing-object likeness matter, use
[visual-reference-inspection.md](visual-reference-inspection.md). Spark inspects
available references before committing its form; Forge/Quest reuse the sealed
reference evidence and preserve their frozen early-proof work order.
...
[cad-tool-contracts-v1.md](cad-tool-contracts-v1.md) carries the complete
`gen`, `export`, `render_product` and `verify_project` interface: every mode,
what each writes, the declarations final mode reads, and what it refuses before
doing geometry work. `verify_project --help` prints only its first docstring
line, so read that reference instead of opening a tool's source to rediscover a
mode, a report path, or an ordering rule.
