...
Alice, Bob, and Eve all execute shared Foundation on normal product paths. This is a
deliberate incremental migration: each inventor keeps its characterized
creative state machine while Foundation owns infrastructure at a narrow, tested
boundary. It avoids a dual-written lifecycle while still removing the most
dangerous publication and artifact duplication.

| Inventor | Executed Foundation boundary | Inventor-local authority retained |
|---|---|---|
| Alice | `publish.packet` builds a Foundation artifact manifest and canonical packet identity; the Vibe adapter reconstructs and verifies the complete binding before an effect. The always-on service explicitly hashes the clean tracked `foundation/src/inventor_core` checkout and seals those bytes with Alice's code. | Alice's stronger SQLite leases, evidence graph, release decision, effect reconciliation, and production-manifest hash. |
...
`foundation/src` path, so an unrelated same-version global install cannot satisfy a
...
first selected artifact. Corrected bytes require a new slug until Foundation has an
...
Alice intentionally does not replace its mature store with the younger Foundation
store. Its release bridge independently verifies Foundation's deterministic artifact
...
hashes the explicit mutable Foundation checkout, while the same `inventor_core` files
are copied into the owner-only execution snapshot used by isolated children.

## Compatibility contract

CI installs the same local `foundation/` package before testing every native inventor
and exercises these runtime ranges:

- Foundation: Python 3.9 and 3.13;
...
python3 foundation/tools/verify_snapshot_locks.py
PYTHONPATH=foundation/src python3 -m inventor_core registry --root inventors --check-entrypoints
PYTHONPATH=foundation/src python3 -m unittest discover -s foundation/tests -p 'test_*.py'
```
...
artifact revision, followed by Foundation lifecycle/lease/budget adapters backed by
...
unless Foundation first proves equivalent behavior.
