...
The inventor Foundation should own the parts that must be identical for every inventor: artifact identity and packaging, deterministic acceptance gates, bounded repair, resumable job state, publication receipts, and narrow provider ports. It should not absorb Panda, Factory, Vibe, ecommerce, print-farm, media-provider, or model-provider implementations. Those remain independently deployed systems behind adapters.

The highest-value donors are:

- [`autonomous-vibe`](https://github.com/autonomous-ai/autonomous-vibe/tree/fcaacde5eb3bd97f01aed4ab7b81a09faa9dc81d) for CAD generation, validation, print preparation, and the Panda client contract.
...
- [`autonomous-harness`](https://github.com/autonomous-ai/autonomous-harness/tree/f06febd6ea180eedd4d57507bd801a6833745619) for an agent-engine protocol that does not bind Foundation to one CLI or model vendor.

Panda is the publication/catalog boundary, not a code dependency. Factory fulfillment is further downstream and its backend source was not present in the audited organization.
...
For the 15 repositories with a plausible Foundation or integration role, the audit inspected documentation and implementation at a pinned default-branch commit rather than relying on repository descriptions. The decision labels used below are:

- **Vendor/extract**: bring a small, generic contract or implementation into Foundation with provenance and tests; do not copy the enclosing application.
- **Adapter**: define a Foundation-owned port and integrate over a versioned network or process boundary.
- **Depend**: consume a separately versioned artifact at an explicit pin.
- **Reference**: use the implementation to understand live behavior, but do not couple Foundation to it.
- **Ignore**: do not use it for the inventor Foundation.

## Target boundary

```text
inventors/<name>
    niche policy + prompts + domain generators + domain evaluators
                          |
                          v
Foundation
    lifecycle | artifact bundle/hash | gates | repair budget | receipts | outbox
                          |
          +---------------+----------------+----------------+
          v               v                v                v
      AgentPort        CadPort       PublisherPort    FulfillmentPort
   Harness / Grid    cadpy/tools        Panda API       Store/Factory
```

Foundation should be usable without live credentials. Deterministic mock providers and evaluators must exercise the complete lifecycle before an inventor is allowed to invoke paid generation, publish a design, or request fulfillment. Inventors may add stricter gates, but they must not bypass the common artifact, safety, identity, or publication contracts.

The Foundation artifact boundary should be a content-addressed bundle containing source, manufacturing outputs, previews, a tree manifest, provenance, validation evidence, and an acceptance receipt. Remote identifiers are annotations on that immutable local identity; a mutable Panda slug, database record, or model-authored success message is not the artifact identity.

## Pinned integration and donor map

| Repository | Inspected commit | Main surface / language | Foundation decision |
|---|---|---|---|
| `autonomous-vibe` | [`fcaacde`](https://github.com/autonomous-ai/autonomous-vibe/tree/fcaacde5eb3bd97f01aed4ab7b81a09faa9dc81d) | CAD packages and desktop integrations; Python, TypeScript/JavaScript, Rust | **Vendor/extract** CAD contracts and canonical skills; **adapter** for slicing, printing, and Panda |
| `panda-social-backend` | [`8f122be`](https://github.com/autonomous-ai/panda-social-backend/tree/8f122beb489b5bafe14ac7aa60165dadcbb9d125) | Panda API and catalog; Go | **Adapter** over HTTP/OpenAPI; never share its Mongo models |
...
| `media-gen-gateway` | [`a42ee97`](https://github.com/autonomous-ai/media-gen-gateway/tree/a42ee9719bb8c56674830edbcc885f3fa2521917) | Async media gateway; Python/FastAPI | **Adapter, experimental** after auth and correctness hardening |
| `autonomous-harness` | [`f06febd`](https://github.com/autonomous-ai/autonomous-harness/tree/f06febd6ea180eedd4d57507bd801a6833745619) | Agent protocol and CLI; TypeScript | **Vendor/depend** on schemas and conformance; one Foundation adapter |
...
| `autonomous-grid` | [`8964c40`](https://github.com/autonomous-ai/autonomous-grid/tree/8964c404ec336017a5d7becfcb446f42cdde21b5) | OpenAI-compatible local/remote inference; Python | **Adapter** as an optional model/media provider |
| `autonomous-grid-be` | [`77130ef`](https://github.com/autonomous-ai/autonomous-grid-be/tree/77130ef6b3cebf30721803fc96749080db17ef5a) | Grid auth/control plane; Python/FastAPI | **Ignore** in Foundation; deployment concern only |
...
### Panda: publication, catalog, and identity adapter
...
[`docs/worker-contract.md`](https://github.com/autonomous-ai/panda-social-backend/blob/8f122beb489b5bafe14ac7aa60165dadcbb9d125/docs/worker-contract.md) defines the important storage contract: a content-addressed, immutable `project_url` tree, complete source in object/CDN storage, `_tree.json`, and explicit API-versus-worker field ownership. Foundation should generate the same sort of immutable local bundle and treat Panda's URL as a remote publication receipt.

The current import implementation in [`services/import.go`](https://github.com/autonomous-ai/panda-social-backend/blob/8f122beb489b5bafe14ac7aa60165dadcbb9d125/services/import.go) is synchronous, non-idempotent, and defaults to public unless the client requests `status=draft`. It de-gits the upload and writes an immutable CDN snapshot. At this pin its relevant limits are 100 MiB uploaded, 512 MiB expanded, 4,096 entries, 256 MiB per expanded archive entry, and 95 MiB per published file. Every inventor must therefore import as a draft, fence retries locally by artifact hash, and reconcile an ambiguous response instead of blindly retrying.

Publishing is a JSON request whose media/attachments must already be hosted URLs; the current limit is 12 attachments. [`apis/handlers_listing.go`](https://github.com/autonomous-ai/panda-social-backend/blob/8f122beb489b5bafe14ac7aa60165dadcbb9d125/apis/handlers_listing.go) mints an immutable SKU and computes the current price floor from sliced weight, 30% waste, material cost, and shipping. Foundation should send creator intent and retain the authenticated response, not reproduce Panda's pricing logic.

The recommended autonomous sequence is:

1. Generate with the canonical CAD skill and a domain-specific inventor.
2. Run deterministic geometry, artifact, safety, and printability gates; optionally slice or test-print locally.
3. Freeze a content-hashed artifact bundle and an acceptance receipt.
4. `POST /designs/import` with `status=draft`; persist the returned owner, slug, history IDs, status, and `project_url` against the exact bundle hash.
...
8. Let Panda mint the SKU; send commerce and fulfillment through the external Store/Factory boundary.

Foundation must not import Panda's Go models or access its Mongo collections. API and worker field ownership are deliberate, and direct database sharing would make every inventor sensitive to Panda migrations.
...
| Publish request | [`panda-website/src/services/api/designs.ts`](https://github.com/autonomous-ai/panda-website/blob/fdb84d0698cb2b1461a74c5ae88155fb36ddd2bf/src/services/api/designs.ts) sends multipart publish media; current backend Swagger and [`ecm-website/social/api.ts`](https://github.com/autonomous-ai/ecm-website/blob/3c3fd22a8c1b07cd96c4489cfc9bf6a9ac78d879/src/views/Vibe2Page/social/api.ts) use JSON with previously uploaded URLs. | Foundation publisher uses JSON URLs and cross-repo contract tests. |
| Worker documentation | [`panda-social-cc-agent/docs/generation-worker.md`](https://github.com/autonomous-ai/panda-social-cc-agent/blob/70113cd986f1b5f1dfb645673d63169617a4ae2a/docs/generation-worker.md) contains an obsolete banner, while the worker/job/artifact modules are active production code. | Extract behavior from code/tests and write new Foundation-native state-machine documentation. |
| Hosted authentication | [`autonomous-vibe/social.rs`](https://github.com/autonomous-ai/autonomous-vibe/blob/fcaacde5eb3bd97f01aed4ab7b81a09faa9dc81d/desktop/src-tauri/src/commands/social.rs) contains a hosted login URL that no longer matches the current backend identity surface. | Keep login outside Foundation; require an injected credential provider and test token refresh/exchange against a versioned environment. |
| Agent gateway | [`panda-social-backend/docs/agent-gateway.md`](https://github.com/autonomous-ai/panda-social-backend/blob/8f122beb489b5bafe14ac7aa60165dadcbb9d125/docs/agent-gateway.md) describes a dormant, not-deployed gateway, and `/agent-creator` represents one server-configured identity rather than many inventor principals. | Use Harness/`AgentPort` for execution and design explicit service identity before unattended publishing. |
...
These are platform gaps or Foundation obligations, not reasons to couple the inventor implementation to an application repository.

1. **Per-inventor service identity.** The inspected Panda flows are human OAuth/PKCE/refresh, import ownership comes from the bearer token, and the dormant agent route maps to one configured account. Define a distinct principal for every inventor, scoped credentials, revocation/rotation, and explicit creator attribution. A human token shared by ten unattended agents is not an acceptable boundary.
2. **End-to-end idempotency.** Panda import is non-idempotent. Use artifact-tree hash plus a durable local outbox/intent, never blind-retry an unknown response, and add platform support for `Idempotency-Key` or artifact-hash lookup. Publishing and media upload need the same treatment.
...
4. **Verified acceptance receipts.** Every gate result must name the evaluator, record evidence, and bind to the exact artifact bytes. Publish only the accepted history and prove it with authenticated readback; never trust an HTTP success alone or an LLM's self-evaluation.
5. **One canonical skill distribution path.** CAD skills are copied across Vibe and worker repositories. Foundation needs a versioned source, compatibility metadata, fixtures, and a release/update mechanism so ten inventors do not fork printability rules.
6. **Isolation and secret hygiene.** Give each run an isolated workspace and each inventor least-privilege secrets. Fail packaging when it encounters environment files, tokens, private keys, VCS internals, unsafe links, traversal, or disallowed binaries; do not rely solely on Panda's server-side stripping.
7. **Normalized lifecycle and recovery.** Foundation needs leases, heartbeats, compare-and-set transitions, cancellation, checkpoints, input/review pauses, bounded repair budgets, and crash recovery. Map provider-specific status explosions into a small common lifecycle with a structured reason.
8. **Fulfillment integration contract.** The Store/Factory backend was unavailable in the 64-repository inventory. Before inventors can autonomously sell physical goods, document quote, material/profile, manufacturability, order, refund, shipment, webhook-authentication, and sandbox contracts with the owning team. Until then, stop Foundation automation at a verified Panda listing/SKU.
9. **Provider cost and determinism controls.** Supply deterministic model/media/CAD mocks, cache stages by complete input hashes, preflight before paid calls, record cost/latency/provenance, and cap repair attempts. A retry must resume the failed stage rather than regenerate an entire product.
...
These are the only organization repositories for which the audit found a plausible reusable Foundation primitive or a direct integration boundary. The detailed decision for each is above.

### Current consolidation target — 1

[`inventors`](https://github.com/autonomous-ai/inventors).

This repository is the destination for shared Foundation plus one `inventors/<id>/`
...
### Adjacent agent/product platforms, not required by inventor Foundation — 22
...
These overlap with coding agents, shells, model routing, office creation, or agent UI. Foundation should reach such systems only through `AgentPort`/provider adapters; importing their application frameworks would widen the dependency surface without improving the physical-product lifecycle.
...
These are device backends, firmware, dashboards, apps, metrics, or organization administration. Ignore them for the inventor Foundation. Revisit a specific repository only when an inventor gains a direct device-control requirement.
...
- Repository state can move after this snapshot. Update pins and rerun drift checks before adopting donor code or changing a production adapter.
