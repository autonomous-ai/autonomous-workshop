...
They use Foundation for artifacts and/or publication without claiming the complete
definition below; creative lifecycle, lease, and budget migrations remain
fixture-gated follow-up work.

## Definition of Foundation-connected

An inventor is Foundation-connected when it has:
...
5. immutable active budget policies, Foundation-clock windows, per-spend policy/result
...
7. `GatePolicy`-pinned evaluator/version/config/freshness plus a hash-bound
   ...
   plus physical evidence for critical claims, typed semantics for all ten Foundation
...
Foundation connectivity does not yet mean unattended Panda publication is safe. An
...
- Keep `foundation/snapshots.lock.json` synchronized only after intentional review;
...
- Complete: Bob's byte-identical CAD and STEP-parts copies now live in
  `foundation/skills`; Bob uses compatibility symlinks.
...
- Port Alice's store/effect/reconciliation fixtures and Bob's legal-edge/fenced-
  lease fixtures into Foundation (the private transition primitive, active-lease
...
- Move Eve first to Foundation state because its current queue is ignored/unfenced;
...
- Pin the exact validator set, version, and configuration SHA-256. Preserve at
  least the Foundation `manifest`, `brep`, `mesh-topology`, `dimensions`,
...
- Port results into the typed measurement contract for all ten Foundation checks.
...
- Replace direct publishers with the Foundation Panda coordinator.
...
| Inventor | Keep local | Move/reuse from Foundation first |
|---|---|---|
| Alice | research laboratory, reward/release policy, cited corpus | align its mature store/effects/CAD receipts with Foundation APIs; keep stronger guarantees |
