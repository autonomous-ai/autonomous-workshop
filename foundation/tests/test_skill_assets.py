...
from inventor_core.models import require_exact_version, require_safe_evidence_path
...
class ProductToCadAssetTest(unittest.TestCase):
    def test_cad_project_example_matches_core_contract(self):
    ...
    def test_validator_policy_example_covers_all_core_checks(self):
        raw = json.loads(
        ...
        self.assertEqual(covered, set(CORE_CHECKS))
        self.assertEqual(
            {item.substrate for item in requirements},
            {"deterministic", "independent-review", "physical"},
        )

    def test_receipt_schema_tracks_core_measurement_contract(self):
        schema = json.loads(
        ...
        self.assertEqual(set(required_by_check), set(CORE_CHECKS))
        self.assertEqual(
            required_by_check,
            {
                check_id: set(measurements)
                for check_id, measurements in CORE_CHECK_MEASUREMENTS.items()
            },
        )

    def test_receipt_schema_types_partial_core_measurements(self):
        schema = json.loads(
            (SCHEMAS / "verification-receipt.schema.json").read_text(
                encoding="utf-8"
            )
        ...
        self.assertEqual(set(typed_refs), set(CORE_CHECKS))
        for check_id, runtime_rules in CORE_CHECK_MEASUREMENTS.items():
