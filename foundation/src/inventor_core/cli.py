...
def _new_inventor(args: argparse.Namespace) -> int:
    destination = scaffold_inventor(
        inventor_collection(args.root), args.inventor_id, args.name, args.niche
    )
...
def parser() -> argparse.ArgumentParser:
    root = Path.cwd()
    command = argparse.ArgumentParser(prog="inventor-foundation")
    ...
    new = subcommands.add_parser(
        "new", help="scaffold a Foundation-connected inventor"
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    try:
        ...
        print("inventor-foundation: %s" % exc, file=sys.stderr)
        return 2
