...
def _normalize_import_metadata(
    ...
    if normalized["status"] != "draft":
        raise ContractError("Foundation import always requires status=draft")
...
def _validate_packet_bytes(content: bytes) -> Tuple[bytes, str, str]:
    """Verify immutable bytes as one exact canonical Foundation publish packet."""
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            ...
            if names.count("_inventor-artifact.json") != 1:
                raise ContractError("publish packet needs exactly one Foundation artifact manifest")
            ...
            if any(
                ...
                raise ContractError("publish packet members are not in canonical Foundation form")
            ...
            if canonical_buffer.getvalue() != content:
                raise ContractError(
                    "publish packet bytes are not the exact canonical Foundation ZIP"
                )
    except (OSError, UnicodeDecodeError, ValueError, zipfile.BadZipFile) as exc:
        raise ContractError("publish packet is not a valid Foundation zip: %s" % exc) from exc
    return content, packet_sha256, artifact_sha256


def _load_packet(packet: Path) -> Tuple[bytes, str, str]:
    """Read once, verify the Foundation manifest, and identify the exact upload bytes."""
