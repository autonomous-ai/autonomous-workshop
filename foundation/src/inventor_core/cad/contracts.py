...
CORE_REQUIRED_CHECKS = frozenset(
    (
        "manifest",
        "brep",
        "mesh-topology",
...
CORE_CHECK_SUBSTRATES = MappingProxyType({
...
CORE_CHECKS = frozenset(CORE_CHECK_SUBSTRATES)

# These are deliberately small, engine-neutral release floors. Validators may
# emit additional project-specific measurements, but a Foundation check cannot pass
...
CORE_CHECK_MEASUREMENTS = MappingProxyType(
    {
        ...
        for check_id, measurements in _CORE_CHECK_MEASUREMENTS.items()
    }
)
SUBSTRATES = frozenset(CORE_CHECK_SUBSTRATES.values())
ZERO_SHA256 = "0" * 64
...
def _validate_core_measurements(
    check_id: str, status: str, measurements: Mapping[str, Any]
) -> None:
    """Validate typed Foundation metrics and the release floor for passed checks."""
    ...
    rules = CORE_CHECK_MEASUREMENTS.get(check_id)
    if rules is None:
        return
    for name, rule in rules.items():
        ...
        if kind == "boolean":
            ...
            raise RuntimeError("unknown Foundation CAD measurement type %r" % kind)
        if not valid_type:
            raise ContractError(
...
class VerificationCheck:
    ...
    def assert_valid(self) -> None:
        ...
        _validate_core_measurements(self.check_id, self.status, self.measurements)
...
def assert_release_ready(
    ...
    minimum_checks = set(CORE_REQUIRED_CHECKS)
    ...
    if missing_floor:
        raise TransitionError("CAD validator policy waives Foundation checks: %s" % sorted(missing_floor))
    ...
    for receipt in receipts:
        ...
        for check in receipt.checks:
            ...
            expected_substrate = CORE_CHECK_SUBSTRATES.get(check.check_id)
