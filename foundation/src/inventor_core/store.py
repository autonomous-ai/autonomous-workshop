...
class InventorStore:
            ...
            if meta_exists is not None:
                ...
                if version is not None:
                    ...
                    if observed_version not in (1, 2, SCHEMA_VERSION):
                        raise ContractError(
                            "state database schema version %s is unsupported; Foundation requires %s"
    ...
    def configure_budget(
        ...
        now = self._utc_boundary(utc_now(), "Foundation clock")
        with self._transaction() as connection:
            existing = connection.execute(
    ...
    def spend(
        ...
        # use the Foundation clock, never a timestamp supplied by an inventor.
        now = self._utc_boundary(utc_now(), "Foundation clock")
        with self._transaction() as connection:
            existing = connection.execute(
    ...
    def latest_publish_intent(
        ...
        without querying Foundation's private schema.  Ordering includes the random
