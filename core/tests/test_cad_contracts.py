...
from inventor_core.errors import ContractError, TransitionError
...
SUBSTRATE_CHECKS = {
    "deterministic": tuple(
        sorted(
            name
            for name in CORE_REQUIRED_CHECKS
            if name
...
class CadContractTest(unittest.TestCase):
    ...
    def test_all_ten_core_checks_have_typed_release_floors(self):
        self.assertEqual(set(CORE_CHECK_MEASUREMENTS), set(CORE_CHECKS))
        self.assertEqual(set(PASSING_MEASUREMENTS), set(CORE_CHECKS))
        for check_id in sorted(CORE_CHECKS):
            VerificationCheck(
                check_id,
                "passed",
    ...
    def test_core_pass_thresholds_cannot_be_replaced_by_truthy_claims(self):
    ...
    def test_nonpassing_core_checks_may_record_partial_valid_measurements(self):
        VerificationCheck(
