...
# emit additional project-specific measurements, but a core check cannot pass
...
def _validate_core_measurements(
    ...
    """Validate typed core metrics and the release floor for passed checks."""
    ...
    for name, rule in rules.items():
        ...
        if kind == "boolean":
            ...
            raise RuntimeError("unknown core CAD measurement type %r" % kind)
        if not valid_type:
            raise ContractError(
...
def assert_release_ready(
    ...
    if missing_floor:
        raise TransitionError("CAD validator policy waives core checks: %s" % sorted(missing_floor))
