"""Content-addressed artifact manifests and deterministic publish packets."""
...
DEFAULT_EXCLUDED_PREFIXES = (".env", "auth.", "credential.", "credentials.", "secrets.", "token.")
MAX_ENTRIES = 4096
...
class _PacketStaging:
    ...
    @classmethod
    def create(cls, parent: Path, destination_name: str) -> "_PacketStaging":
        ...
        if _ANCHORED_STAGING:
            ...
            try:
                ...
                if (opened_parent.st_dev, opened_parent.st_ino) != (
                    ...
                    raise ArtifactError(
                        "publish packet destination parent was replaced"
                    )
                for _ in range(32):
                    candidate = ".%s.stage-%s" % (
                        destination_name,
                ...
                if staging_name is None:
                    raise ArtifactError(
                        "cannot allocate private publish packet staging"
                    )
                staging_fd = os.open(
                    staging_name, directory_flags, dir_fd=parent_fd
                )
                os.fchmod(staging_fd, 0o700)
                member_name = "packet.tmp"
                flags = (
                    os.O_RDWR
                    | os.O_CREAT
                    | os.O_EXCL
        ...
        if (current_parent.st_dev, current_parent.st_ino) != (
            ...
            try:
                ...
                pass
            raise ArtifactError("publish packet destination parent was replaced")
        return cls(
            fd=fd,
            identity=identity,
            parent=parent,
    ...
    def assert_named(self) -> None:
        ...
        if (
            ...
            raise ArtifactError("publish packet temporary path was replaced")
    ...
    def publish(self, destination: Path) -> None:
        ...
        if (
            ...
            raise ArtifactError("publish packet destination was replaced")
...
def assert_publishable_content(relative_path: str, content: bytes) -> None:
    """Apply the final packet credential policy to one named byte payload.

    Both the builder and the Panda sender call this function.  That prevents a
    ...
    if (
        ...
        raise ArtifactError("publishable content requires a path and bytes")
...
def _source_files(root: Path, extra_excludes: Iterable[str] = ()) -> List[Tuple[Path, Path]]:
    ...
    for directory, dirnames, filenames in os.walk(str(root), followlinks=False):
        ...
        for dirname in sorted(dirnames):
            ...
            if dirname in DEFAULT_EXCLUDED_DIRS or _excluded(relative, excluded):
                continue
    ...
    if not result:
        raise ArtifactError("artifact has no publishable files")
...
def build_publish_packet(
    ...
    fixed, so identical input bytes produce identical packet bytes on every
    inventor machine.
    """
    ...
    if requested_destination.name in ("", ".", ".."):
        raise ArtifactError("publish packet destination must name one file")
    if requested_destination.is_symlink():
        raise ArtifactError("publish packet destination must not be a symlink")
    ...
    try:
        ...
        raise ArtifactError("publish packet destination must be outside the artifact root")
    if not isinstance(maximum_bytes, int) or isinstance(maximum_bytes, bool) or maximum_bytes <= 0:
        raise ArtifactError("publish packet limit must be a positive integer")
    ...
    staging = _PacketStaging.create(destination.parent, destination.name)
    fd = staging.fd
    ...
    try:
        ...
        # The backend expands the archive anyway; exact cross-machine packet
        ...
        with os.fdopen(fd, "w+b", closefd=False) as handle:
            with zipfile.ZipFile(
                ...
                for relative, absolute in files:
                    ...
                    assert_publishable_content(relative.as_posix(), content)
        ...
        if (
            ...
            raise ArtifactError("publish packet temporary file identity changed")
        size = completed.st_size
        if size > maximum_bytes:
            raise ArtifactError(
                "publish packet is %d bytes; configured limit is %d" % (size, maximum_bytes)
            )
        digest = hashlib.sha256()
