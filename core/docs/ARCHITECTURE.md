...
| Inventor owns | Core owns | Platform adapter owns |
|---|---|---|
...
Core never imports an inventor. An inventor may add stages or stricter gates,
but cannot waive core artifact, identity, safety, spend, or publication rules.
...
lease-window decisions use core's clock, never an inventor-supplied timestamp.
...
access control or signed remote receipts. Runtime directories created by core
...
3. The core Panda receipt identifies both packet and embedded artifact hashes,
...
Core caps a source artifact at 4,096 entries, each expanded member at 95 MiB,
...
Panda import is synchronous and non-idempotent. Core first proves that the
...
Core's Panda reconciler therefore fails closed and never reissues that POST.
...
Core supports build123d, CadQuery/cadpy, and future engines through one manifest:
...
SHA-256, substrate, and named checks. It cannot waive core's `manifest`, `brep`,
...
physical substrate. A core check appearing under the wrong substrate blocks
...
All ten core checks define typed minimum/pass semantics, including exact
...
New scaffolds target Python 3.11+ even though the dependency-free core package
