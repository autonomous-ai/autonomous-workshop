...
class StoreTest(unittest.TestCase):
    ...
    def publish_request(self, **metadata):
        return {
            "status": "draft",
            "_workshop_artifact_sha256": SHA,
            "_workshop_owner_id": "alice-owner",
    ...
    def test_future_schema_is_rejected_before_workshop_mutates_it(self):
    ...
    def test_supported_legacy_meta_table_is_renamed_in_place(self):
        legacy = Path(self.temp.name) / "legacy.sqlite"
        with closing(sqlite3.connect(str(legacy))) as connection:
            connection.execute(
                "CREATE TABLE core_meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)"
            )
            connection.execute(
                "INSERT INTO core_meta(key, value) VALUES ('schema_version', '2')"
            )
            connection.commit()
        InventorStore(legacy)
        with closing(sqlite3.connect(str(legacy))) as connection:
            tables = {
                row[0]
                for row in connection.execute(
            ...
            version = connection.execute(
                "SELECT value FROM workshop_meta WHERE key='schema_version'"
            ).fetchone()[0]
        self.assertIn("workshop_meta", tables)
        self.assertNotIn("core_meta", tables)
    ...
    def test_budget_policy_and_idempotent_result_are_immutable(self):
        ...
        with mock.patch(
            "inventor_workshop.store.utc_now",
    ...
    def test_schema_v1_launch_request_replays_under_workshop_names(self):
        legacy = {
            "status": "draft",
        ...
        intent = self.store.prepare_publish("game", SHA, legacy)
        replay = self.store.prepare_publish("game", SHA, self.publish_request())
        self.assertEqual(replay["id"], intent["id"])
        conflicting = dict(
            self.publish_request(),
            _core_owner_id="different-owner",
        )
        with self.assertRaisesRegex(ContractError, "conflicting"):
            self.store.prepare_publish("game", "b" * 64, conflicting)
