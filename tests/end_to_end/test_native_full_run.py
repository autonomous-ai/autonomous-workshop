...
class _OneSessionProductAgent:
    ...
    def _author_make(self, run_root, stage):
        ...
        for group in (
            invented["concept"]["build_plan"]
            if invented.get("schema_version", 5) >= 5
            else ()
        ):
...
class _FactoryEffects:
    ...
    def credentials(self, unused_inventor_id=None):
...
class NativeFullRunTest(unittest.TestCase):
    ...
    def test_quest_make_can_return_unbuildable_concept_to_invent(self):
        ...
        self.assertEqual(
            ...
            # Concept-v2 runs use the direct Make path; no early proof groups are sealed.
