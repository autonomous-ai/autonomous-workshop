...
@pytest.mark.parametrize(
    "limit", [True, 999, MAX_PRODUCT_TOKENS + 1, 1000.0, "1000", None]
)
...
def test_cli_default_and_explicit_configuration(command):
    ...
    ceiling = str(MAX_PRODUCT_TOKENS)
    assert parser().parse_args(
        (*command, "--max-tokens", ceiling)
    ).max_tokens == MAX_PRODUCT_TOKENS
    for value in ("0", "-1", str(MAX_PRODUCT_TOKENS + 1), "1.5", "no"):
        with pytest.raises(SystemExit):
            parser().parse_args((*command, "--max-tokens", value))
    assert parser().parse_args(("resume", "wish-id")).max_tokens is None


@pytest.mark.parametrize("saved_limit", [10000000, 100000000, 200000000])
