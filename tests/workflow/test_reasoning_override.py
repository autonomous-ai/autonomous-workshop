...
def resume_context(paths, run, callback):
    ...
    # Motion migration has its own real-run integration suite.
    stack.enter_context(mock.patch.object(host, "_adopt_resume_motion_policy",
        side_effect=lambda paths, run, checkpoint, enabled: checkpoint))
