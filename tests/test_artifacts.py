...
from inventor_workshop import artifacts as artifact_module
...
class ArtifactTest(unittest.TestCase):
    ...
    def test_publish_packet_temp_path_replacement_cannot_clobber_target(self):
        ...
        with mock.patch(
            "inventor_workshop.artifacts.tempfile.mkstemp",
            side_effect=remember_mkstemp,
        ), mock.patch(
            "inventor_workshop.artifacts.os.fdopen", side_effect=replace_before_write
        ), mock.patch(
            "inventor_workshop.artifacts._ANCHORED_STAGING", False
        ):
    ...
    def test_publish_packet_destination_parent_replacement_fails_closed(self):
        ...
        with mock.patch(
            "inventor_workshop.artifacts.os.open", side_effect=replace_parent
        ):
            with self.assertRaises(ArtifactError):
                build_publish_packet(self.root, destination)
    ...
    def test_packet_refuses_changed_bytes(self):
        ...
        with mock.patch(
            "inventor_workshop.artifacts._read_open_file",
    ...
    def test_packet_refuses_changed_executable_mode(self):
        ...
        with mock.patch(
            "inventor_workshop.artifacts._read_open_file", side_effect=change_mode
        ):
            with self.assertRaises(ArtifactError):
                build_publish_packet(self.root, destination)
    ...
    def test_no_follow_open_rejects_regular_file_replacement(self):
        ...
        with mock.patch(
            "inventor_workshop.artifacts.os.open", side_effect=replace_file
        ):
            with self.assertRaises(ArtifactError):
                artifact_module._open_regular_no_follow(
