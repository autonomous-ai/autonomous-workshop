...
from workshop.runtime import Receipt, Runtime
from workshop.playtest.service import Playtest
from workshop.reviews.service import ReviewAuthentication, review_sha256
from workshop.workflow.engine import Workshop, WorkshopTools
from tests.fixtures.delivery_support import fixture_delivery_evidence
        ...
        *,
        ...
        def release_proof(capability):
            ...
            if capability == "game-simulation":
                ...
                pairings = tuple(
                    (left, right) for left in styles for right in styles
                )
                ordered_pairing_games = {
                    "%s|%s" % pairing: 0 for pairing in pairings
                }
                ordered_pairing_wins = {
                    key: {"0": 0, "1": 0} for key in ordered_pairing_games
                }
                first_seat_games = {"0": 0, "1": 0}
                move_order_wins = {"first": 0, "second": 0}
                first_action_results = {
                    '{"fixture_action":"advance","turn":0}': {
                        "games": 0,
                        "first_mover_wins": 0,
                    }
                }
                games = []
                optimizing_cases = optimizing_first_wins = 0
                for index in range(1_000):
                    pairing = pairings[index % len(pairings)]
                    first_seat = index % 2
                    winner = (index // len(pairings)) % 2
                    pairing_key = "%s|%s" % pairing
                    ordered_pairing_games[pairing_key] += 1
                    ordered_pairing_wins[pairing_key][str(winner)] += 1
                    first_seat_games[str(first_seat)] += 1
                    move_order_wins[
                        "first" if winner == first_seat else "second"
                    ] += 1
                    first_action_results[
                        '{"fixture_action":"advance","turn":0}'
                    ]["games"] += 1
                    first_action_results[
                        '{"fixture_action":"advance","turn":0}'
                    ]["first_mover_wins"] += winner == first_seat
                    if pairing == ("optimizing", "optimizing"):
                        optimizing_cases += 1
                        optimizing_first_wins += winner == first_seat
                    games.append(
                        {
                            "index": index,
                            "seed": index,
                            ...
                            "player_styles": list(pairing),
                            "outcome": json.dumps({"winner": winner}),
                            "issues": [],
                        }
                    )
                measurements = {
                    ...
                    "balance_cases": 4,
                    "ordered_pairing_cases": 16,
                    "first_seat_cases": 2,
                    "move_order_cases": 2,
                    "solved_first_player_cases": optimizing_cases,
                    "solved_first_player_wins": optimizing_first_wins,
                    "exploit_cases": 10,
                    "choice_cases": 10,
                    "flow_cases": 10,
                    "balance_failures": 0,
                    "ordered_pairing_failures": 0,
                    "first_seat_failures": 0,
                    "move_order_failures": 0,
                    "solved_first_player_failures": 0,
                    "exploits_found": 0,
                ...
                analysis, analysis_sha256 = write_json(
                    "game-analysis.json",
                    {
                        "artifact_sha256": artifact_sha256,
                        "measurements": measurements,
                        "ordered_pairing_games": ordered_pairing_games,
                        "ordered_pairing_wins": ordered_pairing_wins,
                        "first_seat_games": first_seat_games,
