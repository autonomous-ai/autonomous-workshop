...
from workshop.contributors.scaffold import create_inventor, scaffold_inventor
...
class ScaffoldTest(unittest.TestCase):
    ...
    def test_existing_taste_cannot_change_during_atomic_creation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch(
                "workshop.contributors.contribution.run_declared_checks",
    ...
    def test_generated_package_layout_contains_a_runnable_exact_identity(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            environment = dict(os.environ)
            environment["PYTHONPATH"] = os.pathsep.join(
                (str(target), str(Path(__file__).resolve().parents[2] / "src"))
            )
            environment["WORKSHOP_AGENT_WORKERS"] = "disabled"
            profile = subprocess.run(
