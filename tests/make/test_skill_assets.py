...
MAKE_ROOT = CORE / "src" / "workshop" / "make"
ASSETS = MAKE_ROOT / "skills" / "product-to-cad" / "assets"
SCHEMA_OWNERS = {
...
def schema_path(name: str) -> Path:
    return CORE / "src" / "workshop" / SCHEMA_OWNERS[name] / "schemas" / name
...
class ProductToCadAssetTest(unittest.TestCase):
    ...
    def test_receipt_schema_tracks_workshop_measurement_contract(self):
        schema = json.loads(
            schema_path("verification-receipt.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_cad_schemas_bind_physical_claim_evidence_and_reviewer_identity(self):
        project = json.loads(
            schema_path("cad-project.schema.json").read_text(encoding="utf-8")
        )
        receipt = json.loads(
            schema_path("verification-receipt.schema.json").read_text(
                encoding="utf-8"
        ...
        policy = json.loads(
            schema_path("validator-policy.schema.json").read_text(encoding="utf-8")
        )
        gate = json.loads(
            schema_path("gate-result.schema.json").read_text(encoding="utf-8")
        )
    ...
    def test_all_evidence_path_schemas_match_the_runtime_contract(self):
        artifact = json.loads(
            schema_path("artifact-manifest.schema.json").read_text(encoding="utf-8")
        )
        project = json.loads(
            schema_path("cad-project.schema.json").read_text(encoding="utf-8")
        )
