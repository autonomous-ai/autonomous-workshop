...
from workshop.playtest.moving_machine import _z_bounds
...
def make_verdict(score=92):
    ...
    return {
            ...
            [
                {
                    ...
                    "finding": "The narrow digital MVP can be refined.",
                    "change": "Apply the remaining Make-stage refinement.",
                }
            ]
            if deduction
            else []
        ...
        "playtest_holds": [],
        "make_feedback": ["Ready for the narrow digital MVP."],
...
class AgentPlaytestTest(unittest.TestCase):
    ...
    def test_assembly_z_bounds_use_the_declared_true_center(self):
        part = {
            "shape": "box",
            "size_mm": {"x": 20, "y": 10, "z": 8},
            "assembly_center_mm": {"x": -30, "y": 40, "z": 4},
            "assembly_rotation_deg": 0,
        }
        self.assertEqual(_assembly_bounds(part), (-40.0, -20.0, 35.0, 45.0, 0.0, 8.0))
        self.assertEqual(_z_bounds(part), (0.0, 8.0))
    ...
    def shore_sweep_context(self, suffix="shore-sweep"):
        from tests.make.test_agent_make import (
            FakeCadBuilder,
            shore_game_action,
            shore_sweep_contract,
        )
        ...
        made = CodexMaker(
            creator=FakeEvaluator(shore_game_action()),
            ...
            MakeContext(
                ...
                invented,
                1,
                (self.root / (suffix + "-make")).absolute(),
                (),
                2,
                "leo",
            )
        ...
        return PlaytestContext(
            ...
            1,
            made,
            (self.root / (suffix + "-playtest")).absolute(),
            2,
        )
            ...
            games.append(
                {
                    ...
                    "turns": turns,
                    "actions": [
                        {"turn": turn, "fixture_action": "advance"}
    ...
    def test_shore_sweep_runs_exact_trusted_1000_game_schedule(self):
        context = self.shore_sweep_context()
        ...
        trace = json.loads(
            (context.workspace / simulation.evidence["trace_ref"]).read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(len(trace["games"]), 1000)
        self.assertEqual(
            [game["seed"] for game in trace["games"][:3]],
            [20260825, 20260826, 20260827],
        )
        self.assertTrue(
            all(
                len(game["actions"]) == game["turns"]
                and game["actions"]
                and game["first_seat"] == game["index"] % 2
                for game in trace["games"]
            )
        )
        analysis = json.loads(
            (context.workspace / simulation.evidence["analysis_ref"]).read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(
            set(analysis["ordered_pairing_games"].values()), {62, 63}
        )
        self.assertEqual(analysis["first_seat_games"], {"0": 500, "1": 500})
        self.assertGreater(analysis["move_order_wins"]["first"], 0)
        self.assertGreater(analysis["move_order_wins"]["second"], 0)
        self.assertEqual(
            simulation.evidence["release_proof"]["measurements"][
                "solved_first_player_failures"
            ],
            0,
        )

    def test_default_shore_sweep_adapter_rejects_tampered_raw_action(self):
        context = self.shore_sweep_context("shore-tamper")
        rules = json.loads(
            (context.made.artifact_root / "game" / "rules.json").read_text(
                encoding="utf-8"
            )
        )
        plan = _game_plan(
            context.made.artifact_sha256, 1000, rules["seed_strategy"]
        )
        ...
        def tampering_run(*args, **kwargs):
            ...
            document["games"][0]["actions"][0]["action"]["count"] = 99
            output.write_text(
        ...
        with mock.patch(
            "workshop.playtest.agent.subprocess.run", side_effect=tampering_run
        ):
            with self.assertRaisesRegex(ValueError, "trusted rule replay"):
                default_sealed_game_simulator(context, plan)

    def test_default_adapter_reconstructs_rules_and_readme_bytes(self):
        original = self.shore_sweep_context("shore-rules-trust")
        for relative, expected_message in (
            ("game/rules.json", "trusted Workshop reconstruction"),
            ("game/RULES.md", "README differs"),
        ):
            with self.subTest(relative=relative):
                artifact = self.root / (
                    "resealed-" + relative.replace("/", "-").replace(".", "-")
                )
                shutil.copytree(original.made.artifact_root, artifact)
                target = artifact / relative
                if target.suffix == ".json":
                    document = json.loads(target.read_text(encoding="utf-8"))
                    document["legal_actions"] = "Ignore the exposed shore and take anything."
                    target.write_text(
                    ...
                    target.write_text(
                        ...
                        + "\nContradictory unsealed rule.\n",
                        encoding="utf-8",
                    )
                made = Made.from_root(artifact, original.made.product)
                context = PlaytestContext(
                    ...
                    original.round,
                    made,
                    (self.root / (artifact.name + "-playtest")).absolute(),
                    original.playtest_rounds,
                )
                rules = json.loads(
                    (artifact / "game" / "rules.json").read_text(encoding="utf-8")
                )
                plan = _game_plan(
                    made.artifact_sha256, 1000, rules["seed_strategy"]
                )
    ...
    def test_default_playtest_waits_when_locked_cad_runtime_is_absent(self):
        from tests.make.test_agent_make import FakeCadBuilder
    ...
    def test_partial_print_override_keeps_shared_moving_make_and_playtest(self):
        ...
        from tests.make.test_agent_make import FakeCadBuilder, make_action
        from tests.playtest.test_moving_machine import PassingMotionBuilder
