...
class GameplayEngineTest(unittest.TestCase):
    def test_mulberry32_raw_u32_vector_is_pinned(self):
        rng = _FiniteGameRng(1)
        for _ in range(5):
            rng.random()
        self.assertEqual(
            rng.values,
    ...
    def test_pinned_finite_game_schema_is_strict_for_both_rule_families(self):
        shared = {
            "enabled": True,
            "title": "Seven Sparks",
            ...
            "max_take": 3,
            "last_take_wins": True,
            "theme": "A shared constellation.",
            "token_part_ids": ["spark-%d" % index for index in range(1, 8)],
            "token_sweep_values": [],
        }
        shore = {
            **shared,
            ...
            "max_take": 3,
            "theme": "Signals sweep inward from exposed shores.",
            "token_part_ids": ["signal-%d" % index for index in range(1, 8)],
            "token_sweep_values": [1, 2, 3, 1, 3, 2, 1],
        }
        self.assertEqual(validate_finite_game_spec(shared), shared)
        self.assertEqual(validate_finite_game_spec(shore), shore)

        for malformed in (
            {key: value for key, value in shore.items() if key != "rule_kind"},
            {**shared, "token_sweep_values": [1] * 7},
            {**shore, "token_sweep_values": [1, 2]},
            {**shore, "max_take": 2},
        ):
            with self.subTest(malformed=malformed), self.assertRaises(ContractError):
                validate_finite_game_spec(malformed)

    def test_pinned_shore_sweep_replay_removes_only_legal_contiguous_ends(self):
        spec = {
            "enabled": True,
            "title": "Signal Spine",
            "rule_kind": "ordered-shore-sweep",
        ...
        request = {
            "index": 17,
            "seed": 260825,
            "player_styles": ["adversarial", "exploratory"],
            "first_seat": 1,
        }
        first = replay_finite_game(spec, request)
        second = replay_finite_game(spec, request)
        self.assertEqual(first, second)
        self.assertTrue(first["completed"])
        self.assertLessEqual(first["turns"], 7)
        sweep_by_id = dict(
            zip(spec["token_part_ids"], spec["token_sweep_values"])
        )
        for turn in first["actions"]:
            pre = turn["pre_remaining_token_part_ids"]
            action = turn["action"]
            count = action["k"]
            exposed = pre[0] if action["shore"] == "L" else pre[-1]
            self.assertLessEqual(count, min(sweep_by_id[exposed], len(pre)))
            expected_removed = (
                pre[:count]
                if action["shore"] == "L"
                else list(reversed(pre[-count:]))
            )
            self.assertEqual(turn["removed_token_part_ids"], expected_removed)
            self.assertIn(action, turn["legal_actions"])

        with self.assertRaises(ContractError):
            replay_finite_game(spec, {**request, "untrusted": True})
