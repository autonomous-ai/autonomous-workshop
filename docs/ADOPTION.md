...
Alice and Bob execute shared Workshop code on normal product paths. Each
...
Bob's launchd installer pins the exact repository `src` directory so
an unrelated same-version global install cannot satisfy a scheduled runtime by
accident.

Its Workshop SQLite database is the only authority for outside effects.
...
import, live, readback, redirect, and launchd behavior.
...
golden transition fixtures. Alice and Bob keep stronger native behavior
