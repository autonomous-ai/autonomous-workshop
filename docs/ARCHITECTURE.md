...
- **Make** consumes that exact sealed Invent result and creates the actual
  ...
  blocking visual defect. Make first checks the fresh draft mesh and wall
  thickness, permits at most two review rounds, and resolves at most one focused
  ...
  finalizer requires its current record to be a passing final full-tier run with
  a successful thickness row before the host repeats the isolated gate.
