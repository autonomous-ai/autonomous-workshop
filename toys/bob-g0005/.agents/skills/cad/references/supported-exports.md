...
STL, 3MF, and native GLB are mesh exports, not substitutes for STEP. Validate the primary CAD geometry first, then export the requested formats. Do not treat a mesh export as CAD validation; inspect the primary model per the standard workflow.

Native GLB exports are ordinary glTF 2.0 binary files for external tools: Y-up, meter-scaled, and free of the `STEP_topology` extension. Do not confuse them with the render package — the component-GLB package directory at `<folder>/__cadgen__/models/<name>.step/` (an `assembly.json` descriptor plus a `components/` dir of content-addressed GLBs) — which `scripts/gen` builds and `scripts/export` never writes.
...
```bash
python "$CAD_SKILL_ROOT/scripts/export" path/to/model.step.py --stl --3mf --glb
```
...
```bash
python "$CAD_SKILL_ROOT/scripts/export" path/to/model.step.py \
...
```bash
python "$CAD_SKILL_ROOT/scripts/export" path/to/imported.step --stl --3mf
```
...
1. Validate the model per the standard workflow (generate, inspect).
...
```bash
python "$CAD_SKILL_ROOT/scripts/gen" <project-dir>/<name>.step.py --write

python "$CAD_SKILL_ROOT/scripts/export" <project-dir>/<name>.step.py \
  --stl <project-dir>/meshes/<name>.stl \
  --glb <project-dir>/meshes/<name>.glb \
  --mesh-tolerance 0.2 \
  --mesh-angular-tolerance 0.2

python "$CAD_SKILL_ROOT/scripts/inspect" refs <project-dir>/<name>.step --facts --planes --positioning
```
