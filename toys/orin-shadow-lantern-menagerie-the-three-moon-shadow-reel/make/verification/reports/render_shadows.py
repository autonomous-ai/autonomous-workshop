...
def source_and_wall(
    source_to_mask: float = SOURCE_TO_MASK_MM,
    mask_to_wall: float = MASK_TO_WALL_MM,
    source_x: float = 0.0,
    source_z: float = PORTAL_WORLD_Z,
) -> tuple[np.ndarray, float]:
    mask_y = -(REEL_Z + 1.6)
    source = np.array([source_x, mask_y - source_to_mask, source_z])
    wall_y = mask_y + mask_to_wall
...
def raster_shadow(
    ...
    size: int,
    light: tuple[np.ndarray, float] | None = None,
) -> Image.Image:
    source, wall_y = light or source_and_wall()
...
def composite(
    static_shadow: Image.Image,
    reel_triangles: np.ndarray,
    size: int,
    light: tuple[np.ndarray, float] | None = None,
) -> Image.Image:
    reel_shadow = raster_shadow(reel_triangles, size, light)
...
def metrics(
    ...
    center_x_mm = source[0] + magnification * (0.0 - source[0])
    center_z_mm = source[2] + magnification * (PORTAL_WORLD_Z - source[2])
    center_x_px = size / 2 + center_x_mm * size / VIEW_MM
    center_y_px = size / 2 - (center_z_mm - PORTAL_WORLD_Z) * size / VIEW_MM
    yy, xx = np.ogrid[:size, :size]
    circle = (xx - center_x_px) ** 2 + (yy - center_y_px) ** 2 <= radius_px ** 2
    ...
    return {
        ...
        "projected_portal_center_x_mm": round(float(center_x_mm), 3),
        "projected_portal_center_z_mm": round(float(center_z_mm), 3),
    }
...
def main() -> int:
    ...
    nominal_light = source_and_wall()
    static_work = raster_shadow(static, WORK_SIZE, nominal_light)
    static_state = raster_shadow(static, STATE_SIZE, nominal_light)
    rows = []
    thumbs = []
    for angle in range(0, 360, 5):
        reel = transform_reel(meshes["shadow_reel"], angle)
        image = composite(static_work, reel, WORK_SIZE, nominal_light)
        rows.append({"clockwise_deg": angle, **metrics(image, nominal_light)})
        thumbs.append(image.resize((144, 144), Image.Resampling.LANCZOS))
        if angle in STATE_NAMES:
            state = composite(static_state, transform_reel(meshes["shadow_reel"], angle), STATE_SIZE, nominal_light)
    ...
    range_rows, range_tiles = [], []
    for source_distance in (400.0, 500.0):
        for wall_distance in (600.0, 900.0):
            light = source_and_wall(source_distance, wall_distance)
            static_range = raster_shadow(static, WORK_SIZE, light)
            for angle, name in STATE_NAMES.items():
                image = composite(static_range, transform_reel(meshes["shadow_reel"], angle), WORK_SIZE, light)
                range_rows.append({"source_to_mask_mm": source_distance, "mask_to_wall_mm": wall_distance, "state": name, "clockwise_deg": angle, **metrics(image, light)})
                range_tiles.append((f"{name}  S{source_distance:.0f} W{wall_distance:.0f}", image.resize((220, 220), Image.Resampling.LANCZOS)))
    range_sheet = Image.new("RGB", (3 * 220, 4 * 244), (230, 225, 213))
    range_draw = ImageDraw.Draw(range_sheet)
    for index, (label, tile) in enumerate(range_tiles):
        x, y = index % 3 * 220, index // 3 * 244
        range_sheet.paste(tile, (x, y)); range_draw.text((x + 6, y + 224), label, fill=(20, 25, 35))
    range_sheet.save(output / "distance-corners-contact-sheet.png")
    offset = math.tan(math.radians(2.0)) * SOURCE_TO_MASK_MM
    alignment_cases = (("centered", 0.0, 0.0), ("yaw-left-2deg", -offset, 0.0), ("yaw-right-2deg", offset, 0.0), ("pitch-down-2deg", 0.0, -offset), ("pitch-up-2deg", 0.0, offset))
    alignment_rows, alignment_tiles = [], []
    for angle, name in STATE_NAMES.items():
        for label, source_x, source_z_offset in alignment_cases:
            light = source_and_wall(source_x=source_x, source_z=PORTAL_WORLD_Z + source_z_offset)
            static_alignment = raster_shadow(static, WORK_SIZE, light)
            image = composite(static_alignment, transform_reel(meshes["shadow_reel"], angle), WORK_SIZE, light)
            alignment_rows.append({"state": name, "clockwise_deg": angle, "case": label, "source_x_mm": round(source_x, 3), "source_z_offset_mm": round(source_z_offset, 3), **metrics(image, light)})
            alignment_tiles.append((f"{name}  {label}", image.resize((220, 220), Image.Resampling.LANCZOS)))
    alignment_sheet = Image.new("RGB", (5 * 220, 3 * 244), (230, 225, 213))
    alignment_draw = ImageDraw.Draw(alignment_sheet)
    for index, (label, tile) in enumerate(alignment_tiles):
        x, y = index % 5 * 220, index // 5 * 244
        alignment_sheet.paste(tile, (x, y)); alignment_draw.text((x + 6, y + 224), label, fill=(20, 25, 35))
    alignment_sheet.save(output / "alignment-sweep-contact-sheet.png")
    source, wall_y = nominal_light
    report = {
        "schema_version": 2,
        ...
        "samples": rows,
        "distance_corner_samples": range_rows,
        "alignment_samples": alignment_rows,
        "files": {
            "rabbit": "state-rabbit-000.png", "fox": "state-fox-120.png",
            "owl": "state-owl-240.png", "motion": "motion-contact-sheet.png",
            "distance_corners": "distance-corners-contact-sheet.png",
            "alignment_sweep": "alignment-sweep-contact-sheet.png",
        },
        "limitations": [
