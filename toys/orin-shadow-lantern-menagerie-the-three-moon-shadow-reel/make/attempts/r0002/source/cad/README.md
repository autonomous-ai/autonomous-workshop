...
continuously; the kickstand opens until its paired trunnion tabs meet the broad
rear stop posts. Reset at the visibly unique double-V rim notch beneath the
front-shell leaf opening: rabbit home is also the only pocket that releases an
additional 0.25 mm of nominal leaf travel. Printed snap
...
every nominal paired clearance, including chamfered hook entry, retained
overhang, and the three detent travels. `measure/motion.json` drives the full
reel and stand collision sweeps, rigid shell pullout retention, and both stand
endpoint overtravel checks. `measure/render_shadows.py` derives the 72-pose
sweep, all 12 indexed state/distance corners, and centered/±2 degree yaw and
pitch views from the exact four exported meshes.
