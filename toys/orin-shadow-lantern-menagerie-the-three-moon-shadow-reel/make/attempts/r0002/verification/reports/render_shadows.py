...
def main() -> int:
    ...
    for label, source_x, source_z_offset in alignment_cases:
        ...
        image = composite(static_alignment, transform_reel(meshes["shadow_reel"], 240), WORK_SIZE, light)
        alignment_rows.append({"case": label, "source_x_mm": round(source_x, 3), "source_z_offset_mm": round(source_z_offset, 3), **metrics(image, light)})
        alignment_tiles.append((label, image.resize((220, 220), Image.Resampling.LANCZOS)))
    alignment_sheet = Image.new("RGB", (5 * 220, 244), (230, 225, 213))
    alignment_draw = ImageDraw.Draw(alignment_sheet)
    for index, (label, tile) in enumerate(alignment_tiles):
        x = index * 220
        alignment_sheet.paste(tile, (x, 0)); alignment_draw.text((x + 6, 224), label, fill=(20, 25, 35))
