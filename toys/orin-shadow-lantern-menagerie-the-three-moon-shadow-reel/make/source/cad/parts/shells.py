...
def rear_bearing_support(shell):
    for side in (-1.0, 1.0):
        ...
        # Seat the diagonal support through a broad rounded land instead of a
        # narrow rectangular corner meeting the chamfered shell nearly
        # tangentially.  The old construction left one 0.26 mm knife-edge
        # sample near (-53.6, -37.9, 3.2) in the exact rear-shell STL.
        rib = capsule((side * 46.0, -47.0), (x, STAND_HINGE_Y), 8.0, REAR_FACE_T)
...
def make_rear_shell():
    ...
    # Broad through-cut strokes form a phone outline, camera dot, and beam
    # arrow aimed at the portal.  The negative pictogram remains legible in a
    # one-colour print and backlighting, unlike the old disconnected relief
    # tiles or a shallow same-colour emblem.
    cue_h = REAR_FACE_T + 0.4
    phone = Pos(-33.0, -8.0, -0.2) * rounded_plate(14.0, 24.0, 2.0, cue_h)
    beam = fuse_all([
        Pos(-23.5, -8.0, -0.2) * Box(7.0, 5.0, cue_h, align=(Align.CENTER, Align.CENTER, Align.MIN)),
        Pos(-17.0, -8.0, -0.2) * Box(7.0, 8.0, cue_h, align=(Align.CENTER, Align.CENTER, Align.MIN)),
        Pos(-10.5, -8.0, -0.2) * Box(7.0, 12.0, cue_h, align=(Align.CENTER, Align.CENTER, Align.MIN)),
    ])
    shell = shell.cut(phone, beam)
    for x in (-40.0, 40.0):
