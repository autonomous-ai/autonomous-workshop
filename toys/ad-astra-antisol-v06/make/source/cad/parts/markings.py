...
Neptune, Uranus and Saturn are described entirely by latitude, longitude and
angular size, because a belt of latitude and a round patch are what a band
system and a storm actually are.  Jupiter is described BOTH ways: its four
narrow belts are still plain latitude bands, but its two widest -- the South
Equatorial and the North Equatorial -- are outline rings carrying a 2.5 degree
wave on each boundary, because an exact circle of latitude reads as a machined
edge and at 13 and 10 degrees wide those two are the only belts with room for
a wave.  Its Great Red Spot and its collar are outline ovals as well.  A belt
that encircles the globe is an annulus and no single radial cone is one, so
each wavy belt is six longitude sectors abutting on exact radial planes.
...
Mercury's in `parts/mercury_atlas.py`, Venus's in `parts/venus_atlas.py` and
Jupiter's belts, zones, spot and collar in `parts/jupiter_atlas.py`.
...
reason; Earth's, Mars's, Mercury's and Venus's keep the subtractions they
were built with.
"""
