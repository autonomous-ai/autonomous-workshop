...
part = asm.add(import_step("<project-dir>/parts/<part>.step"), "part")
```
...
python "$CAD_SKILL_ROOT/scripts/inspect" refs path/to/assembly.step --facts --planes --positioning
```
...
```bash
python "$CAD_SKILL_ROOT/scripts/inspect" frame path/to/assembly.step '#selector'
```
...
```bash
python "$CAD_SKILL_ROOT/scripts/inspect" measure path/to/assembly.step \
  --from '#selector_a' \
  --to '#selector_b' \
  --axis z
```
