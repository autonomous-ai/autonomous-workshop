...
def _ref_display_candidates(display_name: str) -> set[str]:
    ...
    A ref shows a ``.step.py`` generator as a bare stem, so ``<name>`` is what the Viewer emits
    for ``<name>.step.py``. Every other file keeps its suffix and is already literal, which is
...
def ensure_ref_file_matches(file_prefix: str, entry_target: str, *, source_label: str = "ref") -> None:
    ...
    # a viewer prefix keeps its extension (`<name>.step.py`) while an entry's cad_path has
    # already been reduced to its logical form (`<project-dir>/<name>`). Matching either way
    # means `<name>.step.py`, `<name>.step` and `<name>` all name the same entry, which is
