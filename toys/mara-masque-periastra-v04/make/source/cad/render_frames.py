...
def main():
    ...
    parser.add_argument('--tolerance', type=float, default=0.05,
                        help='tessellation deflection, mm. The blend at the telescope root and '
                             'the dome are curved surfaces whose flat-shaded triangles dither at '
                             'the default; this only changes how finely the exact STEP is '
                             'triangulated for display and never touches the geometry.')
    ...
    state = module.tessellate_occurrences(shape, args.tolerance)
    if args.crop:
    ...
    record = {'source': str(args.source), 'source_sha256': hashlib.sha256(args.source.read_bytes()).hexdigest(),
              'camera': {'azimuth_deg': args.azimuth, 'elevation_deg': args.elevation,
                         ...
                         'tessellation_mm': args.tolerance},
