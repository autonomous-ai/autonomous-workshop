...
def main():
    ...
    parser.add_argument('--playing', type=Path, default=Path(__file__).parent.parent/'evidence-states/playing.step')
    ...
    states = []
    sources=(args.closed,args.opened,args.playing)
    for path in sources:
        _,shape=module.build_shape(path.resolve())
        states.append(module.tessellate_occurrences(shape,args.tolerance))
    ...
    sheet=Image.new('RGB',(len(frames)*args.size,args.size),module.BACKGROUND)
    ...
    proof={
        ...
        'camera':{'azimuth_deg':args.azimuth,'elevation_deg':args.elevation,'projection':'orthographic','tessellation_mm':args.tolerance},
        'signature_common_framing':True,
        'framing_min_mm':framing.min(axis=0).tolist(),'framing_max_mm':framing.max(axis=0).tolist(),
        'states':['closed','opened (roof lifted 100 mm)','playing (roof off, board populated)'],
        'inputs':{str(path):hashlib.sha256(path.read_bytes()).hexdigest() for path in sources},
