...
# Uranus's own block.  Saturn's numbers above are not touched and must not be:
# LADDER_CONSTANT was solved against Saturn's ring ceiling, and the Saturn brief
# in this chain forbade moving any of them.  These are parallel constants with
# their own names, which is what that brief asked a later ringed world to do.
#
# A ring lies in its planet's equatorial plane.  Uranus's obliquity is 97.77
# degrees, so its equatorial plane stands 7.77 degrees past vertical and the
# ring is an upright hoop rather than a flat brim.  That orientation is free --
# it falls out of the obliquity the set already builds every world at -- and it
# is what keeps this piece from reading as a second Saturn.
URANUS_RING_PROJECTION = 1.00        # [assumed] per side, from the owner's
                                     # instruction that the ring be small: a
                                     # little over half Saturn's 2.00 mm
URANUS_RING_INNER_D = 22.02          # [inferred] tangent to Uranus's equator,
                                     # so the hoop's inner edge springs from the
                                     # globe all the way round and no part of it
                                     # bridges unsupported air
URANUS_RING_OUTER_D = 24.02          # [inferred] 22.02 + 2 * 1.00
URANUS_RING_THICKNESS = 1.00         # [assumed] 2.5 extrusion widths at a
                                     # 0.4 mm nozzle, and narrower than Saturn's
                                     # 1.40 for the same reason the projection is
URANUS_RING_GLOBE_BITE = 0.80        # [assumed] how far the hoop cuts into the
                                     # globe before the globe is subtracted, so
                                     # the two fuse across a real overlap
URANUS_RING_FOOT_SLOPE = RING_WEB_SLOPE   # 1.08: 47.2 deg from horizontal, the
                                     # angle this set already trusts under
                                     # Saturn's ring for exactly this problem
URANUS_RING_FOOT_REACH = 1.30        # [inferred] how far the foot stands proud
                                     # of the hoop where it leaves the disc top.
                                     # An upright hoop meets the flat cut at
                                     # DISC_H on the steep part of its own curve,
                                     # so the rim there overhangs by 48.6 deg and
                                     # stays over the 45 deg gate until Z = 5.52.
                                     # 1.30 mm of reach carries the foot's own
                                     # 42.8 deg cone past that crossing with
                                     # about a degree in hand; less lands the
                                     # crossing on the gate itself
URANUS_RING_FOOT_RISE = 1.50         # [inferred] how far the foot's cone runs
                                     # up before it is inside the globe, where
                                     # its end face stops being a surface

#: Which worlds carry a ring, and the constants each one's ring is built from.
#: `saturn` restates its own block above and changes nothing; `uranus` names the
#: block just above this one.  `support` chooses what holds the ring up: Saturn's
#: near-horizontal plate needs the full revolved web under it, and Uranus's
#: upright hoop needs only a small wedge at each of the two places it leaves the
#: disc top.
RINGED_WORLDS = {
    "saturn": {
        "outer_d": RING_OUTER_D,
        "thickness": RING_THICKNESS,
        "bite": RING_GLOBE_BITE,
        "support": "web",
        "colour": "white",
    },
    "uranus": {
        "outer_d": URANUS_RING_OUTER_D,
...
GLOBE_COLOUR = {
    "mercury": "gray",
    "mars": "red",
    # The Magellan radar mosaic is a saturated golden amber, and `beige`
    # #F7E6DE is a pale pink-cream; that gap is too wide to close with
    # markings.  `sunflower_yellow` #FFB549 is the closest amber the shop
    # stocks and is already in the set as the Sol den plug, so the change
    # costs no new filament.  `beige` stays -- Earth's dryland uses it.
    "venus": "sunflower_yellow",
    "earth": "blue",
    "neptune": "blue",
    "uranus": "cyan",
    "saturn": "yellow",
