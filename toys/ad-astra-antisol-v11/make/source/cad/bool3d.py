...
from build123d import CenterOf, Compound, Pos, Rot, scale as _scale
...
def _same_shape(one, other) -> bool:
    ...
    Volume and bounding box are not enough, and the shortfall is not academic.
    Uranus's two polar hoods are one cap at each pole of a globe leaning 97.77
    degrees: turned 180 degrees about Z about its own bounding-box centre, the
    southern cap keeps its volume to the last decimal and keeps its bounding box
    exactly, and points at the north pole instead.  Offered that as a
    seam-moved alternate against a globe whose northern cap had already been
    taken out, the kernel answered honestly -- nothing inside, everything
    outside -- and the two halves added back up to the whole, so `_split_pair`
    accepted it and the southern hood silently vanished from the part.

    The centre of mass closes it.  A turn that only moves a revolved seam is a
    symmetry of the solid and cannot move its centre of mass; a turn that
    points the solid somewhere else moves it, by 0.59 mm on that cap.  The
    bounding box cannot see the difference because a bounding box is blind to
    where the material inside it sits.
    """
    ...
    if not all(
        ...
        return False
    try:
        here, there = one.center(CenterOf.MASS), other.center(CenterOf.MASS)
    except (AttributeError, ValueError, RuntimeError):   # pragma: no cover
        return False
    return all(
        abs(getattr(here, axis) - getattr(there, axis)) <= 1e-6 * span
        for axis in ("X", "Y", "Z")
    )
