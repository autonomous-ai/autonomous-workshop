...
def main() -> int:
    ...
    for lat in lats[::2]:
        for lon in lons[::2]:
            ...
            right = classify_anti(lat, lon)
            checked += 1
            if (left is None) != (right is None):
                continue          # the collar cuts at a different Venusian
                                  # latitude on each army, by 5.28 degrees
            if left != right:
                mismatch += 1
    print("%d points classified on both armies at half this grid's resolution."
          % checked)
    print("**%d disagree on which marking they carry.** " % mismatch, end="")
    print("The two pieces carry the same map; what differs is the direction the")
    print("globe leans, which is the ownership cue and is mirrored by")
    print("construction. The collar cuts the two at Venusian latitudes %.2f"
          % (2 * (180.0 - TILT)))
    print("degrees apart, so a point near its edge can be on one piece and")
    print("inside the other's disc; those points are excluded above and are")
    print("measured as visible fractions in `measure/venus-atlas-resolution.md`.")
    print()
