...
def main() -> int:
    ...
    print()
    print("## The colour, reversed by the owner and measured again")
    print()
    print("This ring prints in `%s`." % spec["colour"])
    print()
    print("It has printed in two filaments across this chain and the history is")
    print("worth having in one place, because the second change reverses the")
    print("first.")
    print()
    print("The Uranus edition took `white` first -- the tone Saturn's ring already")
    print("wears, because in this set rings are white -- and then moved off it on")
    print("a measured reading. An independent reviewer, shown the board cold and")
    print("not told what the set was, named both Uranus pieces as tennis balls,")
    print("twice, and gave the mechanism in his own words: 'a bold white curved")
    print("line arcing down one side of a saturated-colour ball, and a large pale")
    print("panel on the opposite side'. `gray` was tried and measured WORSE in")
    print("use -- quieter as a number, a more convincing seam to the eye, because")
    print("a tennis seam is a dark curve on a bright ball -- so the ring was given")
    print("the globe's own `cyan` and stopped being a marking at all.")
    print()
    print("**The owner has reversed that, and the ring is `white` again.** The")
    print("appearance of the toy is his to decide and an argument from a reference")
    print("photograph does not outrank him. The geometry did not move by so much")
    print("as a micron: this is one existing body changing spools.")
    print()
    print("Read the reviewer's mechanism again, because it has two halves and this")
    print("revision removes one of them. The pale panel was the polar hood, and")
    print("there is no hood on this world any more -- there is no marking on it at")
    print("all. Whether a white hoop ALONE on a bare `cyan` ball still reads as a")
    print("seam is a different question from the one that reviewer answered, and")
    print("it is not assumed here in either direction: it is put to this build's")
    print("blind review as its own question, unprimed, and whatever comes back is")
    print("recorded in `snap/SIGNATURE-REVIEW.json` beside the number.")
    print()
    print("The number is in `measure/uranus-ring-tone.md`, measured the same way:")
    print("one piece built once and rendered twice at one camera with only the")
    print("ring repainted, so the pixels that move are the hoop's and nothing")
    print("else moves at all.")
    print()
    print("**What the reversal gains.** Both ringed worlds now print their ring in")
    print("`white`, which is what `RING_COLOUR` holds as this set's default. The")
    print("'rings are white' rule holds without a footnote for the first time, and")
    print("the two rings are the same KIND of part again: a separate white piece")
    print("on each, told apart by how it stands rather than by what it is made of.")
    print()
    print("**What it costs.** Uranus's piece drops from four spools to three and")
    print("its globe-and-what-stands-on-it count from three to two, because the")
    print("`beige` hoods went with the same decision. The ring is now the loudest")
    print("single boundary on the piece, on a world whose reference is the")
    print("quietest in the folder, and it is the only thing on the globe at all.")
