...
more wear their weather. **One wears nothing at all, and that is what this
edition changes.**
...
ran for **652.29 seconds** over eleven steps: **seven ran and passed, three are
not applicable and are recorded as not run rather than as passed, and none was
left unverified, interrupted or timed out.** The geometry inspection completed
**all 51 of its checks, every one `passed`** — that includes validating every
one of the 220 pieces in the assembled set for sound topology and positive
volume, and checking the whole assembly for interference, which is the only way
to answer whether any two parts occupy the same space. Nothing does. (220 rather
than the previous edition's 224: the difference is exactly the four beige bodies
this edition removed, one hood at each pole of each Uranus piece.
`cad/measure/occurrence-geometry.md` lists them by name.) Of the 652.29 seconds,
237.43 went on regenerating all 25 entries from source and 398.06 on the single
inspection batch.

An earlier attempt at this same pipeline was interrupted part-way by a session
ending. That run produced no verdict and none was taken from it; the numbers
above are a complete run from the beginning, and the report it wrote is
`cad/measure/verification-pipeline.md`.

Separately, in their own rounds, all 24 printed parts cleared the 0.80 mm wall
gate and the 45-degree overhang gate at a 0.4 mm nozzle — 24 of 24 on each, with
no region anywhere needing support and no bridges.

**Both Uranus files came out byte-for-byte identical to the last edition's**,
and that is the check rather than a coincidence. Taking a marking off a globe
stops partitioning it and moves no surface; repainting a ring moves one body
from one spool to another and moves no surface either. If either of those had
touched the ball or the hoop, those two hashes would have moved with it.
20 of the 24 printed geometries are byte-identical. The four board panels differ
in exactly one line each — a `NEXT_ASSEMBLY_USAGE_OCCURRENCE`, the exporter's own
per-session occurrence counter, carrying no geometry — and the amount it differs
by is itself the proof: the counters read 225–228 in the published set and
221–224 here, which is a shift of exactly four, which is exactly the four hoods.
Every one of those four files was diffed against the archive copy line by line in
this run rather than asserted.

Measured on the solids as well: each Uranus globe now holds its published volume
plus both the hoods it lost — 5323.412 + 109.5887 + 109.5887 = 5542.588 mm³, with
**0.0014 mm³** of residue on both armies — and the ring is unmoved to nine
decimal places at 54.830480 mm³ and 54.830494 mm³, in the same bounding box, with
the 45-degree gate still measuring 44.3° and 0.0000 mm² over it. Saturn's two
rings come out at 574.192167 and 574.192096 mm³, zero delta.
`cad/measure/source-diff.md` carries the entire source difference, pasted whole
rather than summarised: three edits that change the design, and no dimension
moved.

The three checks that were not run are recorded as not run rather than as
passed: the mounting and power checks, because there is no bought part and no
powered system in this set to check, and the motion check, which is switched off
for this build — so **motion is unverified**, not passed. Nothing in the set
moves against anything else in any case.
