...
Four of the eight worlds are described by latitude, longitude and angular
size, because a union of round patches or a belt of latitude is what a cloud
pattern, a band system or a storm actually is.  Neptune, Uranus, Saturn and
Jupiter are those four.

Earth, Mars, Mercury and Venus are the four exceptions and are described by
outline.  They are the four worlds in the set with a real mapped surface --
Earth's coastlines, the dark triangle of Syrtis Major that every telescope
owner has drawn since 1659, Mercury's smooth plains with the Caloris basin in
them, and Venus's highland continents under its clouds -- and drawing any of
the four as circles throws that away.  Earth's and Mars's are recognisable
silhouettes a player can check the model against, and so is Aphrodite Terra;
Mercury's plains are not, because its albedo boundaries are soft and unnamed,
but circles are still the one thing they must not be: a union of discs reads
as a beach ball rather than as terrain.  Jupiter's Great Red Spot and
Neptune's dark spot stay round, because a storm in an atmosphere really is a
round patch.  Earth's rings live in `parts/atlas.py`, Mars's in
`parts/mars_atlas.py`, Mercury's in `parts/mercury_atlas.py` and Venus's in
`parts/venus_atlas.py`.
