...
| `parts/` | one module per physical part, plus `markings.py`, the per-planet surface table, and the four outline atlases: `atlas.py` for Earth's coastlines, `mars_atlas.py` for Mars's albedo map, `mercury_atlas.py` for Mercury's smooth plains and Caloris basin, and `venus_atlas.py` for Venus's radar highlands and plains |
...
the numeral, which every world shares: Earth takes four; Mercury, Mars, Venus,
Neptune, Saturn and Jupiter take three; Uranus alone takes two. **Venus prints
in three — `sunflower_yellow`, `beige` and `cocoa_brown` — where it printed in
...
patches is not a compromise but the honest answer — Jupiter's Great Red Spot
and Neptune's dark spot stay round for exactly that reason.
