...
def yz_prism(points, width):
    """Extrude a YZ profile symmetrically along X."""
    profile = Plane.YZ * make_face(Polygon(*points))
    return Pos(-width / 2, 0, 0) * extrude(profile, amount=width, dir=(1, 0, 0))
...
def regular_prism(radius, height, sides=6):
    """Vertical supportless polygonal boss used for large printable rivets."""
    points = [
        (radius * cos(2.0 * pi * index / sides), radius * sin(2.0 * pi * index / sides))
