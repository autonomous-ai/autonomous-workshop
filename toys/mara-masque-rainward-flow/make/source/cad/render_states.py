...
def exact_occurrences(state,path):
    ...
    for intended in expected:
        ...
        row=(points,faces,colour);named[intended.label]=row
        if intended.label.startswith('die'):
            # Render only the exact STEP faces. Small native conical
            # faces are the modeled pip recesses; paint fills
            # those existing faces. No dot is drawn and no geometry is added.
            for face in s.faces():
                vv,ff=face.tessellate(.025)
                cc=(17,14,13) if face.area<10 else colour
                result.append((np.array([[p.X,p.Y,p.Z] for p in vv]),np.array(ff,dtype=np.int64),cc))
...
def details(snap):
    ...
    from rainward_lib import DIE_COLOR,SINGLE_COLOR,FORK_COLOR
    die=import_step(ROOT/'part_die_1.step').solids()[0]
    occ=[]
    for face in die.faces():
        vv,ff=face.tessellate(.025)
        color=(17,14,13) if face.area<10 else tuple(round(c*255) for c in DIE_COLOR)
        occ.append((np.array([[p.X,p.Y,p.Z] for p in vv]),np.array(ff,dtype=np.int64),color))
    left=API['render'](occ,35,35,1000,.12)
    right=API['render'](occ,-145,-35,1000,.12)
    sheet=Image.new('RGB',(2000,1050),'#edf0f5');sheet.paste(left,(0,50));sheet.paste(right,(1000,50))
    draw=ImageDraw.Draw(sheet);draw.text((30,15),'SAME DIE / OPPOSITE VIEWS',fill='#222222',font_size=24)
    sheet.save(snap/'dice-detail.png')
    shapes=[]
    for name,x,color in [('single',-8,SINGLE_COLOR),('fork',8,FORK_COLOR)]:
        shape=import_step(ROOT/f'part_{name}_01.step').solids()[0].moved(Location((x,0,0)))
        vv,ff=shape.tessellate(.008)
        shapes.append((np.array([[p.X,p.Y,p.Z] for p in vv]),np.array(ff,dtype=np.int64),tuple(round(c*255) for c in color)))
    top=API['render'](shapes,90,90,1400,.1)
    iso=API['render'](shapes,-75,55,1400,.1)
    ...
    sheet.save(snap/'counter-detail.png')
