...
def build():
    ...
    for (x, y, z), angle in leg_roots.values():
        ...
        collars.append(
            ...
            * Pos(-2.0, 0, 0)
            * Rot(0, 90, 0)
            * Cylinder(8.4, 4.0, align=(Align.CENTER, Align.CENTER, Align.CENTER))
        )
        sockets.append(
            Pos(mount_x, mount_y, local_z)
            * Rot(0, 0, angle)
    ...
    # The shallow dorsal spine reads through the armor gaps and terminates
    # flush with the carapace underside rather than intersecting it.
    dorsal_spine = Pos(0, 0, chassis_height - 0.2) * clipped_prism(
        20.0, 70.0, 2.2, 3.0
    )
    body = (body + lugs + collars + dorsal_spine) - sockets
    armor_sockets = [
        Pos(x, plate_centers_y[name] + (4.0 if name == "fore" else 0.0) - plate_depths[name] / 2 + small_peg * 0.36, chassis_height - 2.0) * Box(small_socket, small_socket, 7.0, align=(Align.CENTER, Align.CENTER, Align.CENTER))
    ...
    hub_spine_clearance = Pos(0, hub_center_y + 6.0, chassis_height + 2.0) * Box(
        hub_width + 2.0,
        hub_depth + 14.0,
        4.0,
