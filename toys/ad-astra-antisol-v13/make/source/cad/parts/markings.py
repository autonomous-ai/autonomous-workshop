...
#: The three worlds whose map is mirrored on the Anti-Sol piece, and nothing
#: else in this set is.
#:
...
#:
#: On three worlds it gives nothing, and the arithmetic is the whole of it.
#: The rule that decides membership is one line -- a world joins this tuple
#: when the angle between its two pieces' leans is too small to show the
#: mirror -- and `measure/mirror-set-consistency.md` puts all eight worlds'
#: tilts and inter-piece angles in one table so a reader can check the rule
...
#:
#:   Jupiter's is 3.13 degrees.  Sol leans +3.13 and Anti-Sol -3.13, so the two
#:   pieces differ by 2 * 3.13 = 6.26 degrees of lean.  Also nothing: an
#:   independent critic shown the pair cold answered eight of twelve pair
#:   comparisons as two different objects and named this one as the exception,
#:   in these words -- "I cannot tell these apart and I would not notice if you
#:   swapped them."
#:
...
#:
#: All three are observed facts and none of them moves: `params.PLANETS` is
#: untouched, `params.lean_sign` is untouched and `features.planet_frame` is
#: untouched.  The defect is CAUSED by those three tilts, so inventing a lean
#: would be correcting the planet rather than the piece.
#:
...
#: it applies it to these three worlds on the Anti-Sol side alone.
#:
#: Jupiter is not like the other two, and the difference is worth naming here
#: because it is what `MERIDIAN_ADJUSTMENT` below is for.  Most of this globe
#: is INVARIANT under a longitude reflection: its six belts and five zones are
#: circles of latitude, which have the same longitude everywhere.  The only
#: features on it that carry a longitude at all are the Great Red Spot, its
#: collar, and the wave phase of the two widest belts, which are built as
#: longitude sectors.  The whole mirror read is therefore carried by the spot,
#: and `measure/mirror-meridian.md` is the sweep that decides how far it swings.
MAP_MIRRORED_WORLDS = ("mercury", "venus", "jupiter")
...
#:
#: Jupiter needs a much larger one, and unlike Venus's it is chosen for
#: VISIBILITY rather than to clear a floor.  Its spot sits at longitude -48.00
#: and its mean facing meridian on the Anti-Sol piece is -48.7675, so the plain
#: mean reflects the spot to -49.54: a swing of 2 * 0.7675 = 1.54 degrees of
#: longitude, 1.42 of great-circle arc, 0.33 mm on this globe -- and westward,
#: the wrong way for a spot that is already west of the meridian.  That is not a mirror, it is a rounding
#: error, and it would have produced a piece whose hash changed and whose
#: appearance did not.  Reflecting a feature at longitude L about a meridian C
#: lands it at 2C - L, so the swing is twice the distance from the spot to the
#: meridian: moving the meridian by A moves the spot by 2A.  Swept at a
#: hundredth of a degree against the +0.30 facing floor, every vertex of the
#: spot and its collar still clears at both photographed frames for any
#: adjustment in -15.55 to +21.57, and the margin peaks at +3.01.  +15.00 is
#: the largest whole degree that keeps at least half that peak margin, and it
#: is what is taken: the spot lands at -19.54, a swing of 28.46 degrees of
#: longitude, which at latitude -22 is 26.35 degrees of great-circle arc and
#: 6.20 mm on a globe 26.97 mm across -- two spot-lengths.  A degree of arc
#: here is pi * d / 360 = 0.2354 mm, a 360th of the circumference; it is NOT
#: pi * d / 180.  `measure/mirror-meridian.md` is the sweep, the clearing
#: range and the per-ring table.
MERIDIAN_ADJUSTMENT = {"venus": 5.00, "jupiter": 15.00}
...
def reflect_longitude(lon_deg: float, meridian_deg: float) -> float:
    ...
    The two obvious flips both fail, and they fail on these worlds
    specifically because every one of their longitudes was SOLVED for the
    camera:
...
def markings_for(planet: str, side: str):
    ...
    For the five worlds not named in `MAP_MIRRORED_WORLDS`, and for the Sol
    piece of the three that are, this returns the identical object `MARKINGS`
