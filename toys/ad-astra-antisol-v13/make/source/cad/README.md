# Antisol Jove Mirror — CAD project
...
**About this document.** It accumulates. Each correction in this set's chain
...
**This one is Antisol Jove Mirror, and it changes exactly one printed part:
`part_world_jupiter_anti`.** Nothing else in the set changes, and in particular
`part_world_jupiter_sol` does not.

Jupiter's two armies were the same object photographed twice. The previous
revision measured that rather than guessing it: asked cold, pair by pair, which
pairs are two different objects, an independent critic answered eight of twelve
comparisons as different and named this world as the exception — *"I cannot tell
these apart and I would not notice if you swapped them."* The cause is the one
Mercury and Venus had, and the cure is the machinery they already installed:
`parts/markings.MAP_MIRRORED_WORLDS` gains `"jupiter"` and `MERIDIAN_ADJUSTMENT`
gains `+15.00°`. There is no second mechanism.

**Jupiter is not like Mercury and Venus, and that is the whole of the work.**
Most of this globe is invariant under a longitude reflection: six belts and five
zones are circles of latitude, which have the same longitude everywhere, so the
reflection does not move them at all. Only three things on the piece carry a
longitude — the Great Red Spot, its collar, and the wave phase of the two widest
belts. The entire mirror read therefore falls on one 3 mm oval, and that oval
sits 0.77° from this world's mean facing meridian, so reflecting about the mean
would have swung it 1.54° of longitude: **0.33 mm, and westward.** A piece whose
hash changed and whose appearance did not. `MERIDIAN_ADJUSTMENT["jupiter"] =
15.00` is what buys a swing a person can see: the spot lands at −19.54°, a swing
of 28.46° of longitude, 26.35° of great-circle arc, **6.20 mm on a Ø26.97 globe
— two spot-lengths.** `measure/mirror-meridian.md` is the sweep, the clearing
range and the rule that chose the number inside it.

Jupiter's 3.13° obliquity did not move, `PLANETS` did not move, `lean_sign` did
not move and `planet_frame` did not move. The defect is CAUSED by that tilt, so
inventing a lean would have been correcting the planet rather than the piece.
`part_world_jupiter_anti.step` comes out byte-identical to the published set's,
because a flush colour inlay partitions the globe without moving its surface;
the change is in the seven colour bodies underneath, which
`measure/occurrence-geometry.md` measures.

Three of eight worlds now carry a mirrored map and five carry one map at two
leans. `measure/mirror-set-consistency.md` is that table, the rule that decides
it, and every world's tilt and inter-piece angle.

**The previous revision was Antisol Mirror, and it changed exactly three printed
parts: `part_world_mercury_anti`, `part_world_venus_anti` and
`part_corona_cell`.** Its own account follows and is left as it stood.
...
| `measure/mirror_meridian.py` | the meridian Mercury's, Venus's and Jupiter's Anti-Sol maps are reflected about, the two flips that were refused, the closure proof that lets Jupiter's twelve belt sectors be gated as two belts, every ring's facing at both photographed frames on both pieces against the +0.30 floor, the adjustment sweep per world on two scores, and what the mirror buys in degrees of arc and millimetres |
| `measure/mirror_set_consistency.py` | which of the eight worlds carry a mirrored map and which carry one map at two leans, the rule that decides it, and every world's obliquity and inter-piece lean angle so a reader can check the rule rather than trust it |
| `measure/jupiter_mirror.py` | the Jove correction's negatives: the Sol piece takes the identity path, no latitude moved anywhere, which region specs a longitude reflection returns unchanged, and the measurement that the South Equatorial Belt's hollow follows the spot to its new longitude by itself |
| `measure/quick_fix_carry.py` | what this quick correction carried forward, the sha256 each part was carried on, every carried report against the archive's own sanitization record, and the wall-clock it saved against the source run's logged figure |
...
| `part_world_jupiter_*` | 2 | Ø33.87 x 29.97 | disc, numeral, globe, four markings — six belts at their real unequal latitudes, the two widest drawn as outline rings with a 2.5° wave on each boundary; five bright `beige` zones between and beyond them; the Great Red Spot as one 13 by 9° oval; and its `cocoa_brown` collar. **The Anti-Sol piece carries the map MIRRORED**, for the same reason as Mercury's and Venus's: 3.13° of obliquity leaves only 6.26° between the two leans. The reflection is about −33.77°, which is the mean of the two photographed frames' own meridians moved +15.00° — chosen for VISIBILITY rather than to clear a floor, because at the plain mean the spot moves 0.33 mm. It is the only world in the set where almost nothing is reflected: the six belt latitudes and five zone latitudes are circles of latitude and are returned unchanged, so the mirror is carried by the spot, its collar and the wave phase of the two widest belts alone. The spot swings 28.46° of longitude, 6.20 mm on the ball, and the belt's hollow follows it. Latitudes, sizes, shapes and counts are untouched, and the printed solid is byte-identical |
...
circles read as a lozenge. Neptune is a mixture of the same kind, though it
arrived at it differently and has since been half reversed: its dark spot is an
...
# per-revision question rather than a fixed list.
python - <<'PAIRS'
...
### This revision (Antisol Jove Mirror)

The block above is the Caelus revision's and is left as it stood. This is what
the Jove correction actually ran, in order. `measure/mirror_meridian.py` and
`measure/markings_refactor.py` are re-runs of existing reports whose content
changed because Jupiter joined `MAP_MIRRORED_WORLDS`; the other four are new.

```bash
# the three worlds' mirror, the sweep that chose Jupiter's meridian, and the
# closure proof that lets its twelve belt sectors be gated as two belts
python <project>/measure/mirror_meridian.py > <project>/measure/mirror-meridian.md
python <project>/measure/markings_refactor.py > <project>/measure/markings-refactor.md

# which of the eight worlds are mirrored, and the rule that decides it
python <project>/measure/mirror_set_consistency.py \
    > <project>/measure/mirror-set-consistency.md

# the Jove correction's negatives, and the hollow that follows the spot
python <project>/measure/jupiter_mirror.py > <project>/measure/jupiter-mirror.md

# re-measured because the Anti-Sol Jupiter's colour bodies moved
python <project>/measure/jupiter_facing.py > <project>/measure/jupiter-facing.md
python <project>/measure/jupiter_atlas_resolution.py \
    > <project>/measure/jupiter-atlas-resolution.md
python <project>/measure/jupiter_tone_separation.py "$(workshop skills path)/cad/scripts" \
    > <project>/measure/jupiter-tone-separation.md
python <project>/measure/pair_separation.py > <project>/measure/pair-separation.md

# the hash proof, and the quick-mode carry-forward record it warrants
python <project>/measure/revision_hashes.py <product-root> <archive-root> \
    --identical world_jupiter_anti \
    > <project>/measure/revision-part-hashes.md
python <project>/measure/quick_fix_carry.py <product-root> <archive-root> \
    > <project>/measure/quick-fix-carry.md

# the assembly, occurrence by occurrence, against the published archive
python <project>/measure/occurrence_geometry.py <archive-root>/make/models/cad/antisol.step \
    > was.json
python <project>/measure/occurrence_geometry.py <project>/antisol.step > now.json
python <project>/measure/occurrence_geometry.py --compare was.json now.json \
    jupiter_anti_ > <project>/measure/occurrence-geometry.md

# the renders this correction invalidated: the whole-object pair, both rank
# ladders, and Jupiter's own eight frames. Every other frame under snap/worlds
# is carried forward, because no other world's geometry moved.
python <project>/world_views.py <scratch>/worlds jupiter \
  "$(workshop skills path)/cad/scripts" <project>/snap/worlds
python <project>/world_views.py ladder <scratch>/worlds \
  "$(workshop skills path)/cad/scripts" <project>/snap sol
python <project>/world_views.py ladder <scratch>/worlds \
  "$(workshop skills path)/cad/scripts" <project>/snap anti
python <project>/snap_frames.py "$(workshop skills path)/cad/scripts" <project>/snap
```
