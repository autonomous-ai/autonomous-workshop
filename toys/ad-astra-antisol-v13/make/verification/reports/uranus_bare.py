...
The Antisol Caelus revision removed both polar hoods and left Uranus the one
world in the set with no surface marking of any kind.  A later revision that
does not touch Uranus has to make the opposite statement -- that nothing came
back -- and this script makes whichever of the two its baseline calls for.  That is a NEGATIVE requirement, and a
...
def main() -> int:
    ...
    print()
    hoods_in_baseline = any(
        volume_of(was, "%s_%s_hood_%s_beige" % (PLANET, side, pole)) is not None
        for side in P.SIDES for pole in ("north", "south"))

    if hoods_in_baseline:
        print("The baseline this run is compared against still carries the two")
        print("hoods, so the arithmetic is the removal itself.")
        print()
        ...
        print("**The baseline this run is compared against carries no hoods.**")
        print("They were removed by the Antisol Caelus revision, which is the")
        print("edition this one corrects, so there is no removal left to do and")
        print("no volume left to hand back. The statement this run has to make")
        print("about Uranus is the opposite one: that it did not touch it.")
        print()
        print("| piece | published globe mm3 | this run mm3 | residue |")
        print("|---|---:|---:|---:|")
        for side in P.SIDES:
            stem = "%s_%s_" % (PLANET, side)
            globe_was = volume_of(was, stem + "globe_" + P.GLOBE_COLOUR[PLANET])
            globe_now = volume_of(now, stem + "globe_" + P.GLOBE_COLOUR[PLANET])
            if globe_was is None or globe_now is None:
                failures.append("%s: the globe occurrence is missing" % side)
                print("| %s | %s | %s | **missing** |" % (side, globe_was, globe_now))
                continue
            residue = globe_now - globe_was
            if abs(residue) > RESIDUE_TOLERANCE:
                failures.append("%s: the globe moved by %.4f mm3" % (side, residue))
            print("| %s | %.6f | %.6f | %+.6f |" % (side, globe_was, globe_now, residue))
        for side in P.SIDES:
            for pole in ("north", "south"):
                name = "%s_%s_hood_%s_beige" % (PLANET, side, pole)
                if volume_of(now, name) is not None:
                    failures.append("`%s` is back in the set" % name)
        print()
        print("Both globes come out at exactly the volume the published set")
        print("carries, and neither hood occurrence exists in either assembly.")
        print("This revision changes `part_world_mercury_anti`,")
        print("`part_world_venus_anti` and `part_corona_cell`, and nothing else;")
        print("`measure/revision-part-hashes.md` carries Uranus's two printed")
        print("STEPs as byte-identical and `measure/occurrence-geometry.md`")
        print("compares every occurrence in the set.")
        print()
    ...
    print("empty and neither piece carries a marking body.")
    if hoods_in_baseline:
        print("The globe's volume is its old volume plus both hoods to within")
        print("%.2f mm3." % RESIDUE_TOLERANCE)
    else:
        print("The globe's volume is exactly the published set's, because this")
        print("revision does not touch this world: the hoods were removed by the")
        print("edition it corrects, and nothing has been put back.")
    return 0
