...
def main():
    ...
    print()
    print("## A note on export determinism, because this run had to establish it")
    print()
    print("`cadgen` stamps a fixed epoch, so identical geometry gives identical")
    print("bytes -- but only when the exporting PROCESS is the same shape. The")
    print("four board panels are the one family in this project whose STEP")
    print("carries a `NEXT_ASSEMBLY_USAGE_OCCURRENCE` record, and that record's")
    print("first field is the exporter's own per-session occurrence counter.")
    print("Built four at a time on their own they come out `1`..`4`; built in")
    print("the run that also writes the 220-occurrence assembly they come out")
    print("`221`..`224`, which is what the published set carries.")
    print()
    print("Mid-run, generated on their own, all four differed from the published")
    print("copy in exactly that one line and in nothing else, diffed line by")
    print("line rather than asserted. A control settled that it had nothing to")
    print("do with this correction: the unedited archive source was unpacked")
    print("into a scratch tree and rebuilt on this machine, and produced the")
    print("same four hashes. The panels import nothing from `parts/markings`.")
    print("The hashes below are the final verifier's own regeneration, which")
    print("builds every entry in one process, and they match the published set")
    print("exactly. `measure/quick-fix-carry.md` carries the consequence for")
    print("what this quick correction was allowed to carry forward.")
    print()
    print("## What changed")
    print()
    ...
    print("carries only the first kind -- %d corrected part%s whose bytes must NOT"
          % (len(wanted), "" if len(wanted) == 1 else "s"))
    print("move -- and a second statement about the %d parts it was asked not to"
          % len(others_all))
    print("touch at all.")
    print()
    print("**The brief's own count checks out.** It says \"the other 23 printed")
    print("parts byte-identical\". This set has %d distinct printed geometries"
          % (len(others_all) + len(wanted)))
    print("and this revision names %d of them, so the number outside the"
          % len(wanted))
    print("correction is **%d**, which is what the brief says. Every one of the"
          % len(others_all))
    print("%d is checked below." % len(others_all))
    print()

    print("### 1. The mirrored world must be BYTE-IDENTICAL")
    print()
    ...
    print("`RELIEF_DEPTH` below the surface. If that hash had moved, something")
    ...
    if not identical_moved:
        print("**%s did not move.** %s byte-identical to the published"
              % ("They" if len(identical_wanted) > 1 else "It",
                 "%s are" % name_list(identical_wanted)
                 if len(identical_wanted) > 1
                 else "%s is" % name_list(identical_wanted)))
    ...
    print()
    if want_changed:
        print("### 2. The parts that must CHANGE")
        print()
        print("%s. These add or remove material, so byte-identical would mean"
              % name_list(changed_wanted))
        print("the correction did not happen.")
        print()
        changed_still = [name for name in changed_wanted if name not in changed]
        if not changed_still:
            print("**They changed.** %s carry new bytes."
                  % name_list(changed_wanted))
        else:
            print("- **%s did not change.**" % name_list(changed_still))
        print()
    else:
        print("### 2. Nothing in this run was asked to change SHAPE")
        print()
        print("This correction moves a marking and nothing else, so there is no")
        print("part whose bytes had to move. Every one of the 24 printed")
        print("geometries is expected to come back the same solid it was, and")
        print("the whole of the change lands in `parts/*.step` -- the per-colour")
        print("bodies underneath the surface -- which")
    ...
    print("That is every printed geometry this run was not asked to touch, and")
    print("`part_world_jupiter_sol` is among them -- the brief names that one")
    print("explicitly as a part that must not change.")
    print()
    if other_changed:
        ...
        print()
        print("**So the brief's word \"byte-identical\" is met on %d of the %d and"
              % (len([n for n in identical if n not in wanted]), len(others_all)))
        print("not on %d, and that is stated rather than rounded.** The four board"
              % len([n for n in explained if n not in wanted]))
        print("panels carry one differing line each and it is an exporter counter,")
        print("not geometry. It is not caused by this correction: the unedited")
        print("archive source was unpacked into a scratch tree and rebuilt on this")
        print("machine, and it produced these same four hashes. The panels import")
        print("nothing from `parts/markings`. `measure/quick-fix-carry.md` carries")
        print("that control and the decision it led to, which was to re-measure")
        print("all four rather than carry their gates forward.")
    print()
