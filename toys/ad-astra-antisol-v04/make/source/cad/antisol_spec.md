...
- **Markings** — flush colour inlays reaching 1.20 mm into the globe, described [assumed]
  ...
  minimum outline width hold from Mercury to Jupiter. **Five of the eight
  worlds are described by latitude, longitude and angular size** — Venus,
  Neptune, Uranus, Saturn and Jupiter — because a union of round patches or a
  belt of latitude is what a cloud pattern, a band system or a storm actually
  is. **Earth, Mars and Mercury are the three exceptions and are described by
  ...
  Those three and not the others, because those three are the worlds in this
  ...
  which every telescope owner has drawn since 1659 — and drawing either as
  circles throws that recognition away. Mercury carries no such silhouette:
  its albedo boundaries are soft and its plains are unnamed, so nothing there
  can be checked against a shape anybody remembers. It earns outlines for the
  ...
  is the one thing it does not read as. The remaining five keep their round
  ...
  Items 14, 16 and 18 of section 11 record what each of the three cost at its
  own globe size. [inferred]
...
| `cocoa_brown` | belt tiles, Mercury plains and Caloris rim, Mars albedo, Saturn bands, Jupiter bands |
...
18. **Mercury is the third and last world drawn from outlines, and its
    ...
    longitude corrections use. Venus's cloud Y and Jupiter's spot each had a
    genuine far-side problem and each was carried in longitude to fix it;
    Mercury did not, and carrying it would not have helped.
...
the tone this revision exists to replace. The two sealed Earth reference
...
the authority the Mars revision was corrected against, and the two Mercury
images, `ref/mercury-sol.png` and `ref/mercury-anti.png`, are the authority
this revision's Mercury was corrected against — except for their crater field,
which is deliberately not drawn for the reason and at the number item 18 gives.
Everything else all six show — the disc, the numeral, the proportions, the lean
and the filaments — still holds, and every other image in `ref/` remains fully
authoritative. The renders are flat-shaded with no ground
