...
| `parts/` | one module per physical part, plus `markings.py`, the per-planet surface table, and the three outline atlases: `atlas.py` for Earth's coastlines, `mars_atlas.py` for Mars's albedo map and `mercury_atlas.py` for Mercury's smooth plains and Caloris basin |
...
42 printed parts, 24 distinct printed geometries, 13 filaments. Mercury prints
in three of them — `gray`, `cocoa_brown` and `white` — where it printed in two;
the reason and the measurement are below.

## Earth, Mars and Mercury are the three worlds drawn from outlines

Five of the eight planets wear a union of round patches or a belt of latitude,
because that is what a cloud pattern, a band system or a storm actually is.
Earth, Mars and Mercury are the exceptions: they are the three worlds in this
set with a real mapped surface. Two of them carry a shape a player already
knows by heart — Earth's coastlines, and the dark triangle of Syrtis Major,
which every telescope owner has drawn since 1659 — and drawn as circles, Earth
reads as a mottled marble rather than as the planet and Mars loses the one
feature on it anybody can name. Mercury carries no such silhouette; its albedo
boundaries are soft and its plains are unnamed. It earns outlines for the other
half of the same argument: on the smallest globe in the set a union of discs
reads as a beach ball, and terrain is the one thing it does not read as. Nobody
carries Jupiter's band spacing or Venus's cloud Y in their head as an outline,
and a storm in an atmosphere really is a round patch, so for the other five a
union of round patches is not a compromise but the honest answer — Jupiter's
Great Red Spot and Neptune's dark spot stay round for exactly that reason.

All three are built the same way: closed longitude/latitude rings, swept into
...
concentric rings of its Caloris basin are in `parts/mercury_atlas.py`. The
globe's outer surface stays a true sphere on all three and nothing an outline
adds faces downward.

At Earth's Ø16.70 one degree of arc is 0.1457 mm against a 0.40 mm nozzle; at
Mars's Ø14.72 it is 0.1285 mm; at Mercury's Ø13.78 it is 0.1203 mm, so the
nozzle is 3.33 degrees of arc there and every outline on Mercury is the finest
of the three. `measure/earth-atlas-resolution.md`,
`measure/mars-atlas-resolution.md` and `measure/mercury-atlas-resolution.md`
...
between the plain and the seat. Nothing was dropped silently on any of the
three, and nothing the reference does not show was added: no craters on Mars or
Mercury, no Olympus Mons, no Valles Marineris, no fourth filament anywhere.

Mercury is also the one of the three whose repair was a colour rather than a
...
## What the three atlases do not draw
...
Nothing was dropped silently on any of the three worlds.
