# Antisol Kronos — design contract
...
*Kronos* is Saturn's older Greek name, and this revision is Saturn's. It is the
first correction in this chain that takes contrast away rather than adding it:
the piece wore four hard `cocoa_brown` bands on a `yellow` globe and read as a
striped gold ball, where its reference is the softest image in the set. Section
11, item 21 is the correction; **nothing else in the set changed, and Saturn's
ring system is not part of it and did not move.**
...
- **Markings** — flush colour inlays reaching 1.20 mm into the globe, described [assumed]
  ...
  minimum outline width hold from Mercury to Jupiter. **Uranus alone is now described entirely by
  latitude** — one band — because a belt of latitude is what a band system
  actually is. Saturn is described both ways, and since this revision so is
  Neptune: its clouds are short arcs rather than closed bands, so its eight
  streaks, its dark spot and the spot's bright companion are all outline
  rings (`parts/neptune_atlas.py`).
  ...
  **Jupiter is the eighth and is described both ways**, and that split is
  ...
  honest shape. A storm is not a circle, though, and neither is a cloud, and
  **Neptune left that group in this revision**: its three closed latitude
  bands became eight short tapered arcs and its dark spot became one oval
  twice as wide as it is tall, with the bright companion cloud beside it that
  the reference shows. Item 22 records it. Jupiter's spot went the same way
  earlier: the real
...
| Neptune | **eight short white cloud streaks, none of which circles the planet** — 46 to 90° of longitude long [assumed], no two the same length, at latitudes −52, −37, −10, +4, +17, +28, +38 and +47 [assumed], at eight longitudes spread over 133° [inferred], each tilted 3 to 12° off its own parallel and bowed 1 to 2° off it and back [assumed], each widest about a third of the way along and tapered to a rounded end, all 1.35 to 1.65 mm [assumed] wide across the middle and 0.69 to 0.99 mm [inferred] at the ends; and the **Great Dark Spot** at latitude −22 as one outline oval 28 by 14° of arc [inferred], capped by a small bright `white` companion cloud drawn parallel to its rim at a constant 0.61 mm [inferred] clearance. Six of the eight streaks face the camera at both product frames on the Sol piece and all eight do on the Anti-Sol piece; the two deepest southern streaks are Anti-Sol-only features, because the Sol piece's cameras cannot reach below latitude −21 at any longitude. `measure/neptune-facing.md`, `measure/neptune-atlas-resolution.md` | `dark_gray`, `white` on `blue` |
...
| filament | used for |
|---|---|
| `white` | Sol disc, Anti-Sol numeral, Mercury Caloris floor, Mars caps, Earth ice, Uranus band, Neptune cloud streaks and the dark spot's bright companion, Saturn ring and northern cap |
...
Neptune's cloud streaks are a seventh kind of evidence, and the one with the
largest deliberate departure from its own reference in it. What the piece
claims is that Neptune's clouds are SHORT -- that each one starts and stops, is
tilted a few degrees off its own parallel, and does not circle the planet --
and that its dark spot is one oval twice as wide as it is tall with a small
bright cloud beside it. Every one of those is in `ref/neptune-sol.png`. What
the piece does NOT claim is any streak's position: the eight latitudes,
longitudes, lengths, tilts, bows and tapers are a design, solved against the
four cameras in `measure/neptune-facing.md` rather than read off the image, and
the reference fixes only that the clouds are short, thin, tilted, unequal and
scattered. **And the width is six to twelve times the reference's own**, which
is the largest departure any world in this set has been given: the reference's
wisps scale to 0.13 - 0.26 mm here and these are drawn at 1.35 to 1.65 mm, [inferred]
because nothing finer than the 0.40 mm [assumed] nozzle can be printed as a colour
boundary and nothing near it can be seen at the scale the set is looked at.
That is the owner's decision of 2026-09-19, recorded in item 22 with the
arithmetic it was taken on, and it is not softened here. The other thing the
piece cannot claim is a marked far side: every streak and the spot are placed
where the product's two cameras look, so the hemisphere opposite the spot
carries the end of one streak and bare globe, which
`snap/worlds/neptune-sol-opposite.png` shows rather than hides.
...
rounded up. Motion verification is switched off for this run and
