...
## Carried forward again, unchanged, by the Antisol Mirror revision

This document and the two reports it carries forward -- `uranus-facing.md` and
`uranus-tone-separation.md` -- are copied into this revision's tree exactly as
the Antisol Caelus revision left them. **They are the record of a decision, not
evidence about this build**, and nothing in this revision cites them as
evidence. `measure/uranus_facing.py` and `measure/uranus_tone_separation.py`
are still kept as the method and are still not run: the first exits on an empty
marking table, which is the correct behaviour for a world that carries no
marking.

This revision does not touch Uranus. Its own evidence is
`measure/mirror-meridian.md`, `measure/markings-refactor.md`,
`measure/pair-separation.md`, `measure/corona-flat.md`,
`measure/seated-piece-clearance.md` and `measure/revision-part-hashes.md`,
every one of which was generated on this build.
