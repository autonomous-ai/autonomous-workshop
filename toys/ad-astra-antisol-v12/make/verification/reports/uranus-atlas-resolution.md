...
retires one:** the hoods' `beige` has left this piece, so where it
loaded four spools it now loads three.
...
shares those. **By that count Uranus is still two, and the two are
different ones.** It was `cyan` for the globe and its ring together
plus `beige` for the hoods; it is now `cyan` for the globe and `white`
for the ring. The tally did not move because two changes crossed: the
hoods took `beige` off the world and the ring's repaint put `white` on
it.

**The count that did fall is the piece's own.** Uranus loads three
spools where the published edition loaded four -- disc, numeral, globe
and ring, against disc, numeral, globe, hoods and ring -- so this is
the first correction in the chain to make a world SIMPLER to print.
