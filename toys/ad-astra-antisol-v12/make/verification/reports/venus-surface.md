...
## Are the two armies mirrors, and is the mirror in the map?

Venus's obliquity is 177.36 degrees, so its two pieces differ by 5.28
degrees of lean -- the same turn measured each way round -- and the
lean has no mirror to give. The Antisol Mirror revision mirrors this
world's MAP instead: on the Anti-Sol piece every marking longitude is
reflected to 2C - L about that piece's own facing meridian,
C = -123.8529 degrees. So the grid is compared twice, and the two
answers have to be opposite.

1647 points classified on both armies at half this grid's resolution;
35 of them are excluded because the collar cuts the two pieces at
Venusian latitudes 5.28 degrees apart, so a point near its edge can
be on one piece and inside the other's disc.

| comparison | points that disagree | of | share |
|---|---:|---:|---:|
| Anti-Sol at the SAME longitude | 272 | 1612 | 16.87% |
| Anti-Sol at the REFLECTED longitude | 0 | 1612 | 0.00% |

The first row is the defect this revision corrects and the second is
the correction, measured on the built colour bodies rather than on
the marking table. The residue in the second row is the grid's own
quantisation at a marking boundary.
