...
>
> This report measured Uranus's two polar hoods -- which pole faced which
> camera, on which army, at which frame -- and it was the evidence that a hood
> at each pole was necessary rather than chosen. **The owner removed both hoods
> in the Antisol Caelus revision.** Uranus now carries no surface marking of
> any kind, so every hood row below describes a feature that is not on the
> piece.
>
> It is kept, not deleted, because it is the measurement that justified the
> hoods and item 23 of the design contract still argues from it. Nothing in it
> is current evidence for this build. `measure/uranus_facing.py` is no longer
> run: it exits on an empty marking table.
>
> What IS still live in it is the ring's projection -- where the hoop lands
> against the globe's silhouette -- and that is re-measured on this run's own
> solids in `measure/uranus-ring.md`. The negative statement this build needs,
> that nothing at all survives on either globe, is measured in
> `measure/uranus-bare.md`. See `measure/retired-hood-evidence.md`.
