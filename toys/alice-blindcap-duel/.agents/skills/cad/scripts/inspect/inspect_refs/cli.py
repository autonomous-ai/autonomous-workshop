...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "examples:\n"
            "  scripts/inspect refs <project-dir>/<name>.step.py '#f9' --detail --facts\n"
            "  scripts/inspect measure <project-dir>/<name>.step.py --from '#f1' --to '#f2' --axis z\n"
            "  scripts/inspect align <project-dir>/<name>.step.py --moving '#f1' --target '#f2' --mode flush --axis z\n"
        ),
    )
    parser.add_argument(
        "--verbose",
    ...
    refs_parser = subparsers.add_parser(
        ...
        epilog=(
            "examples:\n"
            "  scripts/inspect refs <project-dir>/<name>.step.py '#f9' --detail --facts\n"
            "  scripts/inspect refs <project-dir>/<name>.step.py '#f1' '#e2' --positioning\n"
            "  scripts/inspect refs <project-dir>/<name>.step.py --input-file tmp/refs.txt --planes\n"
        ),
    )
    ...
    interfere_parser = subparsers.add_parser(
        ...
        epilog=(
            "examples:\n"
            "  scripts/inspect interfere <project-dir>/<name>.step.py\n"
            "  scripts/inspect interfere <project-dir>/<name>.step.py --refs o1.1,o1.7\n"
            "  scripts/inspect interfere <project-dir>/<name>.step --tolerance 25\n"
        ),
    )
    ...
    validate_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "  scripts/inspect validate <project-dir>/<name>.step.py\n"
            "  scripts/inspect validate <project-dir>/<name>.step.py --refs o1.1,o1.7\n"
            "  scripts/inspect validate <project-dir>/<name>.step.py --allow-open\n"
        ),
    )
...
def inspect_command_result(argv: Sequence[str]) -> tuple[int, dict[str, object]]:
    ...
    try:
        if args.command == "refs":
            ...
            # Keep mandatory geometry gates available to batch callers.  Calling
            # their APIs directly avoids nested stdout capture while preserving
            # the exact command-line defaults.
            from cadgen import interference
