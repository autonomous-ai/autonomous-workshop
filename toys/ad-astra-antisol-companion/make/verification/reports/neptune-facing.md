...
| per-world polar | 0 | 61.680 | Sol | +90.0 | +180.0 |
| per-world polar | 180 | 61.680 | Anti-Sol | +90.0 | +46.3 |
...
## The companion, at every frame

New in this revision, and short, so unlike a band it has a
longitude that can put it on the wrong face. Its centre is at
lat -30.0, lon -65.2 -- the spot's own longitude, 8.0 degrees of
latitude south of the spot's centre.

| frame | army | centre | best vertex | worst vertex | |
|---|---|---:|---:|---:|---|
| hero | Sol | +0.41 | +0.45 | +0.37 | clears the floor |
| hero | Anti-Sol | +0.79 | +0.82 | +0.76 | clears the floor |
| state sheet | Sol | +0.15 | +0.19 | +0.10 | **under the floor** |
| state sheet | Anti-Sol | +0.62 | +0.67 | +0.57 | clears the floor |
| per-world hero | Sol | +0.41 | +0.45 | +0.37 | |
| per-world hero | Anti-Sol | +0.79 | +0.82 | +0.76 | |
| per-world spot | Sol | +0.79 | +0.82 | +0.75 | |
| per-world spot | Anti-Sol | +0.86 | +0.89 | +0.82 | |
| per-world opposite | Sol | -0.79 | -0.75 | -0.82 | |
| per-world opposite | Anti-Sol | -0.86 | -0.82 | -0.89 | |
| per-world polar | Sol | -0.50 | -0.46 | -0.54 | |
| per-world polar | Anti-Sol | -0.50 | -0.46 | -0.54 | |

The companion is 8.0 degrees of latitude SOUTH of the spot, and
on the Sol piece both product cameras look DOWN on the northern
hemisphere, so every southward step costs facing there and gains
it on the Anti-Sol piece. That is the same geometry `b1` is
recorded under above, and the companion's latitude is not a
placement choice made for the camera: 8.0 degrees south is the
owner's own number, and `measure/neptune-atlas-resolution.md`
shows there is no other latitude at this longitude that clears
both the spot and the Sol piece's seat collar at any size.

**Where it falls below the +0.30 floor:**

- the state sheet frame on the Sol piece: centre +0.15, best vertex +0.19

Both Neptune pieces carry the companion, and the frame that
shows it is the per-world `spot` frame -- the level camera
aimed at the dark spot, where the pair reads +0.79 on the Sol
piece and +0.86 on the Anti-Sol one. The `opposite` frame
looks at the far face and does not show it on either army,
which is what a short marking means and is why that frame is
the one that proves the BANDS ring the planet. The product's
hero frame carries it on both pieces, weakly on the Sol one.
All of that is disclosed here and in the product's
limitations rather than left for a reader of one frame to
report as a missing marking.

## The per-world frames, and what they now show

- `hero` (azimuth -55, elevation 22): the product's own azimuth: three closed white bands running the whole way round a blue globe, unequal in width and unevenly spaced, with the dark oval below the lowest of them and the small white companion oval nested into that oval's southern rim, which `measure/neptune-facing.md` measures at every frame on both armies
- `spot` (azimuth -82, elevation 0): the Great Dark Spot in the middle of the picture, a clean oval about twice as wide as it is tall, low and south of the equator, with the small white companion oval nested into its southern rim at the same longitude -- the two touch, and the spot is scalloped where the cloud sits against it -- bare blue globe all round the pair, and the nearest white band a clear band-width below them. This is the frame the companion reads best at on both armies
