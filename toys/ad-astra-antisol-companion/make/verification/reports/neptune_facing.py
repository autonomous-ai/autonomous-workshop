...
What has to be measured is the two SHORT markings.  The DARK SPOT is one: it
did not move, its facing is what the per-world `spot` camera azimuth was solved
for, and its dot products are measured again here and compared against the
archived ones.  The COMPANION CLOUD is the other, and it is new.  It sits at the
spot's own longitude and 8.0 degrees of latitude south of it, so it is a short
marking with a longitude of its own and it can be on the face a camera sees or
on the face it does not.  Its facing is measured here at every frame, and where
it falls below the floor that is recorded as a fact about the frame rather than
repaired by moving it: its latitude is the owner's own number and
`measure/neptune-atlas-resolution.md` shows no other latitude at this longitude
works at any size.
...
def companion_worst(axis) -> float:
    """The worst dot product any VERTEX of the companion's ring reaches."""
    return min(dot(axis, unit(lon, lat)) for lon, lat in N.COMPANION_RING)


def companion_best(axis) -> float:
    """The best dot product any VERTEX of the companion's ring reaches."""
    return max(dot(axis, unit(lon, lat)) for lon, lat in N.COMPANION_RING)
...
def main() -> int:
    ...
    print("## The companion, at every frame")
    print()
    print("New in this revision, and short, so unlike a band it has a")
    print("longitude that can put it on the wrong face. Its centre is at")
    print("lat %+.1f, lon %+.1f -- the spot's own longitude, %.1f degrees of"
          % (N.COMPANION_LAT, N.COMPANION_LON, N.SPOT_LAT - N.COMPANION_LAT))
    print("latitude south of the spot's centre.")
    print()
    print("| frame | army | centre | best vertex | worst vertex | |")
    print("|---|---|---:|---:|---:|---|")
    companion_centre = unit(N.COMPANION_LON, N.COMPANION_LAT)
    hidden = []
    for name in product_names:
        for side, table in armies:
            value = dot(table[name], companion_centre)
            best = companion_best(table[name])
            worst = companion_worst(table[name])
            mark = "clears the floor" if value >= FLOOR else (
                "**behind the limb**" if best <= 0.0 else "**under the floor**")
            if value < FLOOR:
                hidden.append((name, side, value, best))
            print("| %s | %s | %+.2f | %+.2f | %+.2f | %s |"
                  % (name, side, value, best, worst, mark))
    for name in world_names:
        for side, table in armies:
            value = dot(table[name], companion_centre)
            best = companion_best(table[name])
            worst = companion_worst(table[name])
            print("| %s | %s | %+.2f | %+.2f | %+.2f | |"
                  % (name, side, value, best, worst))
    print()
    print("The companion is %.1f degrees of latitude SOUTH of the spot, and"
          % (N.SPOT_LAT - N.COMPANION_LAT))
    print("on the Sol piece both product cameras look DOWN on the northern")
    print("hemisphere, so every southward step costs facing there and gains")
    print("it on the Anti-Sol piece. That is the same geometry `b1` is")
    print("recorded under above, and the companion's latitude is not a")
    print("placement choice made for the camera: 8.0 degrees south is the")
    print("owner's own number, and `measure/neptune-atlas-resolution.md`")
    print("shows there is no other latitude at this longitude that clears")
    print("both the spot and the Sol piece's seat collar at any size.")
    print()
    if hidden:
        print("**Where it falls below the +%.2f floor:**" % FLOOR)
        print()
        for name, side, value, best in hidden:
            print("- the %s frame on the %s piece: centre %+.2f, best vertex "
                  "%+.2f%s" % (name, side, value, best,
                               " -- **behind the limb**" if best <= 0.0
                               else ""))
        print()
        print("Both Neptune pieces carry the companion, and the frame that")
        print("shows it is the per-world `spot` frame -- the level camera")
        print("aimed at the dark spot, where the pair reads %+.2f on the Sol"
              % dot(sol["per-world spot"], companion_centre))
        print("piece and %+.2f on the Anti-Sol one. The `opposite` frame"
              % dot(anti["per-world spot"], companion_centre))
        print("looks at the far face and does not show it on either army,")
        print("which is what a short marking means and is why that frame is")
        print("the one that proves the BANDS ring the planet. The product's")
        print("hero frame carries it on both pieces, weakly on the Sol one.")
        print("All of that is disclosed here and in the product's")
        print("limitations rather than left for a reader of one frame to")
        print("report as a missing marking.")
    else:
        print("It clears the +%.2f floor at every product frame on both"
              % FLOOR)
        print("armies.")
    print()
