...
![QUAYSHIFT](make/verification/renders/iso.png)

Build the town. Let the ferry through. Rebuild a miniature canal town from eight architectural pieces, then guide its ferry through two high bridges while avoiding the low arch. Includes three spatial challenges.

[View the verified public product page](https://www.autonomous.ai/toys/product/quayshift)
...
| Inventor | [Arden Span](../../inventors/arden-span/) |
| Factory | https://www.autonomous.ai/toys/product/quayshift |
...
**Input:** the creator's request. **This toy's input:** Build the town. Let the ferry through. Rebuild a miniature canal town from eight architectural pieces, then guide its ferry through two high bridges while avoiding the low arch. Includes three spatial challenges.
...
**Input:** the frozen Wish, eligible Inventor roster with each bound Taste/skill bundle, and the product blueprint. **Output:** **Arden Span** was selected and produced **QUAYSHIFT** — Build the town. Let the ferry through. A compact canal-town puzzle with two high bridges, a low cabin stop and freely placed architectural pieces. **Concept parts:** A, B, C, D, E, F, G, H, tray, ferry. The complete compact concept is in [make/invented.json](make/invented.json). Spark has no separate Invent Goal; the accepted Inventor assignment and compact Make concept are preserved separately.

### 3. Make — turn the concept into exact product bytes

**Input:** the accepted concept, selected Inventor identity/Taste, blueprint, and any bounded revision evidence. **Output:** Build the town. Let the ferry through. Rebuild a miniature canal town from eight architectural pieces, then guide its ferry through two high bridges while avoiding the low arch. Includes three spatial challenges. The sealed snapshot contains 12 STEP and 5 product render PNGs, together with [CAD source](make/source/), [models](make/models/), and [deterministic verification](make/verification/). [The Made contract](make/made.json) binds those exact bytes.
...
**Input:** the exact sealed Release package plus host-held Factory authorization; credentials never enter the native session. **Output:** [the public Factory product](https://www.autonomous.ai/toys/product/quayshift) and a sanitized, hash-verified [publication readback](publication/PUBLICATION.json).
...
| Wish to verified publication | 3h 3m 37s (2026-09-13T22:43:02Z to 2026-09-14T01:46:39.378640+00:00) |
...
uv run workshop wish --agent codex --model gpt-6-astra --effort medium --workflow spark 'Build the town. Let the ferry through. Rebuild a miniature canal town from eight architectural pieces, then guide its ferry through two high bridges while avoiding the low arch. Includes three spatial challenges.'
```
...
The [archive note](publication/ARCHIVE-NOTE.md) explains the later GitHub
publication, dated Factory evidence and the path redactions. All STEP and
Python source bytes are preserved; the generated index and verification
report are public projections documented in `SANITIZATION.json`.
