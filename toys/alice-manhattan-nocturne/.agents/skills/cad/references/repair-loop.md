...
```bash
python "$CAD_SKILL_ROOT/scripts/export" <entry>.step.py --stl --mesh-tolerance 0.01
```

**To unblock a print in the meantime:**

```bash
python "$CAD_SKILL_ROOT/scripts/repair_mesh" part_<role>.stl -o part_<role>.fixed.stl
python "$CAD_SKILL_ROOT/scripts/check_mesh" part_<role>.fixed.stl --bed 220x220x250
```
