...
FRAMES = {
    ...
    "neptune": {
        ...
        # pieces.  `measure/neptune-facing.md` carries the sweep.
        #
        ...
        # taste.  Neptune's markings run from latitude -52 to +44 -- the widest
        # spread of any world in the set -- and its dark spot is southern, so a
        # camera that looks DOWN foreshortens exactly the half the correction
        # is about.  At 0 the camera's sub-point sits within a few degrees of
        # the equator on both armies and all eight streaks clear the facing
        # floor on both; at +12 two of them do not.  Not below 0: a world's
        # disc is Ø34.00 and every globe is smaller, so a camera under the
        # equator sees the underside of the disc.  The two sealed reference
        # images are level shots as well.
        "spot": (-82.0, 0.0,
                 "the Great Dark Spot in the middle of the picture, a clean "
                 "oval about twice as wide as it is tall, low and south of the "
                 "equator, with its small bright companion cloud just off its "
                 "upper edge and short white streaks scattered above and below "
                 "it, none of them reaching round the planet"),
        "opposite": (98.0, 0.0,
                     "the far face: the ends of the same streaks coming round "
                     "the two limbs, bare blue globe across the middle, and no "
                     "dark spot anywhere in the picture"),
    },
    "saturn": {
...
#: across a table by surface rather than by size.  This frame is the evidence
#: for whether they can.
NEIGHBOUR_GAP = 42.0
...
def render_neighbours(first: str, second: str, steps: Path, scripts: Path,
    ...
    are the ones `measure/neptune_uranus_separation.py` reads:
