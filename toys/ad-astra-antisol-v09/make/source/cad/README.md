...
| `part_world_neptune_*` | 2 | Ø33.87 x 24.89 | disc, numeral, globe, two markings — **eight short cloud streaks, not three closed bands**: 46 to 84° of longitude long, no two the same, at eight unequal latitudes from −52 to +43, each tilted a few degrees off its own parallel and tapered to a rounded end, drawn 1.35 to 1.65 mm wide; and the Great Dark Spot as one outline oval twice as wide as it is tall with a small bright `white` companion cloud beside it |
...
circles read as a lozenge. Neptune has since become a mixture of the same
kind, and for the same reason: its dark spot is an outline oval rather than two
overlapping circles, and its clouds are eight short tapered outline arcs rather
than three closed belts of latitude, because a closed belt runs the whole way
round the planet and `ref/neptune-sol.png` shows wisps that start and stop.
...
python <project>/measure/neptune_uranus_separation.py \
