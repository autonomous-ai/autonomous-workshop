...
`a734435cb30589311abe1b78ec84ad11d95d9f5faec463e35b00e1c6b758190d`.
...
| `part_panel_northeast.step` | `f77045ff795e752f` | `6b72fb6b87c4d2f2` | 1 line differs, NEXT_ASSEMBLY_USAGE_OCCURRENCE: an exporter occurrence counter, no geometry. Every other line matches the archive copy, checked here rather than asserted |
| `part_panel_northwest.step` | `24bfc51bb611bcf8` | `af46270fd67bb4b0` | 1 line differs, NEXT_ASSEMBLY_USAGE_OCCURRENCE: an exporter occurrence counter, no geometry. Every other line matches the archive copy, checked here rather than asserted |
| `part_panel_southeast.step` | `c4c8dc59f850d264` | `00e504d06c80d87c` | 1 line differs, NEXT_ASSEMBLY_USAGE_OCCURRENCE: an exporter occurrence counter, no geometry. Every other line matches the archive copy, checked here rather than asserted |
| `part_panel_southwest.step` | `1146cc5781bf46ca` | `02586c970402827d` | 1 line differs, NEXT_ASSEMBLY_USAGE_OCCURRENCE: an exporter occurrence counter, no geometry. Every other line matches the archive copy, checked here rather than asserted |
...
| `part_world_uranus_anti.step` | `8541bfb66d4960d1` | `d993913622975638` | **changed** |
| `part_world_uranus_sol.step` | `495ebfbadf5cfd3a` | `7eccfab310b21fef` | **changed** |
...
- **18 of the 24 printed geometries are byte-identical.**
...
- **Nothing moved that was not asked to.** The other fourteen
  worlds, the four board panels, the belt tile, the corona tile,
  the star and the tray are the same geometries the published set
  carries, and the four panels are the same shape written with a
  different occurrence counter.

## The two statements, and which one the hashes support

Every previous run in this chain expected its two changed world parts to
come out byte-identical, because a marking in this set is a flush colour
inlay: it partitions the globe's volume without moving the printed
solid's own boundary, so the correction lands entirely in
`parts/*.step`, which `measure/occurrence-geometry.md` measures.

**This run is the exception and expected the opposite.** A ring is
added material. If `part_world_uranus_sol.step`, `part_world_uranus_anti.step` had come out byte-identical the ring did not
get built, and no other evidence in the run would be worth reading.

**The hashes support the second statement: both corrected parts
changed.** `part_world_uranus_sol.step`, `part_world_uranus_anti.step` carry new bytes, and what is in them is a solid
hoop standing in the globe's own equatorial plane, 55 mm3 of it
per piece. `measure/uranus-ring.md` measures it on the built
body; `measure/uranus-mirror.md` shows the two hoops are mirrors.
...
time of the run, so all 236 differ between any two runs whatever the
