"""Two things independent readers reported seeing, answered with geometry.

Both are claims about the solids, made from the rendered frames, and both are
answered here by measuring the solids rather than by arguing with the reader.
One is about protrusion and one is about faceting.

**PROTRUSION.**  An independent reader of `snap/worlds/neptune-*.png` reported
that the white markings "are solid raised lenses that break the ball's
silhouette", with "thin overhanging slivers" and "unsupported knife-edged lips
over the puck".  That is a claim about geometry and about two of this
correction's negative requirements -- nothing protrudes, and no downward-facing
surface was added -- so it is answered with geometry.
...
ellipse is measured below as well.
...
def main() -> int:
    ...
    print("**%+.6f mm** -- the two markings touch the sphere exactly and"
          % worst)
    print("neither crosses it.")
    print()
    if worst > TOLERANCE:
    ...
    print("image, the white wisps included, because it does not antialias.")
    ...
    print()
    print("## Why a reader sees otherwise")
    print()
    print("Reported blind, from the rendered frames: the markings \"are solid")
