...
FLAMES = 32                  # [specified] this correction fixes the count
...
TIP_ACROSS_LO, TIP_ACROSS_HI = 2.00, 2.60     # [specified] tip cap, mm
TIP_Z_MIN = 3.0              # [specified] tip at least 3.0 mm tall in Z
ROOT_FRACTION_MAX = 0.55     # [specified] root over root-to-root arc
MID_OVER_ROOT_MIN = 0.45     # [specified] width at half length over root width
MINORITY_SHARE_LO = 1.0 / 3.0   # [specified] at least one flame in three leans
MINORITY_SHARE_HI = 0.5         # the other way, and at most half of them do
...
def check_corona():
    ...
    assert n == FLAMES, "the correction fixes the flame count at 32"
    ...
    assert not set(P.CORONA_TALL) & set(P.CORONA_SHORT), "a flame is tall and short"
    assert max(P.CORONA_TALL + P.CORONA_SHORT) < n, "a band index is off the ring"
    ...
    assert set(spacing) != {4}, "the tall flames are every fourth flame"
    for k in range(1, n):
    ...
    # 4. the approved flame curve, measured on the built spine. Only its sign
    #    varies now: at least one flame in three leans the other way, in runs
    #    of one to three, never strictly alternating and never in a pattern
    #    that repeats round the ring.
    senses = [G.tongue_sense(i) for i in range(n)]
    assert set(senses) == {1, -1}, "every flame still turns the same way"
    minority = min(senses.count(1), senses.count(-1)) / n
    assert MINORITY_SHARE_LO - 1e-9 <= minority <= MINORITY_SHARE_HI + 1e-9, (
        "%.4f of the flames lean the minority way" % minority)
    runs = G.sense_runs()
    assert sum(runs) == n, "the handedness runs do not close the ring"
    assert max(runs) <= MAX_SENSE_RUN, "a handedness run is %d long" % max(runs)
    assert max(runs) >= 2, "the handedness strictly alternates"
    for k in range(1, n):
        assert any(senses[i] != senses[(i + k) % n] for i in range(n)), (
            "the handedness repeats at shift %d" % k)
    # the mirror is exact, so flipping a sign cannot move a root off the circle
    for i in range(n):
        frame = G.tongue_frame(i)
        assert abs(frame["sense"]) == 1.0, "flame %d has no unit sense" % i
    ...
    # 5. a tongue, not a fin: the width at half a flame's length is at least
    #    45 per cent of its own root width, and the tip cap is a quarter to a
    #    third of the base
    mids = [G.tongue_mid_over_root(i) for i in range(n)]
    assert min(mids) >= MID_OVER_ROOT_MIN, (
        "flame width at half length is %.4f of the root" % min(mids))

    # 6. blunt tips, a falling top, and nothing under the stated caps
    ...
    # 7. the envelope, and nothing inside the base circle
    xs, ys, radii = [], [], []
    for i in range(n):
    ...
    return {
        ...
        "flames_leaning_each_way": [senses.count(1), senses.count(-1)],
        "minority_handedness_share": round(minority, 4),
        "handedness_runs": runs,
        "width_at_half_over_root": [round(min(mids), 4), round(max(mids), 4)],
