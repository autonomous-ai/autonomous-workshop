"""The corona: thirty-two separate flames standing off one unbroken base circle.
...
flames never share a root point, and nothing on this part is a closed loop.
...
breaking wave, not a straight shaft with a hook on the end. Which way it turns
is now this flame's own: `params.CORONA_SENSE` gives fourteen of the
thirty-two the mirror of that curve about their own radial midline, in runs of
one to three, so the fringe flicks instead of spinning. The mirror is exact --
same root chord, same turn magnitude, same taper, same cap, same ramp -- and
the whole part stays one flat plan piece, because a flame's curvature lives
entirely in X and Y and the 45-degree rule governs Z alone.

Its half width leaves the root chord flat and arrives at the blunt tip cap
steeply: it falls as 1 - t**1.50, so the lick holds most of its width through
the lower half and then narrows hard into the point, instead of shedding its
width immediately above the root and running out as a near-parallel filament.
The top of the solid then falls from the deck height at the root to a lower
tip, cut by a single plane so the ramp stays flat and its meeting with each
vertical flank stays a crisp line.
"""
...
def flame_cap(index):
    """Tip cap radius: 1.30 mm on the shortest flame, 1.00 mm on the tallest.

    The falloff is squared, so only the shortest nubs carry the widest cap and
    the long licks spend most of their length already near the 1.00 mm one.
    Across the plan that is a cap of 2.00 to 2.60 mm, a quarter to a third of
    an 8.05 mm root.
    """
...
def tongue_frame(index):
    ...
    # the base sense curls the flame toward the neighbour it faces at B, which
    # is the direction the ring is numbered; CORONA_SENSE[index] keeps that or
    # mirrors this flame about its own radial midline
    sense = -1.0 if u0[0] * m0[1] - u0[1] * m0[0] > 0.0 else 1.0
    sense *= P.CORONA_SENSE[index % P.CORONA_COUNT]
...
def tongue_half_width(frame, t):
    ...
    The surplus over the cap falls as 1 - t**CORONA_TAPER_EXP. With the
    exponent above one that curve leaves the root flat and arrives at the cap
    steeply, which is a tongue; the (1 - t)**exp it replaces did the opposite
    and produced a fin with a parallel filament on the end.
    """
    surplus = 0.5 * frame["root_width"] - frame["cap"]
    t = min(max(t, 0.0), 1.0)
    return frame["cap"] + surplus * (1.0 - t ** P.CORONA_TAPER_EXP)
...
def tongue_mid_over_root(index):
    """Width at half a flame's length, as a share of its own root width."""
    frame = tongue_frame(index)
    return tongue_mid_width(index) / frame["root_width"]


def tongue_sense(index):
    """+1 if flame i curls the way the whole ring used to, -1 if it mirrors."""
    return P.CORONA_SENSE[index % P.CORONA_COUNT]


def sense_runs():
    """Circular run lengths of one handedness, starting at the first change."""
    n = P.CORONA_COUNT
    senses = [tongue_sense(i) for i in range(n)]
    start = next(i for i in range(n) if senses[i] != senses[(i - 1) % n])
    runs, run = [], 1
    for k in range(1, n):
        if senses[(start + k) % n] == senses[(start + k - 1) % n]:
            run += 1
        else:
            runs.append(run)
            run = 1
...
def converging_pairs():
    """Neighbour pairs (i, i+1) that lean toward each other."""
    n = P.CORONA_COUNT
    return [i for i in range(n)
            if tongue_sense(i) > 0 and tongue_sense(i + 1) < 0]


def flame_clearance(index, other, samples=192):
    """Narrowest plan distance between two flame outlines, in mm."""
    a = tongue_outline(index, samples)
    b = tongue_outline(other, samples)
    return min(hypot(p[0] - q[0], p[1] - q[1]) for p in a for q in b)
