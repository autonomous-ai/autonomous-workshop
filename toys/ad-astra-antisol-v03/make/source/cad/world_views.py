...
millimetres of blue and Mars is less.  Both corrections this set has carried
are surface outlines, so each needs a frame where an outline is what you see:
one exact piece, alone and large, with the camera on the face the Wish names.

Written for Earth's coastline correction and reused unchanged in shape for
Mars's albedo one -- the only thing either world needs of it is its own list
of frames, so `FRAMES` is keyed by planet and `polar_frame` takes the planet
whose pole it is looking down.
...
that is plainly one cue inverted and the surfaces are plainly the same.
    ...
    "$WORKSHOP_PYTHON" world_views.py snap/worlds mars
"""
...
def polar_frame(side: str, planet: str = "earth") -> tuple[float, float, str]:
