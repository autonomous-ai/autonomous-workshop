...
- **Markings** — flush colour inlays reaching 1.20 mm into the globe, described [assumed]
  ...
  minimum outline width hold from Mercury to Jupiter. **Six of the eight
  worlds are described by latitude, longitude and angular size**, because a
  union of round patches is what an albedo map, a cloud pattern or a storm
  actually is. **Earth and Mars are the two exceptions and are described by
  outline**: a closed ring of longitude/latitude vertices per feature, taken
  from a stylised outline atlas (`parts/atlas.py` for Earth,
  `parts/mars_atlas.py` for Mars), swept into the globe as the radial cone
  through the ring and trimmed to the same 1.20 mm depth. [assumed]

  Those two and not the others, because those two are the only worlds in this
  set whose real surfaces carry a shape a player already knows by heart:
  Earth's coastlines, and the dark triangle of Syrtis Major, which every
  telescope owner has drawn since 1659. Drawing either as circles throws that
  recognition away. No other world here has one — a band system, a cloud
  pattern and an albedo map of smooth plains have no outline anybody carries
  in their head, and for those a union of round patches is not a compromise
  but the honest shape. Items 14 and 16 of section 11 record what each of the
  two cost at its own globe size. [inferred]

| planet | markings | filaments |
|---|---|---|
| Mercury | albedo plains | `dark_gray` on `gray` |
| Mars | the classical albedo map as outlines — Syrtis Major's dark triangle, Mare Acidalium, Sinus Sabaeus and Meridiani, and the southern belt of Erythraeum, Sirenum, Cimmerium and Tyrrhenum; both polar caps, their rims broken by lobes. The second of the two worlds drawn from outlines, not circles | `cocoa_brown`, `white` on `red` |
| Venus | the ultraviolet cloud Y, inverted with the planet | `orange` on `beige` |
| Earth | continent coastlines, dry interiors, northern ice — the first of the two worlds drawn from outlines, not circles | `green`, `beige`, `white` on `blue` |
...
14. **Earth's surface is drawn from coastline outlines; the six worlds that
    carry no recognisable outline stay round patches.** A blob union is honest for Mercury's albedo map,
    Venus's cloud Y and Neptune's dark spot, because that is what those
    ...
    Earth is one of the two worlds here whose outline a player already knows;
    Mars is the other, and item 16 records it. The
...
16. **Mars's albedo map is drawn from outlines too, and it is the second and
    last world that earns them.** The six round patches this build was handed
...
17. **Two things an independent reader of this build's renders got wrong,
    ...
    **No render in this set shows the south cap**, because there is almost
    ...
    and nothing at all below it. `snap/worlds/mars-sol-south.png` and its
    Anti-Sol twin were added to look for the cap from under the equator; they
    show the southern belt fused into one band, which is worth having, but no
    cap. The asymmetry is real in
...
army and the southern belt is cut where it runs low. The two sealed Earth
reference images, `ref/earth-sol.png` and `ref/earth-anti.png`, show the
round-patch surface an earlier revision replaced and are no longer the authority
on Earth's markings; the two Mars images, `ref/mars-sol.png` and
`ref/mars-anti.png`, are the authority this revision's Mars was corrected
against and remain so. Everything else all four show — the disc, the numeral,
the proportions, the lean and the filaments — still holds, and every other image
in `ref/` remains fully authoritative. The renders are flat-shaded with no ground
