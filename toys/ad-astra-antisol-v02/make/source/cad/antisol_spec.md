...
- **Markings** — flush colour inlays reaching 1.20 mm into the globe, described [assumed]
  ...
  minimum outline width hold from Mercury to Jupiter. **Seven of the eight
  ...
  actually is. **Earth is the exception and is described by coastline**: a
  closed ring of longitude/latitude vertices per landmass, taken from a
  stylised true outline atlas (`parts/atlas.py`), swept into the globe as the
  radial cone through the ring and trimmed to the same 1.20 mm depth. Earth is [assumed]
  the one world in the set whose real surface has an outline a player already
  knows by heart, and drawing it as circles throws that recognition away.
  Item 14 of section 11 records what this cost at a 16.70 mm globe. [inferred]
...
| Earth | continent coastlines, dry interiors, northern ice — the one world drawn from outlines, not circles | `green`, `beige`, `white` on `blue` |
...
14. **Earth's surface is drawn from coastline outlines; the other seven worlds
    stay round patches.** A blob union is honest for Mercury's albedo map,
    ...
    Earth is the only world here whose outline a player already knows. The
...
so neither is present — stated here rather than left to be noticed. The two
sealed Earth reference images, `ref/earth-sol.png` and `ref/earth-anti.png`,
show the round-patch surface this revision replaces and are no longer the
authority on Earth's markings; everything else they show — the disc, the
numeral, the proportions, the lean and the filaments — still holds, and every
other image in `ref/` remains fully authoritative. Motion verification is switched off for this run and
