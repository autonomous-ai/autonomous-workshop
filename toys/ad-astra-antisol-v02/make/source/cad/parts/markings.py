...
Seven of the eight worlds are described by latitude, longitude and angular
size, because a union of round patches is what an albedo map, a cloud pattern
or a storm actually is.  Earth is the exception and is described by coastline:
it is the one world in the set whose real surface has an outline a player
already knows by heart, and drawing it as circles throws that recognition
away.  Earth's rings live in `parts/atlas.py`.

Each entry is (colour, [region specs]).  A region spec is one of
    ("blob",    [(lat, lon, angular_radius), ...])
    ("band",    lat_low, lat_high)
    ("outline", [[(lon, lat), ...], ...])       closed rings, Earth only
