...
def compare(was: dict, now: dict) -> int:
    ...
    strays = [
        ...
        if not name.startswith(("earth_sol_", "earth_anti_"))
    ]
    print("## Verdict")
    print()
    if strays:
        print("**Outside the two Earth pieces: %s**" % ", ".join(sorted(set(strays))))
        return 1
    print("Nothing outside the two Earth pieces gained, lost or moved a solid.")
    return 0
