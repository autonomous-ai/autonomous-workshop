"""Rainward Corona parameters. All dimensions in mm; bed XY, +Z up, underside Z0.

Provenance tags: [specified] fixed by the correction Wish, [preserved] carried
unchanged from the Rainward Lowflow clone, [inferred] derived from a preserved
or specified value, [assumed] engineering choice recorded in rainward_spec.md.
"""
...
# the rim stays one continuous ring of body. The correction Wish reopened that
# radius: the skirt troughs now cut inward past 90, and the ring has to stay
# thick enough between the deepest trough floor (87.00) and the pocket wall.
# 84.5 leaves 2.5 mm of solid body there instead of 0.5 mm. Every tile still
# reaches 89.85, so a counter at the 85 mm station keeps the flat support it
# had; the outer part of each tile is a 1.0 mm lapping lip that rests on the
# ring.
...
# The tile pulls further back than the body in the free window at each lane
# boundary, so the flame crown shows its own base there instead of meeting the
# tile edge. The window closes before any counter at the outermost station.
TILE_BAY_R = 87.60           # [assumed] tile edge at the centre of a bay
TILE_BAY_INNER = 2.60        # [assumed] deg of full bay either side of a boundary
TILE_BAY_OUTER = 3.90        # [assumed] deg at which the tile is back at the rim
...
CORONA_TIP_MIN = 0.65        # [assumed] 1.3 mm across the thinnest tip
CORONA_TIP_MAX = 0.80        # [assumed] a fine rounded point, not a knob
CORONA_LEAN_FRAC = 0.08      # [assumed] tip offset as a share of the tongue span
CORONA_LEAN_DEG = 1.50       # [assumed] fixed lean, one sense all the way round
CORONA_LEAN_FULL_SPAN = 10.0 # [assumed] deg of span that earns the full lean
CORONA_TIP_SHARE = 0.22      # [assumed] cap radius ceiling, share of the root
CORONA_ROOT_PULL = 0.16      # [assumed] root-side flank handle, share of rise
CORONA_TIP_PULL = 0.20       # [assumed] tip-side flank handle; small values keep
                             # each flank a near-straight taper from a broad
                             # root to the tip instead of waisting it
