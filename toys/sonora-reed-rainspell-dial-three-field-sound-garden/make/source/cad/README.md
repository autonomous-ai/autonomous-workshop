...
This active handoff intentionally contains no `__cadgen__` directory and no `measure/verification-pipeline.md`. Those are run-specific verifier products: cache descriptors embed generation time, paths, and run identifiers, while the pipeline record embeds wall-clock time and elapsed durations. The preserved successful Make verification record is `../CAD-VERIFICATION.md`; an isolated verifier may create fresh cache and pipeline files without rewriting any declared file in this project.
