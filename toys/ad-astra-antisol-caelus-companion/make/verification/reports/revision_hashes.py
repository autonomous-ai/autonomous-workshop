...
def main():
    ...
    if expect_changed:
        ...
        if len(still) == len(wanted):
            ...
            print("is the check the whole correction turns on. Both of Uranus's")
            print("polar hoods were taken off the globe and the ring was moved")
            print("from `cyan` to `white`. Removing a flush inlay REMOVES A")
            print("BOUNDARY: the globe stops being partitioned and gets its own")
            print("volume back, and no surface anywhere moves. Repainting the ring")
            print("moves one existing solid from one spool to another and moves no")
            print("surface either. If either of those had touched the ball or the")
            print("hoop -- a cut gone deeper than `RELIEF_DEPTH`, a ring rebuilt at")
            print("a different diameter, a seat re-cut -- these two hashes would")
            print("have moved with it. They did not. The printed solid the shop")
            print("receives is the same solid it was, and the whole revision is in")
            print("which spool prints which part of it.")
            print("`measure/uranus-bare.md` measures the same statement on the")
            print("solids directly: the globe's volume now is its published volume")
            print("plus both hoods, and the ring's volume and bounding box are")
            print("unchanged to nine decimal places.")
        elif still:
