...
and it is not corrected here; it is also why the two armies agree closely, in
the way Mercury's 0.03 degrees make its two agree -- both leans are within a
few degrees of a half-turn apart from each other, so the mirrored lean moves a
marking by about five degrees rather than by twice the tilt.
...
def visible_fraction(name: str, view, side: str) -> float:
    ...
    seen = 0
    for lon, lat in ring:
        carried = lean(direction(lat, lon), side)
        if sum(one * other for one, other in zip(carried, axis)) <= 0.15:
            continue
        if math.degrees(math.asin(max(-1.0, min(1.0, carried[2])))) \
                <= P.SEAT_LATITUDE_DEG:
            continue
        seen += 1
...
def main() -> int:
    ...
    for label, view in FRAMES:
        print("## The %s frame: azimuth %g, elevation %g"
              % (label, view[0], view[1]))
        print()
        print("| province | tone | latitude | longitude +%g | Sol | Anti-Sol | faces the camera |"
              % offset)
        print("|---|---|---:|---:|---:|---:|---|")
        for name, role in PROVINCES:
            ...
            print("| `%s` | %s | %+.1f | %.1f | %+.2f | %+.2f | %s |"
                  % (name, role, lat, (lon + offset) % 360.0, sol, anti,
