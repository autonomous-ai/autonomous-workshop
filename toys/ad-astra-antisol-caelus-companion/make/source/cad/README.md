...
## This correction: Neptune's companion cloud (Antisol Caelus Companion)

**It changes no printed solid.** It adds one white colour inlay to each Neptune
piece: the Great Dark Spot's bright companion cloud, one outline oval 10 by 5
degrees of arc centred at latitude -33.5, longitude -75.2 -- 11.5 degrees south
and 10 degrees west of the spot's centre, inside the spot's east-west span. The
brief said "about 8 south, same longitude". At 8 it would overlap the spot's
southern rim; on the spot's meridian the Sol piece's lean puts a printable
companion 41% under the seat cone (the first build of this run did exactly
that, and the blind reader called it blocking). The offset is the smallest
that clears both. Nearest approach to the spot's rim: 3.07 degrees, 0.587 mm
at the surface and 0.526 mm at the 1.20 mm inlay floor, over the 0.40 mm
nozzle. It is its own marking, `companion`, in `white`, listed after the spot
and the bands in `parts/markings.py`; the arithmetic is in
`parts/neptune_atlas.py`.

- All 24 printed `part_*.step.py` entries rebuild to the source archive's exact
  B-rep identity (`measure/quick-fix-carry.md`): a printed world is the fused
  disc, ball and ring, and never reads the marking table.
- `measure/neptune-companion.md` measures the companion on the built bodies --
  size, place, flushness, both gaps -- and shows the spot, bands, disc and
  numeral of both pieces keep the source's exact volume and bounding box.
- On both pieces the companion lies wholly above the seat cone (lowest point at
  piece latitude -39.5 on the Sol piece, where the seat springs at -42), measured
  in the same report.
- `companion_frame.py` writes `snap/worlds/neptune-<side>-companion-lit.png`: the
  exact piece turned so the companion faces the renderer's fixed light, because
  at the level per-world frames the southern hemisphere is in shade and white
  renders mid grey there.
- `measure/render-diff.md` compares every regenerated render with the source
  archive's: the only real change is the companion.
- The assembly gains exactly two occurrences, 220 -> 222.
...
| `companion_frame.py` | the Neptune pieces turned so the companion cloud faces the light |
