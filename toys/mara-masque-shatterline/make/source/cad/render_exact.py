...
def load(path,neutral=False):
    ...
    for leaf in leaves(import_step(path)):
        ...
        color=(0.62,0.62,0.62) if neutral else (tuple(leaf.color)[:3] if leaf.color else (0.55,0.50,0.45))
...
def main():
    ...
    # Canonical overhead views. The marker habits must be nameable from the
    # neutral one, which carries no colour at all.
    overhead=[0.0,-0.02,1.0]
    render(hero,overhead,hero[0].reshape(-1,3)).save(out/'top.png')
    grey=load(HERE/'veinwake.step',neutral=True)
    render(grey,overhead,grey[0].reshape(-1,3)).save(out/'top-neutral.png')
    ...
    record={'schema_version':1,'scope':'Exact STEP presentation, no physical performance claim',
            ...
            'top_sha256':digest(out/'top.png'),'top_neutral_sha256':digest(out/'top-neutral.png'),
            'overhead_camera_forward':overhead,
