...
def lane_tile(point):
    ...
    The outer edge is one exact circular arc of radius 89.85, the same arc on
    every one of the twenty-four tiles, so the fan of tiles finishes on a
    single continuous circle. Nothing is notched, scalloped or bayed out of a
    tile end and no tile is shortened: the flame lives entirely outside this
    arc, where it already did.
    """
